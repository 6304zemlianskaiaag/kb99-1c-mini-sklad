...
// GRACE:MODULE_START name="ВетИС_ЗаполнениеДокументовСервер"
// @grace-module: ВетИС_ЗаполнениеДокументовСервер
// @grace-purpose: Заполнение ВСД-документов данными: партии, продукция, упаковки, связанные документы
// @grace-depends: ВетИС_ЯдроСервер, ВетИС_УтилитыКлиентСервер, ВетИС_ДокументооборотСервер
// @grace-version: 2.0
// @grace-contract-ref: docs/Contracts.xml -> module="ВетИС_ЗаполнениеДокументовСервер"
...
//
// GRACE:CONTRACT function="ЗаполнитьСвязанныеДокументы"
Функция ЗаполнитьСвязанныеДокументы(ДокВСД) Экспорт
	
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьСвязанныеДокументы");
	Если ПереопределенныйМодуль <> Неопределено Тогда
		Возврат ПереопределенныйМодуль.ЗаполнитьСвязанныеДокументы(ДокВСД);
	КонецЕсли;
	
	// LLM_NOTE: Доработка получения номера ТТН для Расходного ордера на товары.
	// Если основанием является Расходный ордер, то номер ТТН берется из связанной Реализации товаров и услуг.
	// Связь: РасходныйОрдер -> ЗаказКлиента (через ТЧ ТоварыПоРаспоряжениям) -> РеализацияТоваровУслуг (через реквизит ЗаказКлиента).
	
	СтруктураРасходныйОрдер = ВетИС_УтилитыКлиентСервер.ОпределитьТипОбъектаМетаданных("РасходныйОрдер");
	Если СтруктураРасходныйОрдер.ТипДокумента <> Неопределено 
		И ТипЗнч(ДокВСД.ДокументОснование) = СтруктураРасходныйОрдер.ТипДокумента Тогда
		
		ДокВСД.ТТННомер = "";
		ДокВСД.ТтнДата = Дата(1, 1, 1);
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТПР.Распоряжение КАК ЗаказКлиента
		|ПОМЕСТИТЬ втЗаказы
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК ТПР
		|ГДЕ
		|	ТПР.Ссылка = &РасходныйОрдер
		|	И ТПР.Распоряжение ССЫЛКА Документ.ЗаказКлиента
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|			ПО втЗаказы.ЗаказКлиента = РеализацияТоваровУслуг.ЗаказКлиента
		|ГДЕ
		|	РеализацияТоваровУслуг.Проведен";
		
		Запрос.УстановитьПараметр("РасходныйОрдер", ДокВСД.ДокументОснование);
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			ДокВСД.ТТННомер = Выборка.НомерРеализации;
			ДокВСД.ТтнДата = Выборка.ДатаРеализации;
		КонецЕсли;
...
//
// GRACE:CONTRACT function="РассчитатьКоличествоДляВСД"
Функция РассчитатьКоличествоДляВСД(ПараметрыОрганизации, СтрокаТЧДок, Продукция_Элемент = "") Экспорт
	
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("РассчитатьКоличествоДляВСД");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	Рез = 0;
	Коэфф = 0;
	// LLM_NOTE: Блок Попытка используется для обработки ошибок при расчете коэффициента пересчета количества.
	// Это критично, так как неверный параметр может привести к ошибке деления на 0 или доступу к несуществующему свойству.
	Попытка
		//Пересчет ШТ в КГ
		...
		Иначе //остальные считаем с учетом параметра [ПарамКоэффициентПересчетаКоличества]		
			...
			// LLM_NOTE: Внутренняя попытка для безопасного преобразования строкового параметра в число.
			Попытка
		...
		КонецЕсли;
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Указан неверный параметр [ПарамКоэффициентПересчетаКоличества]");
		ВетИС_ЯдроСервер.СообщитьИнфо(ОписаниеОшибки());
		Рез = 0;
	КонецПопытки;
	
	Если Коэфф = 0 Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Коэфф пересчета "+КПересчета+" для "+СтрокаТЧДок.Номенклатура + " равен 0 - ставлю 1" );
		Коэфф = 1;
	КонецЕсли;
	
	// LLM_NOTE: Попытка для безопасного получения количества из строки документа.
	// Параметр "НазваниеРеквизитаКоличество" может отсутствовать в документе.
	Попытка
		Рез = СтрокаТЧДок[ ПараметрыОрганизации["НазваниеРеквизитаКоличество"] ]*Коэфф;
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Указан неверный параметр [НазваниеРеквизитаКоличество] ");
		ВетИС_ЯдроСервер.СообщитьИнфо(ОписаниеОшибки());
		Рез = 0;
	КонецПопытки;
...
//
// GRACE:CONTRACT function="ВыгрузитьТч"
Функция ВыгрузитьТч(Док, ПараметрыОрганизации = "") Экспорт
	
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ВыгрузитьТч");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	Для Каждого СтрокаОснования Из ТаблицаОснование.ТаблицаТоваров Цикл
		...
		ВСДПродукцияЭлемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре(СтрокаОснования.Номенклатура, Контрагент);
		...
		Если НЕ ЗначениеЗаполнено(ВСДПродукцияЭлемент) Тогда
			...
			Иначе
				ВетИС_ЯдроСервер.СообщитьИнфо("[" + СтрокаОснования.Номенклатура + "] -> не указан ВСД_Продукция_Элемент");
			КонецЕсли;
		КонецЕсли;
...
//
// GRACE:CONTRACT function="ПолучитьАртикулНоменклатуры"
Функция ПолучитьАртикулНоменклатуры(ПараметрыОрганизации, НоваяСтрока, СтрокаОснования, ДокСсылка) Экспорт
	
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ПолучитьАртикулНоменклатуры");
	Если ПереопределенныйМодуль <> Неопределено Тогда
...
//
// GRACE:CONTRACT function="ЗаполнитьТабЧастьВСД"
Функция ЗаполнитьТабЧастьВСД(ПараметрыОрганизации, ДанныеЗаполнения = Неопределено, докСсылка, тзАктуальныхПартий = Неопределено, ВыбТермУсловиеПеревозки = Неопределено, Контрагент = Неопределено) Экспорт
	...
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьТабЧастьВСД");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	КонецЕсли;
	ПартииКСписанию = ВетИС_ЯдроСервер.тзПартииСписанияПоТзПродукция_Элемент(ПараметрыОрганизации, тзЭлементыКсписанию, тзАктуальныхПартий, ПолеСортировкиПокупателя, ЗнакСортировкиПокупателя, Контрагент);
	
	Если (ПартииКСписанию.Количество() = 0) и НЕ(ПараметрыОрганизации["ПарамЗаполнятьТранзакциюПриОтсутствииПартий"]) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет актуальных партий для создания ВСД по "+ ?(ДанныеЗаполнения = Неопределено,"Переданным данным",ДанныеЗаполнения.Ссылка));
		Возврат Ложь;
	КонецЕсли;
...
//  Булево - Истина, если заполнение выполнено успешно
//
// GRACE:CONTRACT function="ЗаполнитьТабЧастьВСДпоТЗ"
Функция ЗаполнитьТабЧастьВСДпоТЗ(ПараметрыОрганизации, тзЭлементыОснования, докСсылка, тзАктуальныхПартий = Неопределено, ВыбТермУсловиеПеревозки = Неопределено) Экспорт
	...
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьТабЧастьВСДпоТЗ");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	Иначе 
		...
		ЗнакСортировкиПокупателя = Неопределено;
	КонецЕсли;
	ПартииКСписанию = ВетИС_ЯдроСервер.тзПартииСписанияПоТзПродукция_Элемент(ПараметрыОрганизации, тзЭлементыКсписанию, тзАктуальныхПартий, ПолеСортировкиПокупателя, ЗнакСортировкиПокупателя);
	
	Если (ПартииКСписанию.Количество() = 0) и НЕ(ПараметрыОрганизации["ПарамЗаполнятьТранзакциюПриОтсутствииПартий"]) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет актуальных партий для создания ВСД ", тзЭлементыОснования);
		Возврат Ложь;
	КонецЕсли;
...
//
// GRACE:CONTRACT function="ЗаполнитьКорректировочныйВСД"
...
//
// GRACE:CONTRACT function="РассчитатьКоличествоПотребительскихУпаковок"
Функция РассчитатьКоличествоПотребительскихУпаковок(СтрокаУпак, СтрокаОснование) Экспорт
	
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("РассчитатьКоличествоПотребительскихУпаковок");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Номенклатура, "ВесИспользовать") Тогда 
	...
	Если Коэфф = 0 Тогда
		Коэфф = 1;
		ВетИС_ЯдроСервер.СообщитьИнфо("Коэффициент пересчета равен нулю для " + Номенклатура + ", используется значение 1");
	КонецЕсли;
...
//
// GRACE:CONTRACT function="ЗаполнитьРеквизитыСтрокиВСД"
Процедура ЗаполнитьРеквизитыСтрокиВСД(ПараметрыОрганизации, НоваяСтрока, СтрокаОснования, ДокСсылка) Экспорт
	
	// вызов переопределения	
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьРеквизитыСтрокиВСД");
	Если ПереопределенныйМодуль <> Неопределено Тогда
	...
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "Продукция") Тогда
		НоваяСтрока.Продукция = НоваяСтрока.Продукция_Элемент.Продукция;
	КонецЕсли;
	
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "ВидПродукции") Тогда
		НоваяСтрока.ВидПродукции = НоваяСтрока.Продукция_Элемент.ВидПродукции;
	КонецЕсли;
	
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "НаименованиеПродукции") Тогда
		НоваяСтрока.НаименованиеПродукции = НоваяСтрока.Продукция_Элемент.Наименование;
	КонецЕсли;
	
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "ВидПроисхожденияНеПищевойПродукции") Тогда
	...
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "РезультатыИсследований") Тогда
		Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "Партия") Тогда
	...
	// GTIN и Артикул
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "GTIN") Тогда
		НоваяСтрока.GTIN = НоваяСтрока.Продукция_Элемент.GTIN;
	КонецЕсли;
	
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "Артикул") Тогда
		НоваяСтрока.Артикул = ПолучитьАртикулНоменклатуры(ПараметрыОрганизации, НоваяСтрока, СтрокаОснования, ДокСсылка);
	КонецЕсли;
	
	// Ключ строки
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "КлючСтроки") Тогда
		НоваяСтрока.КлючСтроки = Новый УникальныйИдентификатор();
	КонецЕсли;
	
	// Идентификатор строки
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "ИдентификаторСтроки") Тогда
		Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаОснования, "ИдентификаторСтроки") Тогда
			НоваяСтрока.ИдентификаторСтроки = СтрокаОснования.ИдентификаторСтроки;
		КонецЕсли;
	КонецЕсли;
	
	// Особые отметки
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "ОсобыеОтметки") Тогда
		НоваяСтрока.ОсобыеОтметки = НоваяСтрока.Продукция_Элемент.ОсобыеОтметки;
	КонецЕсли;
	
	// Цель (с проверкой наличия реквизита)
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "Цель") Тогда
		Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, "Партия") Тогда
			НоваяСтрока.Цель = НоваяСтрока.Партия.vetDocument.Цель;
		КонецЕсли;
	...
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(докСсылка, "УровниУпаковки") Тогда 
		...
		КонецЕсли;
		// Упаковка на уровнях
		// LLM_NOTE: Попытка для безопасной обработки упаковок. Реквизит Упаковки может отсутствовать в документе.
		Попытка
			Если ЗначениеЗаполнено(СтрокаОснования.Упаковки) Тогда
				...
				//	ВетИС_ЯдроСервер.СообщитьИнфо("Нулевое кол-во упаковки у "+СтрокаОснования.Продукция_Элемент+" в "+ДокСсылка);	
				//Иначе
...
//
// GRACE:CONTRACT function="СоздатьТаблицуВыгрузкиТЧ"
Функция СоздатьТаблицуВыгрузкиТЧ() Экспорт
	...
	ТаблицаРезультат.Колонки.Добавить("Количество1с");
...
//
// GRACE:CONTRACT function="ПолучитьТабличнуюЧастьДокумента"
Функция ПолучитьТабличнуюЧастьДокумента(Док, ПараметрыОрганизации) Экспорт
	...
	СтруктураДокументаКорректировки = ВетИС_УтилитыКлиентСервер.ОпределитьТипОбъектаМетаданных("Корректировка");
	СтруктураДокументаРасходныйОрдер = ВетИС_УтилитыКлиентСервер.ОпределитьТипОбъектаМетаданных("РасходныйОрдер");
	...
	ИначеЕсли СтруктураДокументаРасходныйОрдер.ТипДокумента <> Неопределено
				И ТипЗнч(Док) = СтруктураДокументаРасходныйОрдер.ТипДокумента Тогда
				
		Результат.ТаблицаТоваров = Док[СтруктураДокументаРасходныйОрдер.ИмяТЧ].Выгрузить();
...
//
// GRACE:CONTRACT function="ПолучитьКонтрагентаДокумента"
Функция ПолучитьКонтрагентаДокумента(Док) Экспорт
	
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Док, "Контрагент") Тогда
...
//
// GRACE:CONTRACT function="ОбработатьСтрокуКорректировки"
Функция ОбработатьСтрокуКорректировки(СтрокаДокумента, СтруктураКорректировки, ПараметрыОрганизации) Экспорт
...
//
// GRACE:CONTRACT function="РассчитатьКоэффициентУпаковки"
Функция РассчитатьКоэффициентУпаковки(Количество, КоличествоУпаковок, Номенклатура = Неопределено) Экспорт
	
	Если КоличествоУпаковок = 0 Тогда
		Если ЗначениеЗаполнено(Номенклатура) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("Количество упаковок равно нулю для " + Номенклатура);
		КонецЕсли;
		Возврат 1;
	КонецЕсли;
...
//  ПараметрыОрганизации - Структура - параметры организации
//
// GRACE:CONTRACT function="ЗаполнитьСтрокуВыгрузкиТЧ"
Процедура ЗаполнитьСтрокуВыгрузкиТЧ(СтрокаРезультата, СтрокаОснования, ВСДПродукцияЭлемент, Контрагент, ПараметрыОрганизации) Экспорт
	
	// вызов переопределения
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьСтрокуВыгрузкиТЧ");
	Если ПереопределенныйМодуль <> Неопределено Тогда
		ПереопределенныйМодуль.ЗаполнитьСтрокуВыгрузкиТЧ(СтрокаРезультата, СтрокаОснования, ВСДПродукцияЭлемент, Контрагент, ПараметрыОрганизации);
		Возврат;
	КонецЕсли;
	
	Количество1с = СтрокаОснования[ПараметрыОрганизации["НазваниеРеквизитаКоличество"]];
	...
	СтрокаРезультата.Количество1с = Количество1с;
		
	// Серия номенклатуры (используем проверку наличия реквизита)
	Если ПараметрыОрганизации["ПарамИспользоватьСерииПродукции"] Тогда
		Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаОснования, "Серия") Тогда
...
//  ПараметрыОрганизации - Структура - параметры организации
//
// GRACE:CONTRACT function="ЗаполнитьУпаковкиИКоэффициент"
Процедура ЗаполнитьУпаковкиИКоэффициент(СтрокаРезультата, СтрокаОснования, ПараметрыОрганизации) Экспорт
	...
	Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаОснования, ИмяРеквизитаКоличествоМест) Тогда
...
// Возвращает способ подбора в форму подбора по документу основанию в ВСД2 Транзакцию
// 
//  Возвращаемое значение:
//  Число:
//		1 - В кг.
//      2 - В упаковках
//
// GRACE:CONTRACT function="ПолучитьСпособПодбораПоУмолчанию"
Функция ПолучитьСпособПодбораПоУмолчанию() Экспорт
	
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ПолучитьСпособПодбораПоУмолчанию");
	Если ПереопределенныйМодуль <> Неопределено Тогда
		Возврат ПереопределенныйМодуль.ПолучитьСпособПодбораПоУмолчанию();
	КонецЕсли;
...
// GRACE:MODULE_END name="ВетИС_ЗаполнениеДокументовСервер"
