// GRACE:MODULE_START name="ВетИС_УправлениеФормойСервер"
// @grace-module: ВетИС_УправлениеФормойСервер
// @grace-purpose: UI-логика форм ВСД: распределение партий по остаткам, условное оформление, надписи
// @grace-depends: ВетИС_ЯдроСервер, ВетИС_ДокументооборотСервер, ВетИС_APIСервер
// @grace-version: 2.0
// @grace-contract-ref: docs/Contracts.xml -> module="ВетИС_УправлениеФормойСервер"
...
//
// GRACE:CONTRACT function="УменьшитьАктуальныеПартииНаРаспределенные"
		...
		Для Каждого СтрокаВСД Из ТаблицаТоваровВСД Цикл
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Если НайденнаяСтрока.Количество < 0 Тогда
					ВетИС_ЯдроСервер.СообщитьИнфо("В заполненных ранее ВСД на отправку обнаружено ПРЕВЫШЕНИЕ количества имеющейся партии по " +
...
//
// GRACE:CONTRACT function="ПодготовитьТребуемыеПартии"
Функция ПодготовитьТребуемыеПартии(ТаблицаОтгрузок, ПараметрыОрганизации, РеквизитГрузополучатель) Экспорт
	...
	ВремПартии.Колонки.Добавить("Количество1с");
	...
	Для Каждого СтрОтгрузки Из ТаблицаОтгрузок Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрОтгрузки.Грузополучатель) Тогда
			...
			ВетИС_ЯдроСервер.СообщитьИнфо("В документе " + СтрОтгрузки.Док + " не заполнен реквизит " + ПолеГрузополучатель + " -> Пропускаю");
			Продолжить;
		КонецЕсли;
		
		// Контроль уже отправленного/удаленного ВСД
		Если ЗначениеЗаполнено(СтрОтгрузки.ВСД) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("Для " + СтрОтгрузки.Док + " уже создан " + СтрОтгрузки.ВСД + " ->Пропускаю");
			Продолжить;
		КонецЕсли;
		
		// Выгружаем товары из документа
		ТаблицаТоваров = ВетИС_ЗаполнениеДокументовСервер.ВыгрузитьТЧ(СтрОтгрузки.Док, ПараметрыОрганизации);
		Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл
			...
			СтрокаПартий.Количество1с = СтрокаТоваров.Количество1с;
		...
		Если СтрОтгрузки.Площадка.ПерсональныеПараметрыСписанияПартий Тогда 
			...
			Если ПараметрыОрганизации["ВыводитьПодробнуюИнформацию"] Тогда 
				ВетИС_ЯдроСервер.СообщитьИнфо("! Сортировка партий в Площадке [" + СтрОтгрузки.Площадка + "] = " + 
		...
		ИначеЕсли СтрОтгрузки.ХозСубъект.ПерсональныеПараметрыСписанияПартий Тогда 
			...
			Если ПараметрыОрганизации["ВыводитьПодробнуюИнформацию"] Тогда 
				ВетИС_ЯдроСервер.СообщитьИнфо("! Сортировка партий в ХозСубъекте [" + СтрОтгрузки.ХозСубъект + "] = " + 
					ПолеСортировкиПокупателя + " сортировка по LIFO = " + ЗнакСортировкиПокупателя);
			КонецЕсли;
		КонецЕсли;
	...
	ВремПартии.Свернуть("ВСД_Продукция_Элемент, СерияНоменклатуры, Номенклатура, Документ, ТермУсловияПеревозки, Контрагент, ВидПродукции, ИдентификаторСтроки",
		"КоличествоСписания, Количество1с");
...
//
// GRACE:CONTRACT function="РаспределитьПартииПоОстаткам"
Функция РаспределитьПартииПоОстаткам(ТребуемыеПартии, АктуальныеПартии, ПараметрыОрганизации) Экспорт
	...
	РаспределенныеПартии.Колонки.Добавить("Количество1с");
	...
	Для Каждого СтрокаТребуемых Из ТребуемыеПартии Цикл
		...
		СтрокиПартий = ВетИС_ЯдроСервер.ПодобратьПартииПоПродукцияЭлемент(
		...
		Коэфициент1с = СтрокаТребуемых.КоличествоСписания / СтрокаТребуемых.Количество1с;
		...
		Если СтрокиПартий.Количество() = 0 Тогда
			...
			ВетИС_ЯдроСервер.СообщитьИнфо("Нет партий для [" + СтрокаТребуемых.ВСД_Продукция_Элемент + "]");
			Продолжить;
		КонецЕсли;
		...
		Если СтрокиПартий.Итог("Количество") < СтрокаТребуемых.КоличествоСписания Тогда
			...
			НоваяСтрока.Количество1с = НоваяСтрока.КоличествоСписания / Коэфициент1с;
		КонецЕсли;
...
//
// GRACE:CONTRACT function="ОбновитьНадписи"
...
//
// GRACE:CONTRACT function="УстановитьУсловноеОформление"
...
Процедура ЗаполнитьСтрокуРаспределенияБезПартии(НоваяСтрока, ИсходнаяСтрока)
	...
	НоваяСтрока.Количество1с = ИсходнаяСтрока.Количество1с;
	...
	// LLM_NOTE: Обработка исключений при преобразовании даты из серии номенклатуры.
	// Используется безопасный подход: при ошибке преобразования поле сортировки остается пустым,
	// что позволяет продолжить обработку без прерывания процесса распределения партий.
	Попытка
		НоваяСтрока.ПолеСортировки = ВетИС_APIСервер.ДатаXML(ИсходнаяСтрока.СерияНоменклатуры);
	Исключение
...
Процедура ЗаполнитьСтрокуРаспределенияСПартией(НоваяСтрока, ТребуемаяСтрока, СтрокаПартии)
	...
	//НоваяСтрока.Количество1с = ТребуемаяСтрока.Количество1с;
...
Процедура ЗаполнитьСтрокуРаспределенияСНехваткой(НоваяСтрока, ТребуемаяСтрока, РаспределеноКоличество)
	...
	// что позволяет продолжить обработку без прерывания процесса распределения партий.
	Попытка
		НоваяСтрока.ПолеСортировки = ВетИС_APIСервер.ДатаXML(ТребуемаяСтрока.СерияНоменклатуры);
	Исключение
		// Игнорируем ошибки преобразования даты - оставляем поле пустым
		НоваяСтрока.ПолеСортировки = "";
	КонецПопытки;
...
// GRACE:MODULE_END name="ВетИС_УправлениеФормойСервер"
