...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.ПередачаПоЗаказам
		...
		МногооборотнаяТараКлиент.ПредложитьПодобратьМногооборотнуюТару(ЭтотОбъект, "Товары", ИменаКолонок,
			ОписаниеОповещения);
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	ПараметрыПроверкиВидаЦены = Новый Структура("ЦенаВключаетНДС, ИспользоватьПриПродаже, Статус", 
		Ложь, Истина, Перечисления.СтатусыДействияВидовЦен.Действует);
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресТоваровВХранилище)
	...
	ШтрихкодыУпаковок = Результат.ШтрихкодыУпаковок;
	...
	Если ПодобранныеТовары.Количество() > 0 И ШтрихкодыУпаковок.Количество() > 0 Тогда
		ВсеШтрихкоды = Новый Соответствие;
		Для Каждого СтрокаТаблицы Из Объект.ШтрихкодыУпаковок Цикл
			ВсеШтрихкоды.Вставить(СтрокаТаблицы.ШтрихкодУпаковки, Истина);
		КонецЦикла;
		Для Каждого Элемент Из ШтрихкодыУпаковок Цикл
			Если ВсеШтрихкоды[Элемент] = Неопределено Тогда
				Объект.ШтрихкодыУпаковок.Добавить().ШтрихкодУпаковки = Элемент;
				ВсеШтрихкоды.Вставить(Элемент, Истина);
			КонецЕсли;
		КонецЦикла;
