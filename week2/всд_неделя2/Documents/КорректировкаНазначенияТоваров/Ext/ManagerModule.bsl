...
Функция ТаблицаСнятияРезерва(Назначение, Товары, Регистратор) Экспорт
	...
	Текст =
		...
		|	КОНЕЦ КАК ВНаличии,
		|	ВЫБОР
		|		КОГДА Таблица.Потребность - Таблица.КЗаказу - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0) > 0
		|			ТОГДА Таблица.Потребность - Таблица.КЗаказу - ЕСТЬNULL(ТаблицаСкорректировано.Количество, 0)
		|		ИНАЧЕ 0
