...
Функция ВСД2_Производство_ЗаполнитьДатыИзготовления( стрПродукция, ВыбДата )  Экспорт
	...
	ПереопределенныйМодуль = кб99_ВСД_Утилиты.ФункцияПереопределена("ВСД2_Производство_ЗаполнитьДатыИзготовления");
...
Процедура ЗаполнитьПоРеализации( ДанныеЗаполнения )
		...
		Попытка
			Производитель_Площадка = кб99_ВСД.НайтиПлощадкуПоСкладу(ДокументОснование[ПереопределяемыеПоляЗапроса.ПолеСклад], Производитель_ХозСубъект);
		Исключение			
		КонецПопытки;
		...
		Если ПараметрыОрганизации["ПарамИспользоватьСерииПродукции"] Тогда 
			...
			Для Каждого ТекСтрокаТовары Из ДанныеЗаполнения.СерииПродукции Цикл
			    ...
			    Попытка
					НоваяСтрока.Количество = кб99_ВСД_ЗаполнениеДокументов.РассчитатьКоличествоДляВСД(ПараметрыОрганизации, ТекСтрокаТовары, НоваяСтрока.Продукция_Элемент);
				Исключение
				...
				кб99_ВСД_Производство.ЗаполнитьУровниУпаковкиИМаркировку(НоваяСтрока, ЭтотОбъект, ПараметрыОрганизации);
			КонецЦикла;
...
Процедура ЗаполнитьПоТЗ( тзПартии, ВыбДата=Неопределено, ПараметрыОрганизации)	
	...
	Для Каждого стрПартий Из тзПартии Цикл
        ...
        кб99_ВСД_Производство.ЗаполнитьУровниУпаковкиИМаркировку(стрПродукция, ЭтотОбъект, ПараметрыОрганизации);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ПереопределенныйМодуль = кб99_ВСД_Утилиты.ФункцияПереопределена("ВСД2_Производство_ОбработкаЗаполнения");
	Если ПереопределенныйМодуль <> Неопределено Тогда
		ПереопределенныйМодуль.ВСД2_Производство_ОбработкаЗаполнения( ДанныеЗаполнения, СтандартнаяОбработка );
	КонецЕсли;
	
	Если кб99_ВСД_Утилиты.ЭтоДокументОтгрузка(ДанныеЗаполнения) Или
		 кб99_ВСД_Утилиты.ЭтоДокументПеремещение(ДанныеЗаполнения) Тогда
			...
			Если Не ЗавершитьОперацию Тогда // контроль что отправка раньше чем запланировано завершение производства
				...
				Если Не Результат.Пустой() Тогда
					...
					Пока Выборка.Следующий() Цикл
						кб99_ВСД.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе: %1 запланировано завершение производственной транзакции с номером %2 раньше даты отправки текущего документа", Выборка.ДокПроизводство, operationId),
	...
	Если ЗавершениеПоРасписанию Тогда
		...
		Если ЗначениеЗаполнено(operationId) Тогда
			...
			Если Не Результат.Пустой() Тогда
				Выборка = Результат.Выбрать();
				Пока Выборка.Следующий() Цикл
					кб99_ВСД.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Уже запланировано завершение производственной транзакция с номером %1 документ: %2", operationId, Выборка.ДокПроизводство),
												  Выборка.ДокПроизводство,,Выборка.ДокПроизводство,Отказ);	
				КонецЦикла;
			КонецЕсли;
		Иначе
