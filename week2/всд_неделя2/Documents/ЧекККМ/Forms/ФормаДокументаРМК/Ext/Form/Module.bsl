...
Функция ДобавитьВКорзинуНаСервере(ПараметрыТовара, НовыеСтроки)
	...
	РозничныеПродажиЛокализация.ОбработатьРазрешенияНаПродажуТоваров(ЭтотОбъект);
...
Процедура ПриИзмененииНоменклатурыХарактеристикиКоличестваЛокализация()
	...
	Если ИнтеграцияИСМпКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(Перечисления.ВидыПродукцииИС.ВетеринарныеПрепараты) Тогда
		РозничныеПродажиЛокализация.ОбработатьРазрешенияНаПродажуТоваров(ЭтотОбъект);
	КонецЕсли;
...
Процедура ОпределятьПринадлежностьТовараИСтавкуНДСПередОплатойЛокализация()
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ЗаполнитьСобственникаТовара", 
		ПакетнаяОбработкаТабличнойЧастиКлиентСерверЛокализация.ПараметрыЗаполненияСобственникаТовара(Объект));
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС",
		ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
