...
Процедура ОформитьОтгрузкуВРозницу(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ШапкаДокумента.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)";
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|		И ТабЧасть.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ИсправлениеОшибки)";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ЧекККМКоррекции.ОбработатьДанныеДляЗаполненияРеквизитаВидКоррекции";
	Обработчик.Версия = "11.5.21.8";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("6dfa502d-dc13-3424-d42e-7bd8278754c3");
	Обработчик.Многопоточный = Истина;	
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЧекККМКоррекции.ЗарегистрироватьДанныеКОбработкеДляЗаполненияРеквизитаВидКоррекции";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	ШаблонКомментарияОбработчика = НСтр("ru = 'Заполнение в документе ""%1"" нового реквизита шапки ""%2"" значениями из перечисления ""%3"" на основании числовых значений из старого реквизита.'");
	
	Обработчик.Комментарий = СтрШаблон(ШаблонКомментарияОбработчика,
							Метаданные.Документы.ЧекККМКоррекции.Синоним,
							Метаданные.Документы.ЧекККМКоррекции.Реквизиты.ВидКоррекции.Синоним,
							Метаданные.Перечисления.ВидыЧековКоррекции.Синоним);
...
// 	Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//
Процедура ЗарегистрироватьДанныеКОбработкеДляЗаполненияРеквизитаВидКоррекции(Параметры) Экспорт
	...
	|	Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
	|ГДЕ
	|	ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.ПустаяСсылка)
	|";
...
//  Параметры - Структура - параметры.
//
Процедура ОбработатьДанныеДляЗаполненияРеквизитаВидКоррекции(Параметры) Экспорт
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			ДокументОбъект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки;
			Если ДокументОбъект.УдалитьВидКоррекции = 1 Тогда
				ДокументОбъект.ВидКоррекции = Перечисления.ВидыЧековКоррекции.НеприменениеККТ;
			Иначе
				ШаблонТекстаСообщения = НСтр("ru = 'В документе %1 неизвестный вид коррекции %2 (поддерживаемые значения: 0, 1) заменен на %3.'");
				ТекстСообщения = СтрШаблон(ШаблонТекстаСообщения,
						ДокументОбъект.Ссылка,
						ДокументОбъект.УдалитьВидКоррекции,
						Метаданные.Перечисления.ВидыЧековКоррекции.ЗначенияПеречисления.ИсправлениеОшибки.Синоним);
