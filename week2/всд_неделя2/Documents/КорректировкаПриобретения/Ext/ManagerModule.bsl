...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ВзаиморасчетыСервер.ПроведениеОплатыПоставщику(Запрос, ТекстыЗапроса, Регистры, ТекстОплата,"3");
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЗначенияРеквизитов.ТаблицаЗначений.Количество() = 0
			...
			|	ДАТАВРЕМЯ(1,1,1)                                 КАК ДатаСогласования,
			|	ЛОЖЬ                                             КАК СверхЗаказа,
			|	СУММА(ВТТовары.Сумма)                            КАК СуммаПлатежа,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = '- зполнение реквизита ""Вариант приемки товаров"".';"));
	СписокОписаний.Добавить(НСтр("ru = '- зполнение реквизита ""Хозяйственная операция"".';"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|ГДЕ
	|	КорректировкаПриобретения.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	КорректировкаПриобретения.ХозяйственнаяОперация <> ПриобретениеТоваровУслуг.ХозяйственнаяОперация";
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(ДокументОбъект.ДокументОснование)
					И ТипЗнч(ДокументОбъект.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
					
					ХозяйственнаяОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						ДокументОбъект.ДокументОснование, "ХозяйственнаяОперация");
					
					Если ХозяйственнаяОперация <> ДокументОбъект.ХозяйственнаяОперация Тогда
			...
			Если ДокументОбъект <> Неопределено И ДокументОбъект.ВариантПриемкиТоваров.Пустая() Тогда
				
				ТипОснования = ТипЗнч(ДокументОбъект.ДокументОснование);
		
				Если (ТипОснования = Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
					Или ТипОснования = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки")) Тогда
					ВариантПриемкиТоваров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
