...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	НаименованияДокументовПечати = ОбщегоНазначенияУТ.НаименованияЭлектронныхДокументовИлиОсновныхПечатныхФорм(
		ТаблицаВзаиморасчетов.ВыгрузитьКолонку("РасчетныйДокумент"));
...
Функция ЗапросДатыОпераций()
	...
	|
	|ВЫБРАТЬ
	|	МИНИМУМ(Расчеты.Период) КАК ДатаОперации,
	|	Расчеты.Регистратор КАК РасчетныйДокумент
	|ИЗ
	|	РегистрНакопления.РасчетыПоФинансовымИнструментам КАК Расчеты
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ втРасчетныеДокументы КАК РасчетныеДокументы
	|	ПО Расчеты.Регистратор = РасчетныеДокументы.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	Расчеты.Регистратор
	|";
