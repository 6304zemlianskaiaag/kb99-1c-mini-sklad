...
Процедура кнПолучитьВСДНаСервере()
	...
	Если ПараметрыОрганизации.ПарамВсдВходящиеИскатьДокументПрихода Тогда
		кб99_ВСД_Гашение.СверитьКоличествоСДокументомПоступления(ПараметрыОрганизации, Объект);
	КонецЕсли;
...
Процедура УстановитьДокументПоступления(Команда)
	...
	ИмяДокументаПоступление = кб99_ВСД_Утилиты.ОпределитьИмяДокументаПоступление();
	Если ИмяДокументаПоступление = Неопределено Тогда
		кб99_ВСД.СообщитьИнфо("Не удалось определить имя документа поступления товаров для вашей конфигурации. Обратитесь в техническую поддержку.");
		Возврат;
	КонецЕсли;
...
Процедура ПриЗакрытииФормыВыбора(РезультатВыбора, ДопПараметры) Экспорт
	...
	Для Каждого Строка Из Объект.ВСДВходящие Цикл
		...
		кб99_ВСД_Гашение.ГашениеПроверитьСтрокуТаблицы(ДанныеСтроки, ПараметрыОрганизации);
