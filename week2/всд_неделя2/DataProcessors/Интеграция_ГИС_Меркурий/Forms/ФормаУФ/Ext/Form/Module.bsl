...
Процедура ПолучитьПартииАктуальные(Команда)
	...
	Результат = ПолучитьПартии2_НаСервере(Дата("20000101"), ТекущаяДата(), Истина);
...
Процедура ПослеВводаДатыЗагрузкиПартииСИзменениями(ВыбДата, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбДата) Тогда
		...
		Результат = ПолучитьПартии2_СИзменениями_НаСервере(ВыбДата, ТекущаяДата());
...
&НаСервере
Процедура ОтменитьДокументыНаСервере()
    ...
    Для Каждого СтрОтгрузки Из Объект.Отгрузки Цикл
        ...
        // Если ВСД уже создан, проверяем можно ли его отменить
        Если ЗначениеЗаполнено(СтрОтгрузки.ВСД) Тогда
            
            СтатусВСД = СтрОтгрузки.СтатусВСД;
            
            // Если уже WITHDRAWN, пропускаем
            Если СтатусВСД = Перечисления.кб99_СтатусВСД.WITHDRAWN Тогда
                кб99_ВСД.СообщитьИнфо("ВСД " + СтрОтгрузки.ВСД + " уже отменен");
                Продолжить;
            КонецЕсли;
            
            // Если CONFIRMED, нужно отменить в Меркурий
            Если СтатусВСД = Перечисления.кб99_СтатусВСД.CONFIRMED Тогда
                кб99_ВСД_Запросы.ВСД2_транзакция_Аннулировать(ПараметрыОрганизации, СтрОтгрузки.ВСД);
                кб99_ВСД.СообщитьИнфо("ВСД " + СтрОтгрузки.ВСД + " отправлен на аннулирование в Меркурий");
            Иначе
                // Просто помечаем как WITHDRAWN локально
                ДокВСД = СтрОтгрузки.ВСД.ПолучитьОбъект();
                ДокВСД.СтатусВСД = Перечисления.кб99_СтатусВСД.WITHDRAWN;
                ДокВСД.Записать();
                кб99_ВСД.СообщитьИнфо("ВСД " + СтрОтгрузки.ВСД + " помечен как отмененный");
            КонецЕсли;
            
        Иначе
            // Создаем новый ВСД со статусом WITHDRAWN
            ...
            ПараметрыДокумента.Вставить("Отправитель_Площадка", Объект.Отправитель_Площадка);
            ПараметрыДокумента.Вставить("ДокОснование", СтрОтгрузки.Док);
            ПараметрыДокумента.Вставить("НеПодбиратьПартии", Истина);
            ...
            кб99_ВСД.СообщитьИнфо("Создан отмененный ВСД " + ДокВСД.Ссылка + " для " + СтрОтгрузки.Док, ДокВСД.Ссылка);
        КонецЕсли;
...
Процедура ОтменитьДокументыОтвет(Ответ, Парам) Экспорт
    ...
    Если Ответ = КодВозвратаДиалога.Да Тогда
        Состояние("Выполняется отмена документов", , "Ожидайте...", БиблиотекаКартинок.kb99_wrench);
        ОтменитьДокументыНаСервере();
        ПоказатьОповещениеПользователя("Выполнено");
    КонецЕсли;
