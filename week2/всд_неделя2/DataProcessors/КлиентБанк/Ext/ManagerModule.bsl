		...
		Иначе
			...
			Для Каждого РасчетныйСчет Из Результат.РасчетныеСчета Цикл
				...
				Для Каждого ТекущийПлатеж Из Результат.ДокументыВыписки Цикл
					...
					ЕстьАналитическийКодРаздела = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
						ТекущийПлатеж, "АналитическийКодРаздела");
					ОдинаковыйРаздел = (ЕстьАналитическийКодРаздела
						И РасчетныйСчет.РазделЛицевогоСчета = ТекущийПлатеж.АналитическийКодРаздела)
						Или Не ЕстьАналитическийКодРаздела;
					
					Если ТекущийПлатеж.НаправлениеПлатежа = "1" Тогда
						
						ОдинаковыйСчет = РасчетныйСчет.РасчСчет = ТекущийПлатеж.ПлательщикРасчСчет;
						ЕстьЛицевойСчет = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
							ТекущийПлатеж, "ПлательщикЛицевойСчет");
						ОдинаковыйЛицевойСчет = (ЕстьЛицевойСчет
							И РасчетныйСчет.НомерЛицевогоСчета = ТекущийПлатеж.ПлательщикЛицевойСчет)
							Или Не ЕстьЛицевойСчет;
						
					Иначе
						
						ОдинаковыйСчет = РасчетныйСчет.РасчСчет = ТекущийПлатеж.ПолучательРасчСчет;
						ЕстьЛицевойСчет = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
							ТекущийПлатеж, "ПолучательЛицевойСчет");
						ОдинаковыйЛицевойСчет = (ЕстьЛицевойСчет
							И РасчетныйСчет.НомерЛицевогоСчета = ТекущийПлатеж.ПолучательЛицевойСчет)
							Или Не ЕстьЛицевойСчет;
...
Процедура ПроверитьДубликатыПлатежейВыписки(ДокументыКЗагрузке)
	...
	ДубликатыПлатежей = ДокументыКЗагрузке.Скопировать(, 
		"Загружать, БанковскийСчет, Номер, Дата, ПолучательСчет, ПлательщикСчет, ДатаСписано, ДатаПоступило");
	...
	ДубликатыПлатежей.Свернуть("Загружать, БанковскийСчет, Номер, Дата, ПолучательСчет, ПлательщикСчет, ДатаСписано, ДатаПоступило", "ВсегоПлатежей");
	
	ОшибкиДублей = Новый Массив;
	
	Для Каждого СтрокаТаблицы Из ДубликатыПлатежей Цикл
		
		Если СтрокаТаблицы.ВсегоПлатежей > 1 Тогда
