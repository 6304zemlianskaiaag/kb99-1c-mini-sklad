	...
	Для каждого ТекущаяСтрокаФайла Из ДанныеФайлов Цикл
		...
		Если Не ЗаданиеВыполнено
			...
			Если Результат.ИдентификаторЗадания <> Неопределено Тогда
				...
				РезультатВыполнения = ДлительныеОперации.ЗаданиеВыполнено(Результат.ИдентификаторЗадания, Истина);
				ЗаданиеВыполнено = РезультатВыполнения.Статус = "Выполнено";
		...
		Если Результат.Статус = "Ошибка" Тогда
			...
			ШаблонТекста = НСтр("ru = 'Ошибка загрузки файла ""%1"" по счету %2:
										|%3'");
			ТекстОшибкиПодробно = СтрШаблон(ШаблонТекста,
									ТекущаяСтрокаФайла.ИмяФайла,
									Счет.Ссылка,
			...
			ТекстОшибкиКратко = СтрШаблон(ШаблонТекста,
									ТекущаяСтрокаФайла.ИмяФайла,
									Счет.Ссылка,
