...
Процедура ПодключитьОжиданиеФоновыхЗаданий();
	...
	Для Каждого Счет Из Объект.БанковскиеСчета Цикл
		...
		Если ЗначениеЗаполнено(Счет.ДанныеВыгрузки)
			...
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьЗавершениеВыгрузкиОповещение", ЭтотОбъект, Счет);
...
Процедура ОбработатьЗавершениеВыгрузкиОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		ДополнительныеПараметры.Выгружается = Ложь;
		ДополнительныеПараметры.СохранитьФайл = Истина;
		ДополнительныеПараметры.ДанныеВыгрузки = "";
		ОбработатьЗавершениеВыгрузки();
	КонецЕсли;
