...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаВУ.Видимость = ОбменСГИСЭПДКлиентСервер.ВхождениеРоли(Роль, РольВодитель, АтомарныеРолиМП.Количество());
	...
	Если ВидЭП = ПредопределенноеЗначение("Перечисление.ВидПодписиМПЭПД.НеИспользуется")
		Или ВидЭП = ПредопределенноеЗначение("Перечисление.ВидПодписиМПЭПД.ПростаяПодпись")
...
Функция СценарийПолучениеКодаРегистрации(ОжиданиеРезультата, ПараметрыЦикла) Экспорт
	...
	Если ЭтапЦикла = "ДобавитьПользователя" Тогда
		...
		СервисВзаимодействияМПЭПДКлиент.НовоеФоновоеЗадание("СервисВзаимодействияМПЭПД.СервисДобавитьВодителя",
	...
	Если ЭтапЦикла = "РегистрацияПользователя" Тогда
		...
		Если ОжиданиеРезультата.Статус = "Выполнено" Тогда
			...
			Если ТипЗнч(ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") 
				И ВладелецФормы.ИмяФормы = "Обработка.ВзаимодействиеМПЭПД.Форма.СостояниеПодключенияМП" Тогда
			...
			Иначе
				ОповеститьОбИзмененииУстройства();
				ЗапуститьОбновлениеИнформацииОРегистрацииУстройства();
			КонецЕсли;
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(ОжиданиеРезультата.КраткоеПредставлениеОшибки);
		КонецЕсли;
...
&НаКлиенте
Процедура ЗапуститьОбновлениеИнформацииОРегистрацииУстройства()
	...
	СервисВзаимодействияМПЭПДКлиент.НачатьАссинхронныйСценарий(
			Новый ОписаниеОповещения("СценарийОбновленияИнформацииОРегистрации", ЭтотОбъект, ПараметрыЦикла));
			
КонецПроцедуры

&НаКлиенте
Функция СценарийОбновленияИнформацииОРегистрации(ОжиданиеРезультата, ПараметрыЦикла) Экспорт
	...
	Если ЭтапЦикла = "Начало" Тогда
		ПараметрыЦикла.Вставить("ЭтапЦикла", "ОбновитьДанныеУстройства");
		
		МассивИдентификаторЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(УчетнаяЗаписьЭДО);
		ПараметрыФункции = СервисВзаимодействияМПЭПДКлиент.МассивИзЗначений(МассивИдентификаторЭДО);
		СервисВзаимодействияМПЭПДКлиент.НовоеФоновоеЗадание("СервисВзаимодействияМПЭПД.СервисПолучитьПользователей",
				ПараметрыФункции, ПараметрыЦикла, УникальныйИдентификатор);
	КонецЕсли;
	
	Если ЭтапЦикла = "ОбновитьДанныеУстройства" Тогда
		...
		Если ОжиданиеРезультата <> Неопределено Тогда
			Если ОжиданиеРезультата.Статус = "Выполнено" Тогда
				...
				Если СтруктураРезультата.УспешностьЗагрузки Тогда
					...
					СтруктураДанныхУстройства = ДанныеУстройства(Состояние, Идентификатор);
					Если СтруктураДанныхУстройства.ДанныеПолучены Тогда
						ИзменитьСостояниеЗаписиОбУстройстве(СтруктураДанныхУстройства.НовоеСостояние,
							СтруктураДанныхУстройства.ИдентификаторМП);
						ОповеститьОбИзмененииУстройства();
					Иначе
						ВремяОжидания = ВремяОжиданияОбновленияИнформацииОРегистраци();
						ПодключитьОбработчикОжидания("ОбработчикОжиданияОбновленияИнформацииОРегистрации",
...
&НаКлиенте
Функция ВремяОжиданияОбновленияИнформацииОРегистраци()
...
Функция ДанныеУстройства(Знач ТекущееСостояние, Знач Идентификатор)
	
	СтруктураДанныхУстройства = Новый Структура;
	СтруктураДанныхУстройства.Вставить("ДанныеПолучены", Ложь);
	СтруктураДанныхУстройства.Вставить("НовоеСостояние");
	СтруктураДанныхУстройства.Вставить("ИдентификаторМП");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПодключенныеМПЭПД.Состояние КАК НовоеСостояние,
	...
	|	И ПодключенныеМПЭПД.Состояние <> &ТекущееСостояние";
	Запрос.УстановитьПараметр("Идентификатор", Идентификатор);
	Запрос.УстановитьПараметр("ТекущееСостояние", ТекущееСостояние);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда 
		ЗаполнитьЗначенияСвойств(СтруктураДанныхУстройства, Выборка);
		СтруктураДанныхУстройства.ДанныеПолучены = Истина;
	КонецЕсли;
	
	Возврат СтруктураДанныхУстройства;
	
КонецФункции

&НаКлиенте
Процедура ОбработчикОжиданияОбновленияИнформацииОРегистрации()
