...
Процедура ОбновитьНаСервереFBS()
	...
	Запрос.Текст =
		...
		|	ЗаказыТорговыхПлощадок.НомерОтправления,
		|	ЗаказыТорговыхПлощадок.Статус,
		|	ЕСТЬNULL(ЗаказКлиентаПрисоединенныеФайлы.Ссылка, ЕСТЬNULL(РеализацияТоваровУслугПрисоединенныеФайлы.Ссылка, ЕСТЬNULL(ПередачаТоваровХранителюПрисоединенныеФайлы.Ссылка, НЕОПРЕДЕЛЕНО))),
		|	ЕСТЬNULL(ЗаказКлиентаПрисоединенныеФайлы.Наименование, ЕСТЬNULL(РеализацияТоваровУслугПрисоединенныеФайлы.Наименование, ЕСТЬNULL(ПередачаТоваровХранителюПрисоединенныеФайлы.Наименование, """"))),
		|	ЕСТЬNULL(ВЫРАЗИТЬ(ЗаказКлиентаПрисоединенныеФайлы.ПутьКФайлу КАК СТРОКА(1024)), ЕСТЬNULL(ВЫРАЗИТЬ(РеализацияТоваровУслугПрисоединенныеФайлы.ПутьКФайлу КАК СТРОКА(1024)), ЕСТЬNULL(ВЫРАЗИТЬ(ПередачаТоваровХранителюПрисоединенныеФайлы.ПутьКФайлу КАК СТРОКА(1024)), """"))),
		|	ЕСТЬNULL(ЗаказКлиентаПрисоединенныеФайлы.Расширение, ЕСТЬNULL(РеализацияТоваровУслугПрисоединенныеФайлы.Расширение, ЕСТЬNULL(ПередачаТоваровХранителюПрисоединенныеФайлы.Расширение, """")))
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК РасходныйОрдерНаТоварыТоварыПоРаспоряжениям
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
		|		ПО ((ВЫРАЗИТЬ(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Документ.ЗаказКлиента)) = ЗаказыТорговыхПлощадок.Заказ)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ЗаказКлиентаПрисоединенныеФайлы КАК ЗаказКлиентаПрисоединенныеФайлы
		|		ПО (ЗаказыТорговыхПлощадок.Заказ = ЗаказКлиентаПрисоединенныеФайлы.ВладелецФайла)
		|			И (НЕ ЗаказКлиентаПрисоединенныеФайлы.ПометкаУдаления)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РеализацияТоваровУслугПрисоединенныеФайлы КАК РеализацияТоваровУслугПрисоединенныеФайлы
		|		ПО ((ВЫРАЗИТЬ(ЗаказыТорговыхПлощадок.ДокументОтгрузки КАК Документ.РеализацияТоваровУслуг)) = РеализацияТоваровУслугПрисоединенныеФайлы.ВладелецФайла)
		|			И (НЕ РеализацияТоваровУслугПрисоединенныеФайлы.ПометкаУдаления)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПередачаТоваровХранителюПрисоединенныеФайлы КАК ПередачаТоваровХранителюПрисоединенныеФайлы
		|		ПО ((ВЫРАЗИТЬ(ЗаказыТорговыхПлощадок.ДокументОтгрузки КАК Документ.ПередачаТоваровХранителю)) = ПередачаТоваровХранителюПрисоединенныеФайлы.ВладелецФайла)
		|			И (НЕ ПередачаТоваровХранителюПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		...
		|		ПО ((ВЫРАЗИТЬ(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Документ.РеализацияТоваровУслуг)) = ЗаказыТорговыхПлощадок.ДокументОтгрузки)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РеализацияТоваровУслугПрисоединенныеФайлы КАК РеализацияТоваровУслугПрисоединенныеФайлы
		|		ПО ((ВЫРАЗИТЬ(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Документ.РеализацияТоваровУслуг)) = РеализацияТоваровУслугПрисоединенныеФайлы.ВладелецФайла)
		|			И (НЕ РеализацияТоваровУслугПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		|	(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Ссылка В (&Объекты)
		|			ИЛИ ЗаказыТорговыхПлощадок.Заказ В (&Объекты)
		...
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК РасходныйОрдерНаТоварыТоварыПоРаспоряжениям
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказыТорговыхПлощадок КАК ЗаказыТорговыхПлощадок
		|		ПО ((ВЫРАЗИТЬ(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Документ.ПередачаТоваровХранителю)) = ЗаказыТорговыхПлощадок.ДокументОтгрузки)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПередачаТоваровХранителюПрисоединенныеФайлы КАК ПередачаТоваровХранителюПрисоединенныеФайлы
		|		ПО ((ВЫРАЗИТЬ(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Распоряжение КАК Документ.ПередачаТоваровХранителю)) = ПередачаТоваровХранителюПрисоединенныеФайлы.ВладелецФайла)
		|			И (НЕ ПередачаТоваровХранителюПрисоединенныеФайлы.ПометкаУдаления)
		|ГДЕ
		|	(РасходныйОрдерНаТоварыТоварыПоРаспоряжениям.Ссылка В (&Объекты)
		|			ИЛИ ЗаказыТорговыхПлощадок.Заказ В (&Объекты)
