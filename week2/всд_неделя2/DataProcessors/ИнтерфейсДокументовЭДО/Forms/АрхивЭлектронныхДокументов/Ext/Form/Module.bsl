...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	...
	Если ИнтерфейсДокументовЭДОВызовСервера.ТребуетсяОткрытиеЛегкойКарточкиПриСозданииПроизвольногоДокумента() Тогда
...
Функция ПредставлениеЭлектронныхДокументов(Знач Документообороты)
	...
	Для Каждого Документооборот Из Документообороты Цикл
		...
		ПредставлениеДанныхСообщения =
			ЭлектронныеДокументыЭДО.ПредставлениеДанныхСообщенияПоСсылке(ПараметрыПолученияПредставления);
		МассивПредставлений.Добавить(ПредставлениеДанныхСообщения.ПредставлениеДокумента);
	КонецЦикла;
