...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьКлючДоступаКСервисуЭДО = АвтоматическоеПолучениеЭДО.ИспользоватьКлючДоступаКСервисуЭДО();
...
Процедура ОбработатьРезультатРегистрацииСертификатаЧерезОблачныйЭДО(РезультатРегистрации, Контекст = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторОрганизации) Тогда
		ИдентификаторОрганизацииСуществующий = ИдентификаторОрганизации;
		Элементы.ГруппаОжиданиеОтветаОтОператора.РасширеннаяПодсказка.Заголовок = "";
		СохранитьНастройкуОблачногоЭДО();
		ТестСвязиСервисаЭДО();
	Иначе
		ВыполняетсяРегистрация = Ложь;
		УстановкаСостоянияПолученияИдентификатораЭД(Истина, "Ошибка");
...
&НаСервере
Процедура СохранитьНастройкуОблачногоЭДО()
	РегистрыСведений.НастройкиОблачногоЭДО.Записать(ИдентификаторОрганизации, УчетнаяЗаписьОблачногоЭДО);
КонецПроцедуры
...
Процедура ВключитьАвтоматическоеПолучениеЭДО(ИдентификаторЭДО)
	...
	Если Не ЭтоОблачныйЭДО 
		И (Не ИспользоватьКлючДоступаКСервисуЭДО
	...
	ПараметрыВключения = АвтоматическоеПолучениеЭДОКлиент.НовыеПараметрыВключенияИспользованияКлючаДоступаКСервисуЭДО();
...
Процедура ВключитьАвтоматическоеПолучениеЭДОЗавершение(Результат, Контекст) Экспорт
	...
	Подключен = Результат.СостояниеПоИдентификаторамЭДО[Контекст.ИдентификаторЭДО] <> Неопределено;
