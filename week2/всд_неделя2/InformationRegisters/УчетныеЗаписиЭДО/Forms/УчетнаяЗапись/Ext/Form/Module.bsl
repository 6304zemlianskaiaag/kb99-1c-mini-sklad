	...
	Если Результат <> Неопределено
		...
		Если ЗначениеЗаполнено(СостояниеЗаявки.ИдентификаторОрганизации) Тогда
			...
			Если СостояниеЗаявки.Результат Тогда
				...
				ИзмененСоставСертификатов = Ложь;
				ИзмененыРегистрационныеДанные = Ложь;
				ВключитьИспользованиеКлючаДоступаКСервисуЭДО(СостояниеЗаявки.ИдентификаторОрганизации);
			Иначе
				Закрыть = Ложь;
...
&НаКлиенте
Процедура ВключитьИспользованиеКлючаДоступаКСервисуЭДО(ИдентификаторЭДО)
	
	Если Не ИспользоватьКлючДоступаКСервисуЭДО() Тогда
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ВключитьИспользованиеКлючаДоступаКСервисуЭДОЗавершение", ЭтотОбъект);
	ПараметрыВключения =
		АвтоматическоеПолучениеЭДОКлиент.НовыеПараметрыВключенияИспользованияКлючаДоступаКСервисуЭДО();
	ПараметрыВключения.ИдентификаторыЭДО.Добавить(ИдентификаторЭДО);
	ПараметрыВключения.ФормаВладелец = ЭтотОбъект;
	ПараметрыВключения.ПаролиСертификатов = ПаролиСертификатов;
	АвтоматическоеПолучениеЭДОКлиент.ВключитьИспользованиеКлючаДоступаКСервисуЭДО(
		Оповещение, ПараметрыВключения);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ИспользоватьКлючДоступаКСервисуЭДО()
	Возврат АвтоматическоеПолучениеЭДО.ИспользоватьКлючДоступаКСервисуЭДО();
КонецФункции

// Параметры:
//  Результат - Неопределено
//            - см. АвтоматическоеПолучениеЭДО.ПодключитьИспользованиеКлючаДоступаКСервисуЭДО
//  Контекст - Неопределено
&НаКлиенте
Процедура ПодключитьИспользованиеКлючаДоступаКСервисуЭДОЗавершение(Результат, Контекст = Неопределено) Экспорт
...
//  Контекст - Неопределено
&НаКлиенте
Процедура ВключитьИспользованиеКлючаДоступаКСервисуЭДОЗавершение(Результат, Контекст = Неопределено) Экспорт
...
Процедура ЗарегистрироватьСертификатЧерезОблачныйЭДО(ПараметрыСохранения)
	...
	ОбщийМодульУчетныеЗаписиЭДОИнтеграцияОблакаКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияОблакаКлиент");
...
Процедура СпроситьОбУдаленииНедействительныхСертификатов(ЗакрытьФормуПриОтказе)
	...
	ОповещениеУдалениеНействительныхСертификатов
		= Новый ОписаниеОповещения("ВопросОбУдаленииНействительныхСертификатовЗавершение", ЭтотОбъект, ЗакрытьФормуПриОтказе);
	ПоказатьВопрос(ОповещениеУдалениеНействительныхСертификатов,
...
&НаКлиенте
Процедура ВопросОбУдаленииНействительныхСертификатовЗавершение(Результат, ЗакрытьФормуПриОтказе) Экспорт
