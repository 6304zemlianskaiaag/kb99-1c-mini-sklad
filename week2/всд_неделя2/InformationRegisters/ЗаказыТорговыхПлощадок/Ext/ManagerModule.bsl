...
Функция СформироватьЗаданиеНаПеревозку(ИнформацияОбОтгрузке, НастройкиСклада = Неопределено) Экспорт
	...
	ДокументОбъект.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1. Сформировано для отгрузки %2 по ""%3""'"),
...
Процедура ОбработатьСтатусОтправленияВозвращенОтменен(УчетнаяЗапись, Заказ, Распоряжение,
	...
	Если ТипРаспоряжения <> Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ТипРаспоряжения <> Тип("ДокументСсылка.ПередачаТоваровХранителю")
			И ТипРаспоряжения <> Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
		Возврат;
	КонецЕсли;
	...
	ПродолжитьОтменуПроведения = Истина;
	
	ВыборкаСвязанныхДокументов = Запрос.Выполнить().Выбрать();
	Пока ВыборкаСвязанныхДокументов.Следующий() Цикл
		...
		СвязанныеДокументы.Добавить(ВыборкаСвязанныхДокументов.Ссылка);
	...
	Если Распоряжение <> Неопределено И РаспоряжениеОтменено Тогда
		ИмяТаблицы = "Товары";
		РезультатЗапроса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Распоряжение, ИмяТаблицы); // РезультатЗапроса
		Если РезультатЗапроса <> Неопределено Тогда
			ОтменитьЗаказ(Заказ, Распоряжение, ПричинаОтмены, РезультатЗапроса.Выбрать(), ИмяСобытия, Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(УчетнаяЗапись) Тогда
		...
		Если НастройкиУчетнойЗаписи.СоздаватьЗадачуОповещение = Истина Тогда
			СвязанныеДокументы.Добавить(Заказ);
...
Функция ЗакрытьЗаказ(Заказ, ИмяСобытия = "")
...
Процедура ДобавитьНапоминания(СвязанныеДокументы)
	...
	Для Каждого ДокументСсылка Из СвязанныеДокументы Цикл
...
Функция ВыборкаДанныхПоДоставке(УчетнаяЗапись, УчитыватьДанныеДоставки, Заказы = Неопределено,
	...
	ДополнитьТекстыЗапросаПоДоставкеВыборкойДанныхПоОтправлениям(ТекстыЗапроса);
...
Процедура ДополнитьТекстыЗапросаПоДоставкеВыборкойДанныхПоДоставкеТоваровРаспоряжений(ТекстыЗапроса)
	...
	ТекстыЗапроса.Добавить(
		...
		|	ДанныеПоДоставкам.Количество КАК Количество,
		|	ДанныеПоДоставкам.ИсключенИзДоставки КАК ИсключенИзДоставки
		...
		|		КОГДА НЕ ТоварыВДоставке.ИсключенИзДоставки
		|			ТОГДА ТоварыВДоставке.Количество
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК КоличествоВДоставке,
		|	ВЫБОР
		|		КОГДА РаспоряженияВДоставке.ИсключенИзДоставки
		|			ТОГДА ТоварыПоРаспоряжениям.КоличествоКОтгрузке
		|		КОГДА ТоварыВДоставке.ИсключенИзДоставки
		|			ТОГДА ТоварыВДоставке.Количество
		|		ИНАЧЕ 0
...
//   ТекстыЗапроса - Массив из Строка - список частей текста запроса.
//
Процедура ДополнитьТекстыЗапросаПоДоставкеВыборкойДанныхПоОтправлениям(ТекстыЗапроса)
