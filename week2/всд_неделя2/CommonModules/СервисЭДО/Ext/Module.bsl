...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
		...
		Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
			...
			АвтоматическоеПолучениеЭДО.ДополнитьЗаголовкиЗапроса(
				ПараметрыВыполненияОперации.Заголовки, КлючСинхронизации, 
				ПараметрыВыполненияОперации.АдресРесурса, ПредставлениеОперации);
...
Функция ПриоритетРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой = "") Экспорт
	...
	ИначеЕсли КодТранзакции = "Invoice" Тогда
		...
		Если ТипДокументаСтрокой = "PrimaryAccountingDocumentVendor"
			...
			Или ТипДокументаСтрокой = "PrimaryAccountingDocumentCustomerWorkResult"
			Или ТипДокументаСтрокой = "CorPrimaryAccountingDocumentVendor"
...
Процедура ОбработатьОшибкуСервиса(ПараметрыОперации, КонтекстДиагностики, ОшибкаОператора,
	...
	Если СервисНастроекЭДО.ЭтоКодОшибкиКонтроляНедействительныхСертификатов(ИдентификаторОператора,
		ОшибкаОператора.Код) Тогда
