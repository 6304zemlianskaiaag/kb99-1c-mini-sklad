...
Функция СоздатьВУчетеЭлектронныйДокумент(Знач ДанныеДокумента) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		"ru = 'Не удалось определить метаданные объекта типа ""%1""'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция РезультатПоискаДокументаПоПубличнойСсылке(Знач СсылкаДляСкачивания, Отказ = Ложь, ТекстОшибки = "") Экспорт
	...
	ИнтеграцияShareПереопределяемый.ПриОпределенииНастроекЗагрузкиДокумента(Результат.НастройкиЗагрузки,
		Результат.ОписаниеЭлектронногоДокумента, Отказ, ТекстОшибки);
...
Функция РезультатГлобальногоПоискаДокументаПоПубличнойСсылке(СсылкаДляСкачивания) Экспорт
	...
	Если Отказ Тогда
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр(
				...
				|%2'", ОбщегоНазначения.КодОсновногоЯзыка()), СсылкаДляСкачивания, ТекстОшибки));
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ПустаяСтрока(ОписаниеЭлектронногоДокумента.ТипДокумента) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр(
			"ru = 'Загрузка документа по ссылке %1 с типом %2 не поддерживается'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СсылкаДляСкачивания, ОписаниеЭлектронногоДокумента.ТипЭлектронногоДокумента));
		Возврат РезультатГлобальногоПоиска;
	КонецЕсли;
...
// Возвращаемое значение:
//  Строка - гиперссылка на инструкцию по работе с сервисом 1С:Share для прикладной базы.
