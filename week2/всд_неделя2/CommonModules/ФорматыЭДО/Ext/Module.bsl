...
Функция ФорматыЭлектронныхДокументов(Отбор = Неопределено) Экспорт
	...
	ОбходВыборкиРезультатаЗапросаФорматыЭлектронныхДокументов(Отбор, Выборка, ТекущаяДата, Результат);
	
	ТребуетсяОбновитьФорматы = Ложь;
	Если Не ЗначениеЗаполнено(Отбор) Тогда 
		ИдентификаторУПД_5_03 = ФорматыЭДО_ФНС.ПоддерживаемыеФорматы().УПД_5_03.ИнформацияПродавца;
		ОтборДляПоискаУПД_5_03 = Новый Структура("ИдентификаторФормата", ИдентификаторУПД_5_03);
		НайденныеСтрокиУПД_5_03 = Результат.НайтиСтроки(ОтборДляПоискаУПД_5_03);
		
		ИдентификаторСчетНаОплату_5_01 = "ON_CHETOP_1_998_01_05_01_01";
		ОтборДляПоискаСчетНаОплату_5_01 = Новый Структура("ИдентификаторФормата", ИдентификаторСчетНаОплату_5_01);
		НайденныеСтрокиСчетНаОплату_5_01 = Результат.НайтиСтроки(ОтборДляПоискаСчетНаОплату_5_01);
		
		ТребуетсяОбновитьФорматы = Не ЗначениеЗаполнено(НайденныеСтрокиУПД_5_03)
			Или НайденныеСтрокиУПД_5_03[0].Приоритет <= 1
			Или Не ЗначениеЗаполнено(НайденныеСтрокиСчетНаОплату_5_01);
	КонецЕсли;
		
	Если ТребуетсяОбновитьФорматы Тогда
		НастройкиОтправкиЭДОСлужебный.ОбновитьФорматыИСвязьВидовФорматовИзСервисаНастроек();
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПокупателя
		...
		Если (Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПокупателя
			...
			Если ЭтоФизЛицо 
				И Не ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи) Тогда
				Должность = НСтр("ru = 'ИП'");
				ОснованиеПолномочий = НСтр("ru = 'Свидетельство ОГРН'");
			Иначе
				Должность = ДолжностьПоСертификату;
...
Функция ЗаполнитьДанныеУчастниковЭДО(ОписаниеФайла, ДанныеУчастниковЭДО) Экспорт
	...
	ПространствоИмен = ПространствоИменПоФорматуДляЗаполненияУчастниковЭДО(Формат);
...
// Возвращаемое значение:
//  Булево
Функция ЗаполнениеДанныхУчастниковЭДОДоступно(Формат)
...
// Возвращает пространство имен xml по формату
// 
// Параметры:
...
// Возвращаемое значение:
//  Строка
Функция ПространствоИменПоФорматуДляЗаполненияУчастниковЭДО(Формат)
	
	ПоддерживаемыеФорматы = ПоддерживаемыеФорматы();
	
	ПространстваИмен = ФорматыЭДО_ФНС.ПространстваИменФорматов();
	
	Если Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.УПД.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.УПД.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.УПД2019.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.УПД2019.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.УКД.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.УКД.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.УКД2020.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.УКД2020.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.ПередачаТоваров.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПродавца Тогда
		Возврат ПространстваИмен.ПередачаРабот.ИнформацияПродавца;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.ПередачаРабот.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.АктОРасхождениях.ИнформацияПокупателя;
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.ПриемкаСдачаРабот.ИнформацияПокупателя Тогда
		Возврат ПространстваИмен.ПриемкаСдачаРабот.ИнформацияПокупателя;
	Иначе
		Возврат "";
	КонецЕсли;
...
// Возвращает объект XDTO документа из двоичных данных файла
// 
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные - Двоичные данные
//  ПространствоИмен - Строка - Пространство имен xml, см. ПространствоИменПоФорматуДляЗаполненияУчастниковЭДО
// 
// Возвращаемое значение:
//  ОбъектXDTO
Функция ОбъектXDTOИзДанныхФайла(ДвоичныеДанные, ПространствоИмен)
	...
	ТипДанныхXDTO = ПолучитьТипЗначенияCML("Файл", ПространствоИмен);
...
Процедура ОбходВыборкиРезультатаЗапросаФорматыЭлектронныхДокументов(Отбор, Выборка, ТекущаяДата, Результат)
