...
Функция СформироватьДанныеДокументовДляВыгрузки(ОписанияОбъектовУчета) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ВерсияФормата = ИзменитьВерсиюФорматаУПДВТекущихНастройкахОтправкиДокументов(Выборка.Организация,
			Выборка.Контрагент, Выборка.ДоговорКонтрагента, Выборка.ВидДокумента, Выборка.Формат);
...
Процедура ПодменитьФайлНаФайлСВизуализацией(ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если КэшВизуализации.ТребуетсяОбновить Тогда
		ВизуализацияЭДОСлужебный.ЗарегистрироватьДокументЭДОВОчередьНаВизуализацию(ЭлектронныйДокумент);
...
//
// Возвращаемое значение:
//  Строка - новая версия формата. 
Функция ИзменитьВерсиюФорматаУПДВТекущихНастройкахОтправкиДокументов(Организация, Контрагент,
	ДоговорКонтрагента, ВидДокумента, ТекущаяВерсияФормата)
	...
	Если  ТекущаяВерсияФормата <> ПоддерживаемыеФорматы.УПД2019.ИнформацияПродавца
		И ТекущаяВерсияФормата <> ПоддерживаемыеФорматы.УПД_5_02.ИнформацияПродавца 
		И ТекущаяВерсияФормата <> ПоддерживаемыеФорматы.ПередачаТоваров.ИнформацияПродавца
		И ТекущаяВерсияФормата <> ПоддерживаемыеФорматы.ПередачаРабот.ИнформацияПродавца Тогда
		Возврат ТекущаяВерсияФормата;
	КонецЕсли;
	
	НоваяВерсияФормата = ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца;
	...
	Попытка
		...
		Для Каждого Запись Из НаборЗаписей Цикл
			...
			ИначеЕсли Запись.ВерсияФорматаУстановленаВручную И (ЭтоФормат551 Или ЭтоФормат552) Тогда	
				НоваяВерсияФормата = ТекущаяВерсияФормата;
			КонецЕсли;
