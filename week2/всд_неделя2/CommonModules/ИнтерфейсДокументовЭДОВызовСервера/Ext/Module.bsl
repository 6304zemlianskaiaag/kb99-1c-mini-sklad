...
Процедура ОбработкаПолученияФормыЭлектронногоДокумента(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация,
	...
	Если ВидФормы <> "ФормаОбъекта" Или Не ИспользоватьИнтеграциюОблачногоЭДО И Не ИспользуютсяЛегкиеКарточки Тогда
		Возврат;
	КонецЕсли;

	Если ИспользоватьИнтеграциюОблачногоЭДО Тогда
		ЭлектронныйДокумент = Неопределено;
		ОткрываетсяСуществующийДокумент = Параметры.Свойство("Ключ", ЭлектронныйДокумент) И ЗначениеЗаполнено(
			ЭлектронныйДокумент) И ОбщегоНазначения.СсылкаСуществует(ЭлектронныйДокумент);
		ОткрываетсяСуществующийЛокальныйДокумент = ОткрываетсяСуществующийДокумент И Не ЭлектронныеДокументыЭДО.ЭтоДокументОблачногоЭДО(
		ЭлектронныйДокумент);
		СоздаетсяНовыйЛокальныйДокумент = Не ОткрываетсяСуществующийДокумент
			И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ЭтоОблачныйЭДО", Истина);
		Если ОткрываетсяСуществующийЛокальныйДокумент Или СоздаетсяНовыйЛокальныйДокумент Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ТребуетсяОткрытиеЛегкойКарточкиПриСозданииПроизвольногоДокумента() Экспорт
