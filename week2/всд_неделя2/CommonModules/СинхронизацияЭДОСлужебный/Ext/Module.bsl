...
Процедура ПолучитьНовыеЭлектронныеДокументыЧерезОператораЭДО(
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Пока Не ПолученыВсеДокументы Цикл
			...
			УпешноеПолучениеТранспортныхКонтейнеров = ПолучитьДокументы(ДокументыВнеПакетов, КонтекстДиагностики, Результат, РезультатыОтправкиПолучения,
				ПараметрыПолучения);
			Если Не УпешноеПолучениеТранспортныхКонтейнеров 
			...
			Для Каждого ИдентификаторПакета Из ИдентификаторыПакетов Цикл
				...
				НачатьТранзакцию();
				Попытка
					УпешноеПолучениеТранспортныхКонтейнеров = ПолучитьДокументы(ТранспортныеКонтейнерыПакета, КонтекстДиагностики,
						Результат, РезультатыОтправкиПолучения, ПараметрыПолучения, Истина);
					ЗафиксироватьТранзакцию();
				Исключение
					ОтменитьТранзакцию();
					УпешноеПолучениеТранспортныхКонтейнеров = Ложь;
			...
			Если УпешноеПолучениеТранспортныхКонтейнеров И ПараметрыПолучения.СдвигатьДату И ЗначениеЗаполнено(ДатаПоследнегоЗапроса)
