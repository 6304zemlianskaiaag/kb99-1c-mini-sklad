...
//    * ШаблоныНазначений - ТаблицаЗначений, Неопределено - см. РегистрыСведений.ШаблоныНазначенийПлатежейСБП.ШаблоныНазначенийДокументаОперации.
//
Функция ЗаказНаОплату(
	...
	Если Не ЗначениеЗаполнено(ДанныеЧастичнойОплаты) Тогда
		...
		ЗаказНаОплату.ШаблоныНазначений =
			РегистрыСведений.ШаблоныНазначенийПлатежейСБП.ШаблоныНазначенийДокументаОперации(
				НастройкаПодключения,
				ДокументОплаты,
				Перечисления.ВариантыНастройкиСБП.c2b);
...
Функция ОпределитьИдентификаторЗаказаНаВозврат(
	...
	Если ПараметрыСтатуса.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНеОплачен()
		Или ПараметрыСтатуса.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНетИнформации()
		Или ПараметрыСтатуса.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаОтклонена()
		Или ПараметрыСтатуса.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаПревышение() Тогда
		Возврат РегистрыСведений.ИдентификаторыОперацийСБПc2b.НовыйИдентификаторОперации(
			ДокументВозврата,
			ПараметрыНастройкиПодключения,
			ИдентификаторОплаты);
...
Процедура ПроверитьЗаказНаОплату(
	...
	ПереводыСБПc2bСлужебный.ПроверитьНазначениеПлатежа(
...
Процедура ОбработатьСтатусВозврата(РезультатОперации, СтатусВозврата)
	...
	Если СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаПревышение() Тогда
		РезультатОперации.СтатусОперации = СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииОшибка();
		РезультатОперации.КодОшибки = СистемаБыстрыхПлатежейСлужебный.КодОшибкиНеверныйФорматЗапроса();
	...
	ИначеЕсли СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНеОплачен() Тогда
		РезультатОперации.СтатусОперации = СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииОшибка();
		РезультатОперации.КодОшибки = СистемаБыстрыхПлатежейСлужебный.КодОшибкиНеверныйФорматЗапроса();
	...
	ИначеЕсли СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНетИнформации() Тогда
		РезультатОперации.СтатусОперации = СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииОшибка();
		РезультатОперации.КодОшибки = СистемаБыстрыхПлатежейСлужебный.КодОшибкиНеверныйФорматЗапроса();
	...
	ИначеЕсли СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаТребуетсяПодтверждение() Тогда
...
Процедура ОбработатьСтатусОтчета(РезультатОперации, СтатусОтчета, ТипОшибки)
	...
	ИначеЕсли СтатусОтчета = ИдентификаторСтатусаОжидание() Тогда
		РезультатОперации.СтатусОперации  = СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииВыполняется();
	ИначеЕсли СтатусОтчета = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаОшибка() Тогда
		РезультатОперации.СтатусОперации = СистемаБыстрыхПлатежейКлиентСервер.СтатусОперацииОшибка();
...
Процедура ОбработатьСтатусыСтатусыОплат(ДанныеОпераций, ОбработанныеОперации)
	...
	Для Каждого ОтложеннаяОперация Из ДанныеОпераций Цикл
		...
		Если РезультатПроверки.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаВПроцессе()
			И (ОтложеннаяОперация.ПериодИспользования + 600) < ТекущаяДатаСеанса() Тогда
			РезультатПроверки.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаОтклонена();
		КонецЕсли;
		...
		Если РезультатПроверки.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаВыполнена()
			Или РезультатПроверки.СтатусОперации = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаОтклонена() Тогда
	...
	Для Каждого ОтложеннаяОперация Из ДанныеОпераций Цикл
		...
		Если ЗначениеЗаполнено(РезультатПроверки.КодОшибки) Тогда
			СистемаБыстрыхПлатежейСлужебный.УвеличитьКоличествоПопытокЗапросаСтатуса(
				ОтложеннаяОперация.ДокументОперации,
				Метаданные.РегистрыСведений.ИдентификаторыОперацийСБПc2b.Имя);
			Если ОтложеннаяОперация.КоличествоПопыток >= СистемаБыстрыхПлатежейСлужебный.МаксимальноеКоличествоПопытокЗапросаСтатуса(Ложь) Тогда
					ПриЗагрузкеСтатусаОперации(
						ОтложеннаяОперация.ДокументОперации,
						ОтложеннаяОперация.НастройкаПодключения,
						ПереводыСБПc2bСлужебный.НовыйОписаниеПараметровОперации(),
		...
		Если РезультатПроверки.СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаВыполнена()
			...
			Или РезультатПроверки.СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНетИнформации()
			Или РезультатПроверки.СтатусВозврата = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаПревышение() Тогда
...
Процедура ПриЗагрузкеСтатусаОперации(
	...
	ИначеЕсли СтатусОперацииСервис = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаПревышение()
		Или СтатусОперацииСервис = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНеОплачен()
		Или СтатусОперацииСервис = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаНетИнформации()
		Или СтатусОперацииСервис = СистемаБыстрыхПлатежейСлужебный.ИдентификаторСтатусаОшибка() Тогда
