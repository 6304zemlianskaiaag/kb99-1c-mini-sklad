...
//  ИскатьПоСрокуИсполненияЗаказа - Булево - Истина, если нужно искать дату поставки, исходя из срока исполнения заказа в способе обеспечения, Ложь, если нужно искать дату по гарантированному сроку исполнения заказа.
//  ТолькоПоСпособуОбеспечения - Булево - Если истина и нет способа обеспечения, то будет возвращена пустая дата.
// 
// Возвращаемое значение:
...
//  * Соединения - Строка - Выражение, для подстановки в текст запроса в качестве необходимых левых соединений с ведущей таблицей.
//  * ПолеДатаПоставкиПоТочкеЗаказа - Строка - Пустая строка, если в параметре ИскатьПоСрокуИсполненияЗаказа передано значение Ложь.
//                                             Выражение, для подстановки в текст запроса, позволяющее получить плановую дату поставки по точке заказа
//                                             Используется если передано Истина в качестве параметра ИскатьПоСрокуИсполненияЗаказа,
//                                             значение исходящего поля Поле вычислено по расписанию. 
Функция ПодстановкиПлановойДатыПоставки(СпособОбеспечения, НачалоЭтогоДня, ИскатьПоСрокуИсполненияЗаказа = Ложь, ТолькоПоСпособуОбеспечения = Ложь) Экспорт
	
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ЕСТЬNULL(
		|		ВЫБОР
// Граничный случай - для клиентских менеджеров нужно давать пустую дату поставки, если нет способа обеспечения.
		|			КОГДА &ТолькоПоСпособуОбеспечения И СпособОбеспеченияПереопределяемый.Ссылка ЕСТЬ NULL
		|					
		|					ТОГДА NULL
		|					
...
// Расписание задано вручную по календарю: График поставок.
		...
		|											ИЛИ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0) = 0
		|										ТОГДА
		|											ВЫБОР
...
// Расчет даты по точке заказа.
		|			ИНАЧЕ &СрокИсполненияЗаказаЛибоГарантированныйСрокДата
		|			
		|		КОНЕЦ,
		|
// Значение, если дату не удается рассчитать.
		...
		|	ЕСТЬNULL(&СрокИсполненияЗаказаДата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПоставкиПоТочкеЗаказа
		|ИЗ
		|	&ВедущаяТаблица КАК ВедущаяТаблица
		|		
		...
		|		ПО ИСТИНА
		|		
		// Получение даты в графике по гарантированному сроку обеспечения.
		...
		|				+ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0)
		...
		|			ДОБАВИТЬКДАТЕ(ВедущаяТаблица.НачалоЭтогоДня, ДЕНЬ, ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0)))
		...
		|			ДОБАВИТЬКДАТЕ(ВедущаяТаблица.НачалоЭтогоДня, ДЕНЬ, ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0))) + 1
		|		И ГрафикПоставокСрокИсполненияЗаказаПервыйДеньСледующегоГодаБезКалендаря.КоличествоДнейВГрафикеСНачалаГода = 1";
	
	ТекстИстина = "ИСТИНА"; // @query-part-1
	ТекстЛожь = "ЛОЖЬ"; // @query-part-1
	ТекстТолькоПоСпособуОбеспечения = ?(ТолькоПоСпособуОбеспечения, ТекстИстина, ТекстЛожь);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТолькоПоСпособуОбеспечения", ТекстТолькоПоСпособуОбеспечения);
	
	СрокИсполненияЗаказаДата = // для упрощения записи исходного текста запроса.
		"ВЫБОР
		|	КОГДА КалендарьПредприятия.Значение = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|				ИЛИ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0) = 0
		|			ТОГДА ДОБАВИТЬКДАТЕ(ВедущаяТаблица.НачалоЭтогоДня, ДЕНЬ,
		|		ЕСТЬNULL(СпособОбеспеченияПереопределяемый.СрокИсполненияЗаказа, 0))
	...
	Соединения = СтрЗаменить(Соединения, "ВедущаяТаблица.СпособОбеспечения", СпособОбеспечения);
	Соединения = СтрЗаменить(Соединения, "ВедущаяТаблица.НачалоЭтогоДня", НачалоЭтогоДня);
	
	Если ИскатьПоСрокуИсполненияЗаказа Тогда
		
		Поле = СтрЗаменить(Поле, "&СрокИсполненияЗаказаЛибоГарантированныйСрокДата", СрокИсполненияЗаказаДата);
		
	Иначе
		
		Тексты = Новый Массив();
		Тексты.Добавить(Соединения);
		Тексты.Добавить(
			"
			|		ЛЕВОЕ СОЕДИНЕНИЕ ГодыГрафикаПредприятия КАК ГарантированныйСрокОбеспеченияГод
			|		ПО ЭтотДеньГод.НачальныйОстаток
			|				+ ЭтотДеньДата.КоличествоДнейВГрафикеСНачалаГода
			|				+ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.ГарантированныйСрокОбеспечения, 0)
			|			МЕЖДУ ГарантированныйСрокОбеспеченияГод.НачальныйОстаток + 1
			|				И ГарантированныйСрокОбеспеченияГод.КонечныйОстаток
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КалендарныеГрафики КАК ГарантированныйСрокОбеспеченияДата
			|		ПО ГарантированныйСрокОбеспеченияДата.Календарь = КалендарьПредприятия.Значение
			|		И ГарантированныйСрокОбеспеченияДата.Год = ГарантированныйСрокОбеспеченияГод.Год
			|		И ГарантированныйСрокОбеспеченияДата.ДеньВключенВГрафик
			|		И ГарантированныйСрокОбеспеченияДата.КоличествоДнейВГрафикеСНачалаГода
			|			= ЭтотДеньГод.НачальныйОстаток
			|				+ ЭтотДеньДата.КоличествоДнейВГрафикеСНачалаГода
			|				+ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.ГарантированныйСрокОбеспечения, 0)
			|				- ГарантированныйСрокОбеспеченияГод.НачальныйОстаток
			|");
		Соединения = СтрСоединить(Тексты);
		
		Текст =
			"ВЫБОР
			|	КОГДА
			|		&ПлановаяДатаПоставки = ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
			|	КОГДА
			|		&СрокИсполненияЗаказаЛибоГарантированныйСрокДата = ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
			|	КОГДА
			|		&ПлановаяДатаПоставки > &СрокИсполненияЗаказаЛибоГарантированныйСрокДата
			|			ТОГДА
			|				&ПлановаяДатаПоставки
			|	ИНАЧЕ
			|		&СрокИсполненияЗаказаЛибоГарантированныйСрокДата
			|КОНЕЦ";
			
		Поле = СтрЗаменить(Текст, "&ПлановаяДатаПоставки", Поле);
		
		Замена =
			"ВЫБОР
			|	КОГДА КалендарьПредприятия.Значение = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
			|				ИЛИ ЕСТЬNULL(СпособОбеспеченияПереопределяемый.ГарантированныйСрокОбеспечения, 0) = 0
			|			ТОГДА ДОБАВИТЬКДАТЕ(ВедущаяТаблица.НачалоЭтогоДня, ДЕНЬ,
			|		ЕСТЬNULL(СпособОбеспеченияПереопределяемый.ГарантированныйСрокОбеспечения, 0))
			|	ИНАЧЕ ГарантированныйСрокОбеспеченияДата.ДатаГрафика
			|КОНЕЦ";
		Поле = СтрЗаменить(Поле, "&СрокИсполненияЗаказаЛибоГарантированныйСрокДата", Замена);
		
	КонецЕсли;
	
	Поле = СтрЗаменить(Поле, "ВедущаяТаблица.НачалоЭтогоДня", НачалоЭтогоДня);
	
	Результат = Новый Структура("Поле,Соединения,ПолеДатаПоставкиПоТочкеЗаказа", Поле, Соединения, "");
	
	Если ИскатьПоСрокуИсполненияЗаказа Тогда
		
		ПолеДатаПоставкиПоТочкеЗаказа = СхемаЗапроса.ПакетЗапросов[0].Колонки[1].Поля[0];
		
		ПолеДатаПоставкиПоТочкеЗаказа = СтрЗаменить(ПолеДатаПоставкиПоТочкеЗаказа, "ВедущаяТаблица.НачалоЭтогоДня",
			НачалоЭтогоДня);
		
		Результат.ПолеДатаПоставкиПоТочкеЗаказа = ПолеДатаПоставкиПоТочкеЗаказа;
...
// Возвращаемое значение:
//  Строка - Текст запроса временной таблицы содержащей информацию о планово йдате поставки по способу обеспечения. 
...
//  * Соединения - Строка - Выражение, для подстановки в текст запроса в качестве необходимых левых соединений с ведущей таблицей.
Функция ПодстановкиДатыПоставкиБлижайшейКДате(СпособОбеспечения, Дата) Экспорт
...
// Значение, если дату не удается рассчитать.
	...
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	Поле = СхемаЗапроса.ПакетЗапросов[0].Колонки[0].Поля[0];
	Поле = СтрЗаменить(Поле, "ВедущаяТаблица.Дата", Дата);
	
	Соединения = Сред(ТекстЗапроса, СтрНайти(ТекстЗапроса, "ЛЕВОЕ СОЕДИНЕНИЕ"));
	Соединения = СтрЗаменить(Соединения, "ВедущаяТаблица.СпособОбеспечения", СпособОбеспечения);
	Соединения = СтрЗаменить(Соединения, "ВедущаяТаблица.Дата", Дата);
...
//                         если Истина, то дата будет отсчитано 0 дней по календарю предприятия, что важно если дата отсчета попадает на выходной день.
// 
// Возвращаемое значение:
//  Структура - Cодежржит поля:
...
Функция ДобавитьКДатеПоКалендарюПредприятия(ДатаОтсчета, ЧислоДней, Искать0Дней = Ложь) Экспорт
	...
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	Поле = СхемаЗапроса.ПакетЗапросов[0].Колонки[0].Поля[0];
	Поле = СтрЗаменить(Поле, "ВедущаяТаблица.ЧислоДней", ЧислоДней);
	Поле = СтрЗаменить(Поле, "ВедущаяТаблица.ДатаОтсчета", ДатаОтсчета);
	
	Соединения = Сред(ТекстЗапроса, СтрНайти(ТекстЗапроса, "ЛЕВОЕ СОЕДИНЕНИЕ"));
	Соединения = СтрЗаменить(Соединения, "ВедущаяТаблица.ЧислоДней", ЧислоДней);
...
Функция ВременнаяТаблицаГраницыПериодаОбеспечения(Товары, НачалоЭтогоДня) Экспорт
	...
	ТекстЗапроса = ВременнаяТаблицаГраницыПериодаОбеспеченияПоТаблицеПлановаяДатаПоставки();
	Тексты.Добавить(ТекстЗапроса);
...
// Возвращаемое значение:
//  Строка - Текст запроса.
Функция ВременнаяТаблицаГраницыПериодаОбеспеченияПоТаблицеПлановаяДатаПоставки() Экспорт
...
// Значение, если дату не удается рассчитать.
		...
		|ГДЕ
		|	&Поле <> ДАТАВРЕМЯ(1, 1, 1)";
	
	СхемаЗапроса = Новый СхемаЗапроса();
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	Поле = СхемаЗапроса.ПакетЗапросов[0].Колонки[4].Поля[0];
...
Процедура ВременнаяТаблицаБлижайшийЦиклПоСпособамОбеспечения(МенеджерВременныхТаблиц, ИмяТаблицыСпособы = "Справочник.СпособыОбеспеченияПотребностей", ИмяТаблицы = "БлижайшийЦиклПоСпособамОбеспечения") Экспорт
	...
	Подстановки = ПодстановкиПлановойДатыПоставки("Таблица.СпособОбеспечения", "&НачалоЭтогоДня", Истина);
	...
	Текст = ВременнаяТаблицаГраницыПериодаОбеспеченияПоТаблицеПлановаяДатаПоставки();
	Тексты.Добавить(Текст);
...
Функция ТекстИПараметрыЗапросаОбеспеченияЗаказовНаОтгрузку(ИмяВременнойТаблицы)
	...
	ТекстЗапроса =
		...
		|										ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.Обеспечить)) ТОГДА
		|									
		|									ЕСТЬNULL(РасчетДатыПоставкиПереопределяемый.ПлановаяДатаПоставки, &НачалоТекущегоДня)
		|									
		|							ИНАЧЕ
		|								ДАТАВРЕМЯ(1,1,1)
	...
	Подстановки = ПодстановкиПлановойДатыПоставки(Подстановки.Поле, "&НачалоТекущегоДня", Ложь, Истина);
