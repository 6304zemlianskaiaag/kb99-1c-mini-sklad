...
Функция ПеренестиНоменклатуруПартнера(УчетнаяЗапись, ПредыдущийПартнер, НовыйПартнер, Скопировать = Истина) Экспорт
	...
	Если Скопировать Тогда
		Запрос.Текст = 
			...
			|	ПредыдущаяНоменклатураКонтрагентов.Идентификатор КАК Идентификатор,
			|	НоваяНоменклатураКонтрагентов.Ссылка КАК НоваяСсылка,
			|	ЕСТЬNULL(ИмпортируемыеТоварыТорговыхПлощадок.УчетнаяЗапись, ЗНАЧЕНИЕ(Справочник.УчетныеЗаписиМаркетплейсов.ПустаяСсылка)) КАК ИмпортУчетнаяЗапись,
			|	ЕСТЬNULL(ИмпортируемыеТоварыТорговыхПлощадок.НоменклатураПартнера, ЗНАЧЕНИЕ(Справочник.НоменклатураКонтрагентов.ПустаяСсылка)) КАК ИмпортНоменклатураПартнера,
			|	ЕСТЬNULL(СтатусыПубликацииОбъектов.УчетнаяЗапись, ЗНАЧЕНИЕ(Справочник.УчетныеЗаписиМаркетплейсов.ПустаяСсылка)) КАК СтатусыУчетнаяЗапись,
			|	ЕСТЬNULL(СтатусыПубликацииОбъектов.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК СтатусыНоменклатура,
			|	ЕСТЬNULL(СтатусыПубликацииОбъектов.Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)) КАК СтатусыХарактеристика,
			|	ЕСТЬNULL(СтатусыПубликацииОбъектов.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК СтатусыУпаковка
			|ИЗ
			...
			|		ПО (НоваяНоменклатураКонтрагентов.Владелец = &НовыйПартнер)
			...
			|		ПО (ИмпортируемыеТоварыТорговыхПлощадок.НоменклатураПартнера = ПредыдущаяНоменклатураКонтрагентов.Ссылка)
			|			И (ИмпортируемыеТоварыТорговыхПлощадок.УчетнаяЗапись = &УчетнаяЗапись)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПубликацииТоваровЯндексМаркет КАК СтатусыПубликацииОбъектов
			|		ПО (СтатусыПубликацииОбъектов.НоменклатураПартнера = ПредыдущаяНоменклатураКонтрагентов.Ссылка)
			|			И (СтатусыПубликацииОбъектов.УчетнаяЗапись = &УчетнаяЗапись)
			|ГДЕ
			|	ПредыдущаяНоменклатураКонтрагентов.Владелец = &ПредыдущийПартнер";
	...
	Запрос.УстановитьПараметр("ПредыдущийПартнер", ПредыдущийПартнер);
	Запрос.УстановитьПараметр("НовыйПартнер",      НовыйПартнер);
	...
	Попытка 
		...
		Если Скопировать Тогда
			...
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("УчетнаяЗапись", "ИмпортУчетнаяЗапись");
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("НоменклатураПартнера", "ИмпортНоменклатураПартнера");
			
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений." + ИмяРегистраСведений);
			ЭлементБлокировкиДанных.ИсточникДанных = РезультатЗапроса;
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных(ИмяИзмеренияУчетнаяЗапись, "СтатусыУчетнаяЗапись");
			ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Номенклатура", "СтатусыНоменклатура");
			Если ИспользоватьХарактеристикиНоменклатуры Тогда
				ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Характеристика", "СтатусыХарактеристика");
			КонецЕсли;
			Если ИспользоватьУпаковкиНоменклатуры Тогда
				ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Упаковка", "СтатусыУпаковка");
			КонецЕсли;
		...
		Пока ВыборкаДанных.Следующий() Цикл
			НоменклатураКонтрагентовСсылка = Дубли.Получить(ВыборкаДанных.Идентификатор);
			
			Если Скопировать
					И ЗначениеЗаполнено(ВыборкаДанных.НоваяСсылка) Тогда
				НоменклатураКонтрагентовСсылка = ВыборкаДанных.НоваяСсылка;
			КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(НоменклатураКонтрагентовСсылка) Тогда
				...
				ЗаполнитьЗначенияСвойств(НоменклатураКонтрагентовОбъект, ВыборкаДанных.ПредыдущаяСсылка,, "Код, Владелец, ВладелецНоменклатуры");
				
				НоменклатураКонтрагентовОбъект.Владелец             = НовыйПартнер;
				НоменклатураКонтрагентовОбъект.ВладелецНоменклатуры = НовыйПартнер;
				
				Попытка
					НоменклатураКонтрагентовОбъект.Записать();
					НоменклатураКонтрагентовСсылка = НоменклатураКонтрагентовОбъект.Ссылка;
					
					Дубли.Вставить(ВыборкаДанных.Идентификатор, НоменклатураКонтрагентовСсылка);
					
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При %1 позиции товарного каталога %2 возникли ошибки.'",
							ОбщегоНазначения.КодОсновногоЯзыка()),
						?(Скопировать, "копировании", "переносе"),
						ВыборкаДанных.ПредыдущаяСсылка);
			...
			Если ЗначениеЗаполнено(НоменклатураКонтрагентовСсылка) 
					И Скопировать Тогда
				Если ЗначениеЗаполнено(ВыборкаДанных.ИмпортУчетнаяЗапись) Тогда
					Запись = РегистрыСведений.ИмпортируемыеТоварыТорговыхПлощадок.СоздатьМенеджерЗаписи();
					Запись.УчетнаяЗапись        = ВыборкаДанных.ИмпортУчетнаяЗапись;
					Запись.НоменклатураПартнера = ВыборкаДанных.ИмпортНоменклатураПартнера;
					Запись.Прочитать();
				...
				Если ЗначениеЗаполнено(ВыборкаДанных.СтатусыУчетнаяЗапись) Тогда
					НаборЗаписей = РегистрыСведений[ИмяРегистраСведений].СоздатьНаборЗаписей();
					НаборЗаписей.Отбор[ИмяИзмеренияУчетнаяЗапись].Установить(ВыборкаДанных.СтатусыУчетнаяЗапись);
					НаборЗаписей.Отбор.Номенклатура.Установить(ВыборкаДанных.СтатусыНоменклатура);
					Если ИспользоватьХарактеристикиНоменклатуры Тогда
						НаборЗаписей.Отбор.Характеристика.Установить(ВыборкаДанных.СтатусыХарактеристика);
					КонецЕсли;
					Если ИспользоватьУпаковкиНоменклатуры Тогда
						НаборЗаписей.Отбор.Упаковка.Установить(ВыборкаДанных.СтатусыУпаковка);
					КонецЕсли;
					...
					Для Каждого Запись Из НаборЗаписей Цикл
						Если Запись.НоменклатураПартнера = ВыборкаДанных.ПредыдущаяСсылка Тогда
							Запись.НоменклатураПартнера = НоменклатураКонтрагентовСсылка;
							Записать = Истина;
						КонецЕсли;
					КонецЦикла;
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При %1 позиций товарного каталога возникли ошибки %2.'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			?(Скопировать, "копировании", "переносе"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
