			...
			Или СтруктураПараметровДействия.НалогообложениеНДС
				= Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
					Для Каждого СтрокаКоллекции Из ОбрабатываемаяКоллекция Цикл
						СтрокаКоллекции.ДатаПолученияСтавки =
							?(ИспользоватьДатуОтгрузки
								И ЗначениеЗаполнено(СтрокаКоллекции.ДатаОтгрузки), СтрокаКоллекции.ДатаОтгрузки, Дата);
					КонецЦикла;
		КонецЕсли;
...
Функция ТекстыЗапросовСторнирования(МетаданныеДокумента) Экспорт
	...
	Если ДвиженияДокумента.Содержит(МетаданныеРегистра) Тогда
		ТекстЗапроса = ПроведениеДокументов.ТекстСторнирующегоЗапроса(
				МетаданныеРегистра,
				МетаданныеДокумента);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "- ДанныеРегистра.НДСРегл",
							"ВЫБОР КОГДА НАЧАЛОПЕРИОДА(ДокументыСторно.Дата, КВАРТАЛ) = НАЧАЛОПЕРИОДА(ДанныеРегистра.Период, КВАРТАЛ)
							|	ТОГДА - ДанныеРегистра.НДСРегл
							|	ИНАЧЕ 0
							|КОНЕЦ");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "- ДанныеРегистра.НДСУпр",
							"ВЫБОР КОГДА НАЧАЛОПЕРИОДА(ДокументыСторно.Дата, КВАРТАЛ) = НАЧАЛОПЕРИОДА(ДанныеРегистра.Период, КВАРТАЛ)
							|	ТОГДА - ДанныеРегистра.НДСУпр
							|	ИНАЧЕ 0
							|КОНЕЦ");
