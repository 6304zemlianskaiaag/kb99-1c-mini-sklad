...
Процедура Выполнить_ФормированиеСводныхСправокНДС(ПараметрыОбработчика) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
		ПараметрыОбработчика.ДанныеЭтапа.Код = Перечисления.ОперацииЗакрытияМесяца.ФормированиеСводныхСправокНДС;
	КонецЕсли;
...
Процедура ПолучитьДокументыОплатыГрафа7(ПараметрыРасчета)
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(Задания.СчетФактура) В (ТИП(Документ.ПервичныйДокумент), ТИП(Документ.Бронирование))
