...
//   Параметры - см. ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ПараметрыНачалаСозданияФайла
//
Процедура СоздатьФайлСДискаПеретаскиванием(МассивФайлов, Параметры) Экспорт
	
	ВыбранныеФайлы = Новый Массив;
	
	Если ТипЗнч(МассивФайлов) = Тип("Массив") Тогда
		Для Каждого Файл Из МассивФайлов Цикл
			Если ТипЗнч(Файл) = Тип("Файл") И Файл.ЭтоФайл() Тогда
				ВыбранныеФайлы.Добавить(Файл.ПолноеИмя);
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипЗнч(МассивФайлов) = Тип("Файл") И МассивФайлов.ЭтоФайл() Тогда
	...
	СоздатьФайлСДискаПослеВыбораФайлов(ВыбранныеФайлы, Параметры);
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура ЗакончитьРедактированиеСРасширениемПослеПомещения(ПомещенныйФайл, Параметры) Экспорт
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура СоздатьФайлСДискаПослеВыбораФайлов(ВыбранныеФайлы, Параметры) Экспорт
...
Процедура СоздатьФайлСДискаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		...
		ВыборФайла.Показать(
			Новый ОписаниеОповещения(
				"СоздатьФайлСДискаПослеВыбораФайлов",
				ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент,
				Параметры));
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура СоздатьФайлСДискаПослеПомещенияФайла(ПомещенныйФайл, Параметры) Экспорт
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура СохранитьИзмененияСРасширениемПослеПомещения(ПомещенныйФайл, Параметры) Экспорт
...
//                          - Неопределено - вызывать ничего не нужно.
//
Процедура СохранитьКакПослеДиалогаВыбораФайла(Знач Результат, Параметры) Экспорт
...
Процедура СоздатьФайлСДискаЗавершение(Параметры)
	...
	Иначе
		...
		СоздатьФайлСДискаПослеВыбораФайлов(Параметры.ПолныеИменаФайлов, Параметры);
	КонецЕсли;
