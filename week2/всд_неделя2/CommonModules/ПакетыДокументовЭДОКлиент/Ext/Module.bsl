...
Процедура СформироватьИДобавитьПроизвольныйДокументВПакет(ПараметрыДобавления, ПараметрыФормирования, ПараметрыФайла)
	...
	ОповещениеПослеФормированияДокумента = Новый ОписаниеОповещения(
		"ДобавитьПроизвольныйДокументВПакетПослеФормированияДокументаПоФайлам", ЭтотОбъект, Контекст);
...
// * Контекст - См. СформироватьЭлектронныйДокументПоФайлам.Контекст
Процедура СформироватьЭлектронныйДокументПоФайлам(Контекст, ОповещениеОЗавершении)
	...
	ПараметрыФайла = Контекст.ПараметрыФайла;
	...
	НовыеФайлыТитула.Основной = ПараметрыФайла;
...
//  * ПараметрыФормирования - См. ИнтерфейсДокументовЭДОКлиентСервер.НовыеПараметрыСозданияДокументаПоФайлу
//  * ПараметрыФайла - См. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
Процедура ДобавитьПроизвольныйДокументВПакетПослеФормированияДокументаПоФайлам(РезультатФормирования, Контекст) Экспорт
	
	ДокументыПакета = ?(Контекст.ПараметрыДобавления.СозданиеНовогоПакета, Новый Массив,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Контекст.ПараметрыДобавления.ЭлектронныйДокумент));
	...
	Для Каждого РезультатОбработкиДокумента Из ОбработанныеДокументы Цикл
		ЭлектронныйДокумент = РезультатОбработкиДокумента.Ключ; // См. ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(ИдентификаторПакета) Тогда
		ОповещениеОЗавершении = Новый ОписаниеОповещения(
			"ДобавитьПроизвольныйДокументВПакетПослеДобавленияДокументаВПакет", ПакетыДокументовЭДОКлиент,
			ПараметрыОповещения);
		ДобавитьДокументыВПакет(ИдентификаторПакета, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЭлектронныйДокумент),
			ОповещениеОЗавершении, РезультатФормирования.КонтекстДиагностики);
	Иначе
		ОповещениеОЗавершении = Новый ОписаниеОповещения(
			"ДобавитьПроизвольныйДокументВПакетПослеСозданияПакетаДокументов", ПакетыДокументовЭДОКлиент,
			ПараметрыОповещения);
		СоздатьПакетДокументов(ДокументыПакета, ОповещениеОЗавершении, РезультатФормирования.КонтекстДиагностики);
	КонецЕсли;
...
//  * ИдентификаторПакета - УникальныйИдентификатор
//  * КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
Процедура ДобавитьПроизвольныйДокументВПакетПослеДобавленияДокументаВПакет(РезультатДобавления, ПараметрыОповещения) Экспорт
...
//  * ИдентификаторПакета - УникальныйИдентификатор
//  * КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
Процедура ДобавитьПроизвольныйДокументВПакетПослеСозданияПакетаДокументов(РезультатСоздания, ПараметрыОповещения) Экспорт
...
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
Процедура ПослеИзмененияПакетаДокументов(ДокументыПакета, ИдентификаторПакета, ИзменениеВыполненоУспешно,
	КонтекстДиагностики)
...
Процедура ПодготовитьДокументДляДобавленияВПакетЗавершение(ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФайла = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
	ПараметрыФайла.ИмяФайла = ИмяФайлаПослеПреобразования;
	ПараметрыФайла.ДвоичныеДанные = ПолучитьИзВременногоХранилища(ДополнительныеПараметры.АдресХранилища);
