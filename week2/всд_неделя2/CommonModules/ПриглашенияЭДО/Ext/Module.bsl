...
// Получает участника ЭДО по приглашениию и проверяет является ли участник актуальной записью справочника. Если участник ЭДО
...
Функция УчастникЭДОПоКлючуИПриглашению(Знач ПараметрыПриглашения, Знач КлючУчастникаЭДО) Экспорт
	...
	Если Не ЗначениеЗаполнено(УчастникЭДО) Тогда
		...
		УчастникЭДО = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП(ТипОбъектаОтправителя, КлючУчастникаЭДО.ИНН,
			КлючУчастникаЭДО.КПП, КлючУчастникаЭДО.ДатаСведений);
		...
		Попытка
			...
			Если ТребуетсяОпределитьКонтрагента(МенеджерЗаписи) Тогда
				МенеджерЗаписи.Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП(ТипОбъектаОтправителя,
						МенеджерЗаписи.ИНН, МенеджерЗаписи.КПП);
			КонецЕсли;				
