...
Процедура кб99_ВСД_УдалитьЗапросы_XML() Экспорт
	...
	Попытка
		...
		КонецЦикла;	
	Исключение
		кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"ВСД Ошибка при очистке файлов логов");
	КонецПопытки;
...
Процедура кб99_ПолучитьАктуальныеПартии(МассивПараметров) Экспорт
	...
	Попытка
		Если ТипЗнч(МассивПараметров) <> Тип("Структура") Тогда
			кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"Ошибка при выполнение регламентного задания: Получить актуальные партии.");
			Возврат;
		КонецЕсли;
...
Процедура кб99_ОбновитьСтатусыВСД(МассивПараметров) Экспорт
	...
	Попытка
		Если ТипЗнч(МассивПараметров) <> Тип("Структура") Тогда
			кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"Ошибка при выполнение регламентного задания: Обновить статусы ВСД");
			Возврат;
		КонецЕсли;
...
Процедура кб99_ГашениеВСД(МассивПараметров) Экспорт
	...
	Попытка
		Если ТипЗнч(МассивПараметров) <> Тип("Структура") Тогда
			кб99_ВСД.кб99_СообщитьОбОшибке("Ошибка при передаче параметров. Ожидалось: Структура, получили: "+ТипЗнч(МассивПараметров)+" ",,"Ошибка при выполнение регламентного задания: Гашение входящих ВСД");
			Возврат;
		КонецЕсли;
		...
		Для Каждого строкаВыборка Из МассивВходящихВСД Цикл
			...
			Номенклатура = кб99_ВСД.ПолучитьНоменклатуруПоПродукцияЭлемент( строкаВыборка.Продукция_Элемент, Истина );
			
			Если ИскатьДокументПрихода Тогда
				
				ДокументПрихода = кб99_ВСД_Гашение.Гашение_НайтиДокументПоступлениеТоваров( строкаВыборка.ДокВСД.ТтнДата, 
			...
			ДатаСрокГодности1= кб99_ВСД_Запросы.СтрокаВДатаВремя( строкаВыборка.ДатаСрокГодности1 );
		...
		КонецЕсли;
	Исключение
		кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"Ошибка при выполнение регламентного задания: Гашение входящих ВСД");
	КонецПопытки;
...
Процедура кб99_ОтложеннаяОтправкаВСД2Транзакции(МассивПараметров) Экспорт
	...
	Попытка
		Если ТипЗнч(МассивПараметров) <> Тип("Структура") Тогда
			кб99_ВСД.кб99_СообщитьОбОшибке("Ошибка при передаче параметров. Ожидалось: Структура, получили: "+ТипЗнч(МассивПараметров)+" ",,"Ошибка при выполнение регламентного задания: Отложенная отправка ВСД2 Транзакции");
			Возврат;
		КонецЕсли;
		...
		КонецЦикла;
	Исключение
		кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"Ошибка при выполнение регламентного задания: Отложенная отправка ВСД2 Транзакции");
	КонецПопытки;
...
Процедура кб99_ОбработатьДокументыПроизводство(МассивПараметров) Экспорт
	...
	Попытка
		Если ТипЗнч(МассивПараметров) <> Тип("Структура") Тогда
			кб99_ВСД.кб99_СообщитьОбОшибке("Ошибка при передаче параметров. Ожидалось: Структура, получили: "+ТипЗнч(МассивПараметров)+" ",,"Ошибка при выполнение регламентного задания: Обработка документов ВСД2 производство");
			Возврат;
		КонецЕсли;
	...
	Исключение
		кб99_ВСД.кб99_СообщитьОбОшибке(ОписаниеОшибки(),,"Ошибка при выполнение регламентного задания: Обработка документов ВСД2 производство");
	КонецПопытки;
