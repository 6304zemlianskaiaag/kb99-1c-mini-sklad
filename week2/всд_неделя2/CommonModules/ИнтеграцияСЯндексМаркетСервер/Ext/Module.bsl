	...
	Попытка
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(НоменклатураКонтрагентовСсылка) Тогда
				НоменклатураКонтрагентовСсылка = ВыборкаДанных.НоменклатураПартнера;
				НоменклатураКонтрагентовОбъект = Неопределено;
				ЭтоНовый                       = Ложь;
				
				ДанныеТовара = ТаблицаТоваров[ВыборкаДанных.НомерСтроки];
				
				Если Не ЗначениеЗаполнено(НоменклатураКонтрагентовСсылка) Тогда
					НоменклатураКонтрагентовОбъект = Справочники.НоменклатураКонтрагентов.СоздатьЭлемент();
					ЭтоНовый = Истина;
					
				ИначеЕсли ВыборкаДанных.НоменклатураПартнераНаименование <> Сред(ДанныеТовара.ПредставлениеТовара, 1, 100)
							Или ВыборкаДанных.НоменклатураПартнераНаименованиеПолное <> Сред(ДанныеТовара.ПредставлениеТовара, 1, 1024)
							Или ВыборкаДанных.НоменклатураПартнераАртикул <> Сред(ДанныеТовара.Артикул, 1, 100)
							Или ВыборкаДанных.НоменклатураПартнераИдентификаторНоменклатуры <> Сред(ДанныеТовара.ИдентификаторКарточкиТовара, 1, 100)
							Или ВыборкаДанных.НоменклатураПартнераНаименованиеНоменклатуры <> Сред(ДанныеТовара.НаименованиеКарточкиТовара, 1, 1000)
							Или ВыборкаДанных.НоменклатураПартнераШтрихкод <> Сред(?(ДанныеТовара.Штрихкоды.Количество() = 0, "", ДанныеТовара.Штрихкоды[0]), 1, 300)
							Или ВыборкаДанных.НоменклатураПартнераКодТНВЭД <> Сред(ДанныеТовара.КодТНВЭД, 1, 10)
							Или СокрЛП(ВыборкаДанных.НоменклатураПартнераСтранаПроисхожденияКод) <> СокрЛП(ВыборкаДанных.СтранаПроисхожденияКод)
							Или ВыборкаДанных.НоменклатураПартнераНедействителен <> ДанныеТовара.Архивный Тогда
					НоменклатураКонтрагентовОбъект = НоменклатураКонтрагентовСсылка.ПолучитьОбъект();
				КонецЕсли;
				
				Если НоменклатураКонтрагентовОбъект <> Неопределено Тогда
					НоменклатураКонтрагентовОбъект.Владелец                           = ВыборкаДанных.Владелец;
					НоменклатураКонтрагентовОбъект.ВладелецНоменклатуры               = ВыборкаДанных.Владелец;
					НоменклатураКонтрагентовОбъект.Наименование                       = ДанныеТовара.ПредставлениеТовара;
					НоменклатураКонтрагентовОбъект.НаименованиеПолное                 = ДанныеТовара.ПредставлениеТовара;
					НоменклатураКонтрагентовОбъект.Артикул                            = ДанныеТовара.Артикул;
					НоменклатураКонтрагентовОбъект.Идентификатор                      = ДанныеТовара.ИдентификаторПредложения;
					НоменклатураКонтрагентовОбъект.ИдентификаторНоменклатуры          = ДанныеТовара.ИдентификаторКарточкиТовара;
					НоменклатураКонтрагентовОбъект.НаименованиеНоменклатуры           = ДанныеТовара.НаименованиеКарточкиТовара;
					НоменклатураКонтрагентовОбъект.Штрихкод                           = ?(ДанныеТовара.Штрихкоды.Количество() = 0, "", ДанныеТовара.Штрихкоды[0]);
					НоменклатураКонтрагентовОбъект.ДругиеШтрихкодыНоменклатурыСтрокой = "";
					НоменклатураКонтрагентовОбъект.КодТНВЭД                           = ДанныеТовара.КодТНВЭД;
					НоменклатураКонтрагентовОбъект.СтранаПроисхожденияКод             = ВыборкаДанных.СтранаПроисхожденияКод;
					НоменклатураКонтрагентовОбъект.Недействителен                     = ДанныеТовара.Архивный;
					НоменклатураКонтрагентовОбъект.ПометкаУдаления                    = Ложь;
					
					НоменклатураКонтрагентовОбъект.ДругиеШтрихкодыНоменклатуры.Очистить();
					Для Каждого ЭлементКоллекции Из ДанныеТовара.Штрихкоды Цикл
						Если ЭлементКоллекции <> НоменклатураКонтрагентовОбъект.Штрихкод Тогда
							СтрокаТабличнойЧасти = НоменклатураКонтрагентовОбъект.ДругиеШтрихкодыНоменклатуры.Добавить();
							СтрокаТабличнойЧасти.Штрихкод = ЭлементКоллекции;
							
							НоменклатураКонтрагентовОбъект.ДругиеШтрихкодыНоменклатурыСтрокой = НоменклатураКонтрагентовОбъект.ДругиеШтрихкодыНоменклатурыСтрокой
								+ "; " + ЭлементКоллекции;
						КонецЕсли;
					КонецЦикла;
					
					Если ЭтоНовый Тогда
						НоменклатураКонтрагентовОбъект.КоличествоУпаковок                  = 1;
						НоменклатураКонтрагентовОбъект.НаименованиеУпаковки                = "-";
						НоменклатураКонтрагентовОбъект.КоличествоБазовойЕдиницыИзмерения   = 1;
						НоменклатураКонтрагентовОбъект.НаименованиеБазовойЕдиницыИзмерения = "-";
						
						Если ВыборкаДанных.ЕстьЗаписьПубликации Тогда
							НоменклатураКонтрагентовОбъект.Номенклатура = ВыборкаДанных.Номенклатура;
							Если ИспользоватьХарактеристикиНоменклатуры Тогда
								НоменклатураКонтрагентовОбъект.Характеристика = ВыборкаДанных.Характеристика;
							КонецЕсли;
							Если ИспользоватьУпаковкиНоменклатуры Тогда
								НоменклатураКонтрагентовОбъект.Упаковка = ?(ВыборкаДанных.Упаковка = ВыборкаДанных.ЕдиницаИзмерения,
																				Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(),
																				ВыборкаДанных.Упаковка);
								Если Не ЗначениеЗаполнено(НоменклатураКонтрагентовОбъект.Упаковка) Тогда
									НоменклатураКонтрагентовОбъект.НаименованиеУпаковки                = Строка(ВыборкаДанных.ЕдиницаИзмерения);
									НоменклатураКонтрагентовОбъект.НаименованиеБазовойЕдиницыИзмерения = НоменклатураКонтрагентовОбъект.НаименованиеУпаковки;
								КонецЕсли;
							КонецЕсли;
							
						ИначеЕсли ПредварительныйПоиск Тогда
							...
							Если ЗначениеЗаполнено(РезультатПоиска.Номенклатура) Тогда
								НоменклатураКонтрагентовОбъект.Номенклатура = РезультатПоиска.Номенклатура;
								Если ИспользоватьХарактеристикиНоменклатуры Тогда
									НоменклатураКонтрагентовОбъект.Характеристика = РезультатПоиска.Характеристика;
								КонецЕсли;
								Если ИспользоватьУпаковкиНоменклатуры Тогда
									НоменклатураКонтрагентовОбъект.Упаковка = РезультатПоиска.Упаковка;
									Если Не ЗначениеЗаполнено(НоменклатураКонтрагентовОбъект.Упаковка) Тогда
										НоменклатураКонтрагентовОбъект.НаименованиеУпаковки                = Строка(РезультатПоиска.ЕдиницаИзмерения);
										НоменклатураКонтрагентовОбъект.НаименованиеБазовойЕдиницыИзмерения = НоменклатураКонтрагентовОбъект.НаименованиеУпаковки;
									КонецЕсли;
								КонецЕсли;
							КонецЕсли;
						...
						Если ЗначениеЗаполнено(НоменклатураКонтрагентовОбъект.Характеристика) Тогда
							НоменклатураКонтрагентовОбъект.ИспользоватьХарактеристики = Истина;
							НоменклатураКонтрагентовОбъект.НаименованиеХарактеристики = Строка(НоменклатураКонтрагентовОбъект.Характеристика);
						КонецЕсли;
						
						Если ЗначениеЗаполнено(НоменклатураКонтрагентовОбъект.Упаковка) Тогда
							ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НоменклатураКонтрагентовОбъект.Упаковка, 
								"Код, Наименование, Числитель, Знаменатель");
							НоменклатураКонтрагентовОбъект.КоличествоУпаковок                  = 1;
							НоменклатураКонтрагентовОбъект.НаименованиеУпаковки                = ЗначенияРеквизитов.Наименование;
							НоменклатураКонтрагентовОбъект.КоличествоБазовойЕдиницыИзмерения   = ?(ЗначенияРеквизитов.Числитель = 0, 1, ЗначенияРеквизитов.Числитель)
								/ ?(ЗначенияРеквизитов.Знаменатель = 0, 1, ЗначенияРеквизитов.Знаменатель);
							НоменклатураКонтрагентовОбъект.НаименованиеБазовойЕдиницыИзмерения = ?(НоменклатураКонтрагентовОбъект.КоличествоБазовойЕдиницыИзмерения = 1,
					...
					Исключение
						НоменклатураКонтрагентовСсылка = Неопределено;
					КонецПопытки;
				КонецЕсли;
