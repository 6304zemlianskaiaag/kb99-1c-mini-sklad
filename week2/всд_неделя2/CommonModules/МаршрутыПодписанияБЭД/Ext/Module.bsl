...
Функция ТаблицаМаршрутаПоПараметрам(ЭлектронныйДокумент, Маршрут, УсловиеПодписания = Неопределено, 
	...
	Если ЗначениеЗаполнено(Маршрут) Тогда
		РеквизитыМаршрута = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Маршрут, "Предопределенный, Организация,
			|ВидПодписи, ТаблицаТребований");
	КонецЕсли;
	
	// Подготовим таблицу маршрута
	Если ЗначениеЗаполнено(Маршрут) И РеквизитыМаршрута.Предопределенный Тогда
		ТаблицаПодписания = ТаблицаПодписанияПоПараметрам(Маршрут, УсловиеПодписания, СписокПодписантов);
	ИначеЕсли ЗначениеЗаполнено(Маршрут) И Не РеквизитыМаршрута.Предопределенный Тогда
	...
	Если ТипЗнч(РодительскийУзелДерева) = Тип("ДеревоЗначений") ИЛИ ЗначениеЗаполнено(РодительскийУзелДерева.Требование) Тогда
		...
		Для Каждого СтрокаДерева Из РодительскийУзелДерева.Строки Цикл
			Если НЕ СтрокаДерева.ТребованиеВыполнимо Тогда
				...
				Если ТекущаяГруппа > 2 Тогда
					Если СтрокаДерева.Строки.Количество() > 0 Тогда
						Если СтрокаДерева.Требование = Перечисления.ТребованияКПодписаниюЭД.ИЛИ Тогда
							СтрокаДерева.Вес = СтрокаДерева.Строки.Количество();
						Иначе
							СтрокаДерева.Вес = 1;
						КонецЕсли;
