...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|	ИЗ
		|		ЧекиККМ КАК ЧекиККМ
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции.ОплатаПлатежнымиКартами КАК ЧекККМОплатаПлатежнымиКартами
		|			ПО (ЧекККМОплатаПлатежнымиКартами.Ссылка = ЧекиККМ.Ссылка)
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
		|			ПО ЧекККМОплатаПлатежнымиКартами.Ссылка = ЧекККМКоррекции.Ссылка
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ
		|		И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
		|			ПО ЧекККМКоррекцииСерии.Ссылка = ЧекККМКоррекции.Ссылка
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ
		|		И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|
		...
		|
		|	ВЫБРАТЬ
		|		ЧекККМКоррекцииПодарочныеСертификаты.ПодарочныйСертификат КАК ПодарочныйСертификат,
		|		1 КАК Количество,
		|		ЧекККМКоррекцииПодарочныеСертификаты.Сумма КАК Сумма,
		|		ЧекККМКоррекцииПодарочныеСертификаты.СуммаВВалютеСертификата КАК СуммаВВалютеСертификата,
		|		ЧекиККМ.КассаККМ КАК КассаККМ,
		|		ЧекиККМ.Организация КАК Организация,
		|		ЧекиККМ.Склад КАК Склад,
		|		ЧекиККМ.Валюта КАК Валюта,
		...
		|	ИЗ
		|		ЧекиККМ КАК ЧекиККМ
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции.ПодарочныеСертификаты КАК ЧекККМКоррекцииПодарочныеСертификаты
		|			ПО (ЧекККМКоррекцииПодарочныеСертификаты.Ссылка = ЧекиККМ.Ссылка)
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
		|			ПО ЧекККМКоррекцииПодарочныеСертификаты.Ссылка = ЧекККМКоррекции.Ссылка
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ
		|		И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		...
		|	ИЗ
		|		ЧекиККМ КАК ЧекиККМ
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции.АкцизныеМарки КАК Таблица
		|			ПО (Таблица.Ссылка = ЧекиККМ.Ссылка)
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
		|			ПО Таблица.Ссылка = ЧекККМКоррекции.Ссылка
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ
		|		И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|
...
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Запрос.Текст = "
		...
		|			ИЛИ ЧекККМТовары.Ссылка.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ,
		|	ЧекККМТовары.Ссылка,
		...
		|	ЧекККМТовары.Ссылка.НалогообложениеНДС,
		|	ВЫБОР
		|		КОГДА ЧекККМТовары.Ссылка.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
		|			ТОГДА НЕОПРЕДЕЛЕНО
...
Функция ВыбратьОтложенныеЧекиККМДляУдаления(КассоваяСмена = Неопределено)
	...
	|ИЗ
	|	Документ.ЧекККМКоррекции КАК ЧекККМКоррекции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ КАК ЧекиККМ
	|		ПО (ЧекиККМ.Ссылка = ЧекККМКоррекции.ЧекККМ)
	|ГДЕ
	|	ЧекККМКоррекции.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
	|	И ЧекККМКоррекции.ВидКоррекции = ЗНАЧЕНИЕ(Перечисление.ВидыЧековКоррекции.НеприменениеККТ)
	|	И (ЧекККМКоррекции.КассоваяСмена = &КассоваяСмена
	|			ИЛИ &КассоваяСмена = НЕОПРЕДЕЛЕНО)
	|	И ЧекиККМ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
