...
Процедура ЗаписатьЛогОперацииНаСервере(Записи) Экспорт
	...
	Для Каждого Операция Из Записи Цикл
		...
		Если ЗначениеЗаполнено(Операция.ВходящиеПараметры) Тогда
			...
			ЗаписатьJSON(ЗаписьJSON, Операция.ВходящиеПараметры, , "ЗаписатьЛогОперацииНаСервере_ПреобразованиеВJSON", ЛогированиеОперацийБПО);
		...
		Если ЗначениеЗаполнено(Операция.ИсходящиеПараметры) Тогда
			ЗаписьJSON = Новый ЗаписьJSON();
			ЗаписьJSON.УстановитьСтроку();
			ЗаписатьJSON(ЗаписьJSON, Операция.ИсходящиеПараметры, , "ЗаписатьЛогОперацииНаСервере_ПреобразованиеВJSON", ЛогированиеОперацийБПО);
