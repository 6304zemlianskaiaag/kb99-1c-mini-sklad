...
Функция ПодготовитьКомандыПечати(ОбъектыПечати) Экспорт
	...
	ШаблонКомандыПечатиНаСервереНеНужен = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаИзТаблицыКоманд);
	...
	Для Каждого КомандаПечатиОбъекта Из КомандыПечатиОбъекта Цикл
		Отбор = Новый Структура("Имя", КомандаПечатиОбъекта.Идентификатор);
		...
		Если НайденныеКоманды.Количество() И НайденныеКоманды[0].Печатать Тогда
			КомандаВКомплект = ОбщегоназначенияКлиентСервер.СкопироватьРекурсивно(ШаблонКомандыПечатиНаСервереНеНужен);
