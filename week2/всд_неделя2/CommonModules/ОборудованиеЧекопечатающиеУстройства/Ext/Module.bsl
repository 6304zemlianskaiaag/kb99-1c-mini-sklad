...
// Возвращает ативность контроля фискализации безналичных платежей
//
// Возвращаемое значение:
//  Булево.
//  
Функция КонтрольФискализацииБезналичныхПлатежей() Экспорт
	...
	Результат = Истина;
	ФискализацииБезналичныхПлатежей = Результат; 
	СтандартнаяОбработка = Истина;
	МенеджерОборудованияВызовСервераПереопределяемый.КонтрольФискализацииБезналичныхПлатежей(ФискализацииБезналичныхПлатежей, СтандартнаяОбработка);
	Результат = ?(СтандартнаяОбработка, Результат, ФискализацииБезналичныхПлатежей);
	Возврат Результат; 
				...
				Если ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					Пока ЧтениеXML.Прочитать() И НЕ (ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента) Цикл
						...
						Если ЧтениеXML.Имя = "IndustryAttribute" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда   
							Если ФискальнаяСтрока <> Неопределено И ДанныхМаркировки Тогда   
...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.Наименование = НСтр("ru = 'ПОСЦентр:ККТ с передачей данных в ОФД (ФФД 1.2) NG УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'ПОСЦентр:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Дримкас:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Инкотекс:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "INCOTEX_KKT_FFD12";   
	
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ;
	Драйвер.ИмяДрайвера  = "ДрайверНЕВА54ФЗ4ФФД12";
	Драйвер.Наименование = НСтр("ru = 'НЕВА:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'РР-Электро:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
...
Процедура ОбработатьПечатьЧекаКоррекции(ДанныеОперации, Знач РезультатВыполнения)
	...
	Если НЕ ДанныеОперации.НеприменениеККТ И ДанныеОперации.СформироватьЧекКоррекцииСторно Тогда
		...
		Исключение
			ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Запись фискальной операции чека коррекции стрно'"), 
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбработатьПолучениеРезультатаПроверкиКодаМаркировки(ДанныеОперации, РезультатВыполнения)
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
		...
		ПараметрыМетода[0] = ДанныеОперации;
		ФоновыеЗадания.Выполнить(
			"ОборудованиеЧекопечатающиеУстройства.ОбработатьПолучениеРезультатаПроверкиКодаМаркировкиВФоне",
			ПараметрыМетода);
...
Процедура ОбработатьПолучениеРезультатаПроверкиКодаМаркировкиВФоне(ДанныеОперации, РезультатВыполнения) Экспорт
...
Процедура ОбработатьПроверкуКодаМаркировки(РезультатВыполнения)
	...
	Если ОбщегоНазначенияБПО.ИспользуетсяМаркировка() Тогда
		...
		ПараметрыМетода = Новый Массив(2);
		ПараметрыМетода[0] = МассивДанныхОбмена;
		ПараметрыМетода[1] = МассивПараметровЗаписей;
		ФоновыеЗадания.Выполнить(
			"ОборудованиеЧекопечатающиеУстройства.ОбработатьПроверкуКодаМаркировкиВФоне",
			ПараметрыМетода);
...
Процедура ОбработатьПроверкуКодаМаркировкиВФоне(МассивДанныхОбмена, МассивПараметровЗаписей) Экспорт
