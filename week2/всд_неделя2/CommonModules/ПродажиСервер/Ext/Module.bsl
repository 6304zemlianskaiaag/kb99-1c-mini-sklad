...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	ОбрабатываемыеСтрокиСуммаДоКорректировки = Новый Соответствие;
	
	СтруктураПересчетаСуммы = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ДокументПродажи);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Сч = 0 По СтрокиЗаказа.Количество()-1 Цикл
			...
			ИначеЕсли ОтменитьВТекущей > 0 Тогда
				...
				ОбрабатываемыеСтрокиСуммаДоКорректировки.Вставить(Строка, СуммаДоКорректировки);
	...
	// Вычисляем СуммаКорректировки после пересчета СуммаСНДС в пакетной обработке 
	Для Каждого Строка Из ОбрабатываемыеСтроки Цикл
		СуммаДоКорректировки = ОбрабатываемыеСтрокиСуммаДоКорректировки.Получить(Строка);
		Если СуммаДоКорректировки <> Неопределено Тогда
				...
				Если СтрокиЗаказа.Количество() > 0 Тогда
					...
					Для Сч = 0 По СтрокиЗаказа.Количество()-1 Цикл
						...
						Если КоличествоНовойСтроки > 0 Тогда
							...
							ОбрабатываемыеСтроки.Добавить(НовСтр);
...
// Адрес грузополучателя брать из адреса доставки.
//
// Параметры:
// 		Ссылка - ДокументСсылка - Ссылка на первичный документ.
//
// Возвращаемое значение:
// 		Булево - Адрес грузополучателя брать из адреса доставки
//
Функция АдресГрузополучателяБратьИзАдресаДоставки(Ссылка) Экспорт
	
	АдресГрузополучателяБратьИзАдресаДоставки = Константы.АдресГрузополучателяБратьИзАдресаДоставки.Получить()
		И (ТипЗнч(Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаРеализации"));
		
	Возврат АдресГрузополучателяБратьИзАдресаДоставки;
	
КонецФункции

// Возвращает Документ основание по указанным полям поиска, используется когда ссылка на основание размещается
// в разных реквизитах (в том числе реквизитах табличной части).
// 
// Параметры:
//     Объект - ДокументОбъект, ДанныеФормыСтруктура - необходимо присутствие свойств указанных в ПоляПоиска.
//         ДокументОснование - ДокументСсылка - свойство обязательно, если параметр ПоляПоиска не передан.
//     ПоляПоиска - Строка - Свойства объекта указанные через запятую в порядке приоритета,
//         если указано поле типа Булево, значение которого Ложь, функция вернет Неопределено,
//         например "АктПоЗаказам" для Акта выполненных работ. Как минимум одно поле должно быть ссылочного типа.
//         Можно указать поле таблицы, например "Товары.ЗаказКлиента".
// 
// Возвращаемое значение:
//     ДокументСсылка, Неопределено - Документ основание.
//
Функция ДокументОснование(Объект, ПоляПоиска = "") Экспорт
	
	ДокументОснование = Неопределено;
	
	ПоляПоискаКоллекция = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПоляПоиска, ",",, Истина);
	
	Для Каждого Имя Из ПоляПоискаКоллекция Цикл
		
		РеквизитТабличнойЧасти = СтрНайти(Имя, ".") > 0;
		
		// Если имя содержит точку "Товары.ЗаказКлиента", документ основание ищем в таблице "Товары".
		// Если документ на основании нескольких заказов, тогда возвращаем Неопределено
		Если РеквизитТабличнойЧасти Тогда
			
			ИмяТаблицыИмяРеквизита = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Имя, ".");
			ИмяТаблицы = ИмяТаблицыИмяРеквизита[0];
			ИмяРеквизита = ИмяТаблицыИмяРеквизита[1];
			
			Основания = Новый Соответствие();
			
			Для Каждого Строка Из Объект[ИмяТаблицы] Цикл
				Если ЗначениеЗаполнено(Строка[ИмяРеквизита]) Тогда
					Основания.Вставить(Строка[ИмяРеквизита]);
				КонецЕсли;
			КонецЦикла;
			
			Если Основания.Количество() = 1 Тогда
				Для Каждого Элемент Из Основания Цикл
					ДокументОснование = Элемент.Ключ;
				КонецЦикла;
			Иначе
				ДокументОснование = Неопределено;
			КонецЕсли;
			
		// Если реквизит с типом булево равен Ложь, следует что документ не имеет основания
		ИначеЕсли ТипЗнч(Объект[Имя]) = Тип("Булево") И Не Объект[Имя] Тогда
			ДокументОснование = Неопределено;
			Прервать;
			
		// Если реквизит с типом булево есть и равен Истина, продолжаем обход
		ИначеЕсли ТипЗнч(Объект[Имя]) = Тип("Булево") И Объект[Имя] Тогда
			Продолжить;
		
		// В общем случае заполняем документ основание из указанного реквизита
		ИначеЕсли ЗначениеЗаполнено(Объект[Имя]) Тогда
			ДокументОснование = Объект[Имя];
			Прервать;
		КонецЕсли;
...
Функция ЗаполнитьПоФактическойОтгрузке(ОбъектДокумента, СтруктураДокументаОтгрузки) Экспорт
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтроки,
		СтруктураДокументаДляЗаполнения.СтруктураДействийСИзмененнымиСтроками,
		СтруктураДокументаДляЗаполнения.Товары,
		КэшированныеЗначения);
	...
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокиТЧ(
		ОбрабатываемыеСтроки,
		СтруктураДокументаДляЗаполнения.СтруктураДействийСДобавленнымиСтроками,
		СтруктураДокументаДляЗаполнения.Товары,
		КэшированныеЗначения);
...
Функция ПараметрыПроверкиКорректностиЗаполненияДокументаПродажи(ДокументПродажи, ИмяТаблицы, ИмяТЧ, ПредставлениеТЧ) Экспорт
	...
	ПараметрыПроверкиКорректностиЗаполнения.Вставить("ПредставлениеПоляСклад", Неопределено);
...
Процедура СообщитьОбОшибкахПроверкиКорректностиЗаполненияДокументаПродажи(МассивПроверок, МассивРезультатов, ПараметрыПроверкиКорректностиЗаполнения, Отказ) Экспорт
	...
	ПредставлениеПоляСклад = ПараметрыПроверкиКорректностиЗаполнения.ПредставлениеПоляСклад;
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьЗаполненияСкладаВТабличнойЧастиТовары" Тогда
			...
			Если ПараметрыПроверкиКорректностиЗаполнения.ИмяТаблицы = "Документ.ВозвратТоваровОтКлиента" Тогда
				
				ПредставлениеПоляСклад = НСтр("ru = 'Получатель'")
				
			ИначеЕсли ПараметрыПроверкиКорректностиЗаполнения.ИмяТаблицы = "Документ.ЗаявкаНаВозвратТоваровОтКлиента" Тогда
				
				ПредставлениеПоляСклад = НСтр("ru = 'Отправитель'")
				
			КонецЕсли;	
			
			СообщитьОбОшибкахКорректностьЗаполненияСкладаВТабличнойЧастиТовары(Выборка, ДокументПродажи, Отказ, ПредставлениеПоляСклад);
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		Если Не ВозвратПоЧеку Тогда
			...
			Иначе
				...
				|	ВЫБОР
				|		КОГДА ВозвратТоваровОтКлиентаТовары.СтатусУказанияСерий В (14, 18)
				...
				|ГДЕ
				|	НЕ ВозвратТоваровОтКлиентаТовары.ДокументРеализации В (
				|			НЕОПРЕДЕЛЕНО,
				...
				|ПОМЕСТИТЬ ПроверяемыеТовары
				|ИЗ
				|	ПроверяемыеТоварыИзТаблицы КАК ВозвратТоваровОтКлиентаТовары
				|
				|ГДЕ
				|	НЕ (ВозвратТоваровОтКлиентаТовары.ДокументРеализации Ссылка Документ.РеализацияТоваровУслуг
				|		И ВЫРАЗИТЬ(ВозвратТоваровОтКлиентаТовары.ДокументРеализации КАК Документ.РеализацияТоваровУслуг).ВернутьМногооборотнуюТару
				|		И ВЫРАЗИТЬ(ВозвратТоваровОтКлиентаТовары.Номенклатура КАК Справочник.Номенклатура).ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
				|	)
				|
				|ИНДЕКСИРОВАТЬ ПО
				|	Номенклатура,
			...
			|ГДЕ
			|	(КорректировкаРеализации.Ссылка.ДокументОснование, КорректировкаРеализации.Дата) В
			|			(ВЫБРАТЬ
			...
			|	ВЫБОР
			|		КОГДА ТоварыДокумента.СтатусУказанияСерий В (14, 18)
			|			ТОГДА ТоварыДокумента.Серия
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
			|	КОНЕЦ								КАК Серия,
			|	СУММА(ТоварыДокумента.Количество)	КАК Количество,
			|	ДанныеШапки.ДокументОснование		КАК ДокументРеализации
			...
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовСторно
			|		ПО ТоварыДокумента.Ссылка = РеестрДокументовСторно.СторнируемыйДокумент
			|			И РеестрДокументовСторно.Проведен
			|			И НЕ РеестрДокументовСторно.ДополнительнаяЗапись
			|
			|ГДЕ
			...
			|	ВЫБОР
			|		КОГДА ТоварыДокумента.СтатусУказанияСерий = 14
			|			ТОГДА ТоварыДокумента.Серия
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
			|	КОНЕЦ								КАК Серия,
			|	СУММА(ТоварыДокумента.Количество)	КАК Количество,
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ДанныеШапки
			|		ПО ТоварыДокумента.Ссылка = ДанныеШапки.Ссылка
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовСторно
			|		ПО ТоварыДокумента.Ссылка = РеестрДокументовСторно.СторнируемыйДокумент
			...
			|	ТоварыДокумента.Назначение		КАК Назначение,
			|	СУММА(ТоварыДокумента.Количество) КАК Количество,
			|	ДанныеШапки.ДокументРеализации	КАК ДокументРеализации
			|ПОМЕСТИТЬ ДанныеДокументовВозврата
			|ИЗ
			|	Документ.ВозвратТоваровОтКлиента.Товары КАК ТоварыДокумента
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента КАК ДанныеШапки
			|		ПО ТоварыДокумента.Ссылка = ДанныеШапки.Ссылка
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументовСторно
			|		ПО ТоварыДокумента.Ссылка = РеестрДокументовСторно.СторнируемыйДокумент
			...
			|	ДанныеШапки.Проведен
			|	И РеестрДокументовСторно.Ссылка ЕСТЬ NULL
			|	И ТоварыДокумента.Ссылка <> &ЭтотВозвратСсылка
			|	И ДанныеШапки.ЗаявкаНаВозвратТоваровОтКлиента <> &ЭтотВозвратСсылка
			|	И (ТоварыДокумента.ДокументРеализации, ТоварыДокумента.Номенклатура, ТоварыДокумента.Характеристика) В
			|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|	КОНЕЦ,
			|	ТоварыДокумента.Назначение,
			|	ДанныеШапки.ДокументРеализации
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура,
			|	Характеристика,
			...
			|ИЗ
			|	ДанныеДокументовВозврата КАК ТоварыВозвратов
			|
			|СГРУППИРОВАТЬ ПО
			|	ТоварыВозвратов.Номенклатура,
			|	ТоварыВозвратов.Характеристика,
			|	ТоварыВозвратов.ДокументРеализации
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура,
			|	Характеристика,
			...
			|ИЗ
			|	ДанныеРеализацииБезНазначения КАК ДанныеРеализации
			|		ЛЕВОЕ СОЕДИНЕНИЕ ПроверяемыеТоварыБезНазначения КАК ПроверяемыеТовары
			...
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеВозвратаБезНазначения КАК ДанныеВозврата
			|		ПО ДанныеРеализации.ДокументРеализации = ДанныеВозврата.ДокументРеализации
			|			И ДанныеРеализации.Номенклатура = ДанныеВозврата.Номенклатура
			|			И ДанныеРеализации.Характеристика = ДанныеВозврата.Характеристика
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ ПроверяемыеТовары КАК ПроверяемыеТовары
			|		ПО ДанныеРеализации.ДокументРеализации = ПроверяемыеТовары.ДокументРеализации
			|			И ДанныеРеализации.Номенклатура = ПроверяемыеТовары.Номенклатура
			|			И ДанныеРеализации.Характеристика = ПроверяемыеТовары.Характеристика
			...
			|	ПроверяемыеТовары.Характеристика		КАК Характеристика,
			|	ВЫБОР
			|		КОГДА НЕ ОбщиеОстатки.Количество ЕСТЬ NULL
			|				И ОбщиеОстатки.Количество >= 0
			|			ТОГДА ПроверяемыеТовары.Назначение
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
			|	КОНЕЦ									КАК Назначение,
			...
			|		ПО ПроверяемыеТовары.Номенклатура = СправочникНоменклатура.Ссылка
			|ГДЕ
...
// 									для вызова "ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ".
...
Функция ТекстВременнойТаблицыКоэффициентовУпаковок(ИмяТаблицыТоваров = "ТаблицаТоваров") Экспорт
	...
	|			ВЫБОР
	|				КОГДА ТаблицаТоваров.Номенклатура.НаборУпаковок = ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры)
	|					ТОГДА ТаблицаТоваров.Номенклатура
	|				КОГДА ТаблицаТоваров.Номенклатура.НаборУпаковок <> ЗНАЧЕНИЕ(Справочник.НаборыУпаковок.ПустаяСсылка)
	|					ТОГДА ТаблицаТоваров.Номенклатура.НаборУпаковок
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|ГДЕ
	...
	|	&ИмяТаблицыТоваров КАК ТаблицаТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиНоменклатуры
	|		ПО УпаковкиНоменклатуры.Владелец = ТаблицаТоваров.Упаковка.Владелец
	|ГДЕ
	|	НЕ УпаковкиНоменклатуры.ПометкаУдаления
	|	И ТаблицаТоваров.Упаковка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|
...
Процедура ДополнитьТекстЗапросаЗаполнитьУсловияПродаж(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ТребуетсяВыполнитьДействия Тогда
		...
		Если КэшированныеЗначения.ИспользуетсяЦенообразование25 Тогда
			СписокЗапросов = ТекстыЗапросовУсловияПродаж25ПакетнаяОбработка(Ложь, ПараметрыДействия, КэшированныеЗначения);
		Иначе
			...
			СписокЗапросов = ТекстыЗапросовУсловияПродажПакетнаяОбработка(Ложь, ИменаКолонок);
	...
	Если ТребуетсяВыполнитьДействия Тогда
		...
		Если КэшированныеЗначения.ИспользуетсяЦенообразование25 Тогда
			СписокЗапросов = ТекстыЗапросовУсловияПродаж25ПакетнаяОбработка(Истина, ПараметрыДействия, КэшированныеЗначения);
		Иначе
...
Функция ВариантОбеспеченияИФлагОбособленноПоУмолчанию(Соглашение, Статус, ВариантИФлагПоУмолчанию = Неопределено,
	...
	Если ВариантИФлагПоУмолчанию <> Неопределено Тогда
		...
		Если ФлагПоУмолчанию = Неопределено Тогда // флаг нужно получать штатно для каждой отдельной номенклатуры
			Результат.Товар.Обособленно = Неопределено;
			Результат.Работа.Обособленно = Неопределено;
		КонецЕсли;
...
Процедура СформироватьЗапросТоварыДокументаПродажи(ТекстЗапроса, ИмяТаблицы, ИспользоватьПроверкиПоЦенам)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	СоглашениеШапка.КомиссионныеПродажи25       КАК КомиссионныеПродажи25,
		|	ВЫБОР
		|		КОГДА
...
Процедура СформироватьЗапросНаличиеТоваровВЕТИСВДокументеПродажи(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ИЛИ ДокументТовары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.КормаДляЖивотныхВлажныеПодконтрольныеВЕТИС)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.ЕстьОшибкиЗаполненияВидЦены И (Выборка.ЦенаСоглашение = 0 Или ИспользуетсяЦенообразование25) Тогда
			...
			Иначе
				...
				Если Выборка.КомиссионныеПродажи25 Тогда
					Продолжить;
				КонецЕсли;
...
//
// Параметры:
//	ТаблицаОшибок - ТаблицаЗначений - таблица, содержащая сведения об ошибках корректности заполнения возвращаемых товаров.
//	ВозвратПоЧеку - Булево - признака возврата по чеку.
//
Процедура СообщитьОбОшибкахКорректностиВозвращаемыхТоваров(Знач ТаблицаОшибок, ВозвратПоЧеку)
	...
	Для Каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		Если ЗначениеЗаполнено(СтрокаОшибки.ДокументРеализации)
			И ТипЗнч(СтрокаОшибки.ДокументРеализации) = Тип("ДокументСсылка.ПередачаТоваровХранителю")
		...
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Возврат по номенклатуре %Номенклатура%%Характеристика%%Серия%%Назначение% превышает количество реализованных товаров по %ТипДокумента% %НомерРеализации% на %Количество% %ЕдиницаИзмерения%'");
		КонецЕсли;
		...
		Если СтрокаОшибки.Количество < 0 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаОшибки.Характеристика) Тогда
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке,
												"%Характеристика%", " \ " + СтрокаОшибки.Характеристика);
			Иначе
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%Характеристика%", "");
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаОшибки.Серия) Тогда
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке,
												"%Серия%", " \ " + СтрокаОшибки.Серия);
			Иначе
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%Серия%", "");
			КонецЕсли;
			
			Если ВозвратПоЧеку Тогда
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%ТипДокумента%", НСтр("ru = 'чеку ККМ'"));
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%Назначение%", "");
			Иначе
				СообщениеОбОшибке = СтрЗаменить(СообщениеОбОшибке, "%ТипДокумента%", НСтр("ru = 'документу продажи'"));
				
				Если ЗначениеЗаполнено(СтрокаОшибки.Назначение) Тогда
...
//  Объект  - СправочникОбъект, СправочникСсылка, ДокументОбъект, ДокументСсылка - объект, создаваемый на основании клиента.
//
Процедура ПроверитьВозможностьВводаНаОснованииПартнераКлиента(Партнер, Объект) Экспорт
	...
	Если Не ПартнерКлиент Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
			ТекстОшибки = НСтр("ru = 'Создание ""%1"" возможно для контрагентов с признаком ""Клиент"".'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Создание ""%1"" возможно для партнеров с признаком ""Клиент"".'");
		КонецЕсли;
		ТекстОшибки = СтрШаблон(ТекстОшибки, Объект.Метаданные().Представление());
...
Процедура ДополнитьТекстЗапросаДанныеПоДокументуПриобретения(СтруктураДействий, ОписаниеЗапроса, КэшированныеЗначения) Экспорт
	...
	Если ПакетнаяОбработкаТабличнойЧастиСервер.ТребуетсяВыполнитьДействие(
		...
		ТипыДокументовВКолонкеДокументПриобретения = ПакетнаяОбработкаТабличнойЧастиСервер.ТипКолонкиВИсточникеДанных("ДокументПриобретения", КэшированныеЗначения).Типы();
...
Функция ТекстЗапросаПоЗначащимРеквизитамРаспоряжений()
	...
	ТекстыРаспоряжениеНаОформление.Добавить(
		...
		|		ВЫБОР
		|			КОГДА ИСТИНА
		|				ТОГДА РаспоряженияНаОтгрузкуОбороты.Номенклатура.ВариантОформленияПродажи
		|		КОНЕЦ КАК ВариантОформленияПродажи,
		...
		|		ВЫБОР
		|			КОГДА ИСТИНА
		|				ТОГДА ТоварыКОтгрузкеОстатки.Номенклатура.ВариантОформленияПродажи
		|		КОНЕЦ КАК ВариантОформленияПродажи,
		|		ВЫБОР
		|			КОГДА ТоварыКОтгрузкеОстатки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг ТОГДА
...
Функция ПолучитьРеквизитыРаспоряжений(МассивРаспоряжений) Экспорт
	...
	|	ЕСТЬNULL(ШапкаДокумента.Договор.КомиссионныеПродажи25, ЛОЖЬ) КАК КомиссионныеПродажи25,
	...
	Для Каждого Колонка Из РезультатЗапроса.Колонки Цикл
		...
		СтруктураРеквизитов.Вставить(ИмяКолонки, Выборка[ИмяКолонки]);
	КонецЦикла;
...
Процедура СоздатьДокументПродажи(ПоляДокумента, ПараметрыОснования, МассивДокументов, ПараметрыФормирования) Экспорт
	...
	ИначеЕсли ПоляДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера Тогда
		...
		Если ПараметрыОснования.РеквизитыШапки.КомиссионныеПродажи25 Тогда
			МенеджерНакладной = Документы.ПоступлениеТоваровОтХранителя;
		Иначе
			МенеджерНакладной = Документы.ВозвратТоваровОтКлиента;
		КонецЕсли;
		НовыйДокумент = МенеджерНакладной.СоздатьДокумент();
	...
	Если НовыйДокумент.ПроверитьЗаполнение() Тогда
		...
		Если ПровестиДокумент Тогда
			Попытка
				НовыйДокумент.ДополнительныеСвойства.Вставить("РежимЗаписи", РежимЗаписиДокумента.Проведение);
...
Функция ТекстыЗапросовУсловияПродажПакетнаяОбработка(УсловияПродажПоУслугам, ИменаКолонок = Неопределено)
	...
	Если УсловияПродажПоУслугам Тогда
		...
		|		(ВЫБРАТЬ
		|			ВременнаяТаблицаВидыЦен.Номенклатура,
		|			ВременнаяТаблицаВидыЦен.Характеристика,
		|			ВременнаяТаблицаВидыЦен.ВидЦены
		|		ИЗ
		|			ВременнаяТаблицаВидыЦен КАК ВременнаяТаблицаВидыЦен)) КАК ТаблицаЦенСрезПоследних
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Характеристика,
	...
	Иначе
		...
		Иначе
			ТекстПоляУпаковка = "СоглашениеТовары.Упаковка";
		КонецЕсли;
		...
		|	ТаблицаЦенСрезПоследних.Характеристика КАК Характеристика,
		|	ТаблицаЦенСрезПоследних.ВидЦены КАК ВидЦены,
		|	ТаблицаЦенСрезПоследних.Цена КАК Цена,
		|	ТаблицаЦенСрезПоследних.Упаковка КАК Упаковка,
		|	ТаблицаЦенСрезПоследних.Валюта КАК Валюта
		|ПОМЕСТИТЬ ВременнаяТаблицаЦеныНоменклатуры
		|ИЗ
		|	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(КОНЕЦПЕРИОДА(&Дата, ДЕНЬ), (Номенклатура, Характеристика, ВидЦены) В
		|		(ВЫБРАТЬ
...
Функция ТекстыЗапросовУсловияПродаж25ПакетнаяОбработка(УсловияПродажПоУслугам, ПараметрыДействия, КэшированныеЗначения)
	...
	Если УсловияПродажПоУслугам Тогда
		...
		|	ВЫБОР
		|		КОГДА
		|			ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоХарактеристике = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
		|		ИНАЧЕ ТаблицаХарактеристик.ХарактеристикаНоменклатурыДляЦенообразования
		|	КОНЕЦ КАК ХарактеристикаЦО,
		|	ЕСТЬNULL(ВЫБОР
		|		КОГДА
		|			ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоСерии = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
		...
		|		ПО ТаблицаТовары.Номенклатура = ТаблицаНоменклатуры.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ТаблицаХарактеристик
		|		ПО ТаблицаТовары.Характеристика = ТаблицаХарактеристик.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК ТаблицаСерий
		|		ПО ТаблицаТовары.Характеристика = ТаблицаСерий.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ТаблицаВидовНоменклатуры
		|		ПО ТаблицаНоменклатуры.ВидНоменклатуры = ТаблицаВидовНоменклатуры.Ссылка
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	ХарактеристикаЦО,
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА
		|			ТаблицаВидовНоменклатуры.НастройкиКлючаЦенПоУпаковке = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.НеИспользовать)
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	...
	ПолеВидЦены = ?(ПакетнаяОбработкаТабличнойЧастиСервер.ЕстьКолонкаВИсточникеДанных("ВидЦены", КэшированныеЗначения),
		"ТаблицаТовары.ВидЦены", "ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеВидЦены", ПолеВидЦены);
	...
	|	ЕСТЬNULL(МИНИМУМ(ВременнаяТаблицаВидыЦен.Приоритет), 0) КАК Приоритет
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
			...
			"ВЫБОР
			|		КОГДА ВременнаяТаблицаВидыЦен.ВидЦены = ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)
			|		ИЛИ ВременнаяТаблицаВидыЦен.ВидЦены ЕСТЬ NULL
	...
	|		ИЗ
	|			ТаблицаОтбора КАК ТаблицаОтбора)) КАК ТаблицаЦен
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	ХарактеристикаЦО,
	...
	|ПОМЕСТИТЬ ЗаполнитьУсловияПродажПредварительная
	|ИЗ
	|	ТаблицаРезультата КАК ТаблицаРезультата
	|ГДЕ
	|	(ТаблицаРезультата.ИдентификаторСтрокиВТ, ТаблицаРезультата.Приоритет) В
	|		(ВЫБРАТЬ
	|			Приоритеты.ИдентификаторСтрокиВТ КАК ИдентификаторСтрокиВТ,
	|			Приоритеты.Приоритет КАК Приоритет
	|		ИЗ
	|			ТаблицаРезультатаПриоритеты КАК Приоритеты)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ИдентификаторСтрокиВТ";
	
	Результат.Добавить(ТекстЗапроса, "ЗаполнитьУсловияПродажПредварительная");
	...
	|ИЗ
	|	ВтИсточникДанных КАК ВтИсточникДанных
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьУсловияПродажПредварительная КАК ТаблицаПредварительная
