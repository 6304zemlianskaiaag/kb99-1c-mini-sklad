...
Функция СвойстваЗапросаПолученияДанныхВыбораСтатейАктивовПассивов(Параметры)
	...
	Если Параметры.Свойство("ОбъектыФинансовогоУчета") Тогда
		...
		Если Параметры.Отбор.Свойство("СчетаУчетаВНастройкахПроводок", СчетаУчетаВНастройкахПроводок)
			И СчетаУчетаВНастройкахПроводок Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
			|	И СтатьиАктивовПассивов.СчетаУчетаВНастройкахПроводок";
		КонецЕсли;
		
		НесобственныеПодконтрольныеЦенности = Ложь;
		Если Параметры.Отбор.Свойство("НесобственныеПодконтрольныеЦенности", НесобственныеПодконтрольныеЦенности)
			И НЕ НесобственныеПодконтрольныеЦенности Тогда
			ОтборПоНазначениюСтатей = ОтборПоНазначениюСтатей + "
