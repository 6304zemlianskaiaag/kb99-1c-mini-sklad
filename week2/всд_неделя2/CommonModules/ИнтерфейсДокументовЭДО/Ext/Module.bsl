...
Функция НастройкиПодбораУчетногоДокумента(ЭлектронныйДокумент, СпособОбработки = "") Экспорт
	...
	Если ИнформацияОтправителя <> Неопределено Тогда
		...
		Если СодержаниеСообщения <> Неопределено Тогда
			...
			Иначе
				ЗначенияРекивзитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				...
				ПараметрыПриглашения.ИдентификаторОрганизации = ЗначенияРекивзитов.ИдентификаторОрганизации;
				ПараметрыПриглашения.ИдентификаторКонтрагента = ЗначенияРекивзитов.ИдентификаторКонтрагента;
...
Процедура ДополнитьЭлементамиВнешнегоЭДО(Форма, Отказ, СтандартнаяОбработка)
	...
	Если ГруппаДляДобавленияЭлемента = Неопределено Или 
		Не ТипЗнч(ГруппаДляДобавленияЭлемента) = Тип("ГруппаФормы") Тогда
		Возврат;
	КонецЕсли;
