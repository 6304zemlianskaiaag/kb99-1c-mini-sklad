...
Процедура ПроверитьПодписьЗавершение(Результат, Контекст) Экспорт
	...
	Контекст.РезультатПроверки.СвойстваПодписи.ДатаПроверкиПодписи = ОбщегоНазначенияКлиент.ДатаСеанса();
	Контекст.РезультатПроверки.СвойстваПодписи.ПодписьВерна = РезультатКЭП.Успех = Истина;
	Контекст.РезультатПроверки.СвойстваПодписи.Вставить("ПропуститьПриПродлении", Истина);
...
#Иначе
	...
	ОповещениеПослеПолученияОтпечатков = Новый ОписаниеОповещения("ПослеПолученияОтпечатков",
		ЭтотОбъект,
		ПараметрыЗавершенияУстановкиРасширения);
		
	ИнтеграцияРутокенМобильныйЭДОКлиент.ПолучитьСертификаты(ОповещениеПослеПолученияОтпечатков);
...
Процедура ДобавитьУникальнуюОшибкуКриптографии(КонтекстДиагностики, ВидОперации,
	...
	ВидОшибкиКриптография.ОписаниеРешения = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		НСтр("ru = '<a href = ""Ознакомьтесь"">Ознакомьтесь</a> с рекомендациями и <a href = ""Выполните"">выполните</a> диагностику криптографии </a>'"));
	...
	ВидОшибкиКриптография.ОписаниеПроблемы = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<a href = ""Показать"">Показать</a> список проблем'"));
...
// Функиция возвращает уникальный идентификатор для ошибки исходя ее описания
// Параметры:
//  ТекстОшибки - Строка
// Возвращаемое значение:
//  Строка
