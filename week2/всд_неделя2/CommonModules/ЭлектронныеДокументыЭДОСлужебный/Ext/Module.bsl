...
Функция СоздатьСообщение(ОписаниеСообщения, ЭлектронныйДокумент, НастройкиДокумента) Экспорт
	...
	ПараметрыФайла = КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(
		ОписаниеСообщения.Данные.Документ.ДвоичныеДанные);
	ИдентификаторДокумента = Неопределено;
	Если ТипЗнч(ПараметрыФайла) = Тип("Структура") 
		И ПараметрыФайла.Свойство("ИдентификаторДокумента", ИдентификаторДокумента) Тогда
		Если ЗначениеЗаполнено(ИдентификаторДокумента) Тогда
			...
			ОписаниеСообщения.Данные.Документ.ИмяФайла = СтрШаблон("%1%2", ИдентификаторДокумента, 
