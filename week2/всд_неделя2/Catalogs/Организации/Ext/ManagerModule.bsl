...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	|ГДЕ
	|	(Организации.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОрганизацийПодразделений.ПустаяСсылка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если НЕ ЗначениеЗаполнено(ДанныеОбъекта.Статус) Тогда
				ДанныеОбъекта.Статус = Перечисления.СтатусыОрганизацийПодразделений.Действует;
			КонецЕсли;
