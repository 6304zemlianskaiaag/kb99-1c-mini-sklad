...
Процедура ПерезаполнитьСведенияОДоверителе(Доверитель)
	...
	Если ЗначениеЗаполнено(Доверитель) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Доверитель)) Тогда
		...
		Если ИнтеграцияЭДО.ЭтоФизЛицо(Доверитель) Тогда
			...
			КонтактнаяИнформация = ИнтеграцияБСПБЭД.КонтактнаяИнформацияОбъекта(Доверитель, "ТелефонОрганизации");
...
&НаСервереБезКонтекста
Функция ЗначениеПолейКонтактнойИнформации(Представление, ВидКонтактнойИнформации)
	
	ТелефонЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(
		Представление, ВидКонтактнойИнформации);
	
	Возврат ТелефонЗначенияПолей;
...
Процедура НачатьРедактированиеТелефона(СтандартнаяОбработка, ИмяЭлемента)
	...
	ТелефонЗначениеПолей = ЗначениеПолейКонтактнойИнформации(
		Телефон, ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонОрганизации"));
	
	ПараметрыФормы = УправлениеКонтактнойИнформациейКлиент.ПараметрыФормыКонтактнойИнформации(
		ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонОрганизации"), ТелефонЗначениеПолей);
		
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаНомераТелефона", ЭтотОбъект, ИмяЭлемента);
...
Процедура НачатьРедактированиеАдресаОрганизации(ИмяЭлемента, ЗаголовокОкнаРедактирования = "")
	...
	ИнтеграцияБСПБЭДКлиент.НачатьРедактированиеАдреса(
		Адрес, ДанныеКонтактнойИнформации.Значение, ОбработкаЗавершения, ДанныеКонтактнойИнформации);
...
&НаСервере
Процедура ЗакончитьРедактированиеАдресаОрганизацииНаСервере(Результат, ИмяЭлемента) Экспорт
