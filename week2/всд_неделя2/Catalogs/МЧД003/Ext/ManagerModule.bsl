			...
			Для Каждого ПолномочиеXDTO Из МашПолнXDTO Цикл
				...
				Если ЕстьОграничения Тогда
					...
					Для Каждого ОграничениеXDTO Из ОгрСвПолнXDTO Цикл	
						...
						ПроизвольныеСведения.Параметры.Буллет = Символ(8226);
...
Функция ТаблицаДанныхМЧДДляПодписания(Доверенности)
	...
	|	МЧД003.СтатусВРеестреФНС КАК СтатусВРеестреФНС,
	|	ЕСТЬNULL(ЖурналМашиночитаемыхДоверенностей.ВсеДоверители, """") КАК ДоверителиИзЖуранала,
	|	МЧД003.Доверители.(
	|		Ссылка,
	|		НомерСтроки,
	|		ТипДоверителя,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ДоверителиИзЖуранала) Тогда
			
			ДобавитьДоверенностиПоДоверителямЖурнала(ТаблицаДанныхМЧД,
				СведенияОСтатусеМЧД,
				РегистрыСведений.ЖурналМашиночитаемыхДоверенностей.СведенияОДоверителях(Выборка.ДоверителиИзЖуранала));
...
//  ДоверителиИзЖуранала - см. РегистрСведенийМенеджер.ЖурналМашиночитаемыхДоверенностей.СведенияОДоверителях
Процедура ДобавитьДоверенностиПоДоверителямЖурнала(ТаблицаДанныхМЧД, СведенияОСтатусеМЧД, ДоверителиИзЖуранала)
	
	Для Каждого ДанныеДоверителя Из ДоверителиИзЖуранала Цикл
