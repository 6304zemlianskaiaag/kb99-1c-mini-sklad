...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ВетИС_ДокументооборотСервер.ПроверитьВозможностьИспользованияОтчета();
КонецПроцедуры
...
Функция СформироватьОтчетНаСервере()
	...
	Пока ВыборкаПоПродукции.Следующий() Цикл
		...
		Пока ВыборкаПартии.Следующий() Цикл
			...
			ОбластьСтрПартия.Параметры.ДатаИзготовления = Формат(ВетИС_APIСервер.СтрокаВДату(ВыборкаПартии.ДатаИзготовления), "ДЛФ=D");
			ОбластьСтрПартия.Параметры.СрокГодности = Формат(ВетИС_APIСервер.СтрокаВДату(ВыборкаПартии.СрокГодности), "ДЛФ=D");
