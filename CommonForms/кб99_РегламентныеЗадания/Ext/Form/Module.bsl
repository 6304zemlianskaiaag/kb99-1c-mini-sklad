...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось получить список регламентных заданий по гашению входящих ВСД"+Символы.ПС+ОписаниеОшибки());
	КонецПопытки;
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось получить список регламентных заданий по отправке ВСД2 Транзакции"+Символы.ПС+ОписаниеОшибки());
	КонецПопытки;
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось получить список регламентных заданий по обработке производства"+Символы.ПС+ОписаниеОшибки());
	КонецПопытки;
...
Функция ЗаписатьПараметрыРегламентногоЗадания()
	...
	Если ИспользованиеЗагружатьПартии И Площадка.Количество() = 0 Тогда
		...
		ВетИС_ЯдроСервер.СообщитьПользователю(ТекстОшибки,,"Площадка");
	...
	ИначеЕсли ИспользованиеОбновитьСтатусыВСД И Не ЗначениеЗаполнено(ОбновлятьСтатусыЗа) Тогда
		...
		ВетИС_ЯдроСервер.СообщитьПользователю(ТекстОшибки,,"ОбновлятьСтатусыЗа");
		НайденыОшибки = Истина;
	КонецЕсли;
	...
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось сохранить данные по регламентным заданиям ""Гашение входящих ВСД"""+Символы.ПС+ОписаниеОшибки());	
	КонецПопытки;
	...
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось сохранить данные по регламентным заданиям ""Отправка ВСД Транзакции"""+Символы.ПС+ОписаниеОшибки());	
	КонецПопытки;
	...
	Исключение
		ВетИС_ЯдроСервер.СообщитьИнфо("Не удалось сохранить данные по регламентным заданиям ""обработка производства"""+Символы.ПС+ОписаниеОшибки());	
	КонецПопытки;
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗаданиеГашение(Знач Действие)
	...
	ОткрытьФорму("ОбщаяФорма.кб99_РегламентноеЗаданиеГашение", ПараметрыФормы, ЭтаФорма);
...
Процедура СписокЗаданийГашениеПередУдалением(Элемент, Отказ)
	...
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияГашениеПередУдалениемЗавершение", ЭтаФорма),
...
Процедура УдалитьРегламентноеЗаданиеГашениеВыполнитьНаСервере(Знач Идентификатор)
	...
	ВетИС_РегламентныеЗаданияСервер.УдалитьРегламентноеЗадание(Идентификатор);
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗаданиеОтправкаВСДТранзакции(Знач Действие)
	...
	ОткрытьФорму("ОбщаяФорма.кб99_РегламентноеЗаданиеОтправкаВСДТранзакции", ПараметрыФормы, ЭтаФорма);
...
Процедура СписокЗаданийОтправкаВСДТранзакцииПередУдалением(Элемент, Отказ)
	...
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияОтправкаВСДПередУдалениемЗавершение", ЭтаФорма),
			НСтр("ru = 'Удалить регламентное задание?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗаданиеПроизводство(Знач Действие)
	...
	ОткрытьФорму("ОбщаяФорма.кб99_РегламентноеЗаданиеПроизводство", ПараметрыФормы, ЭтаФорма);
...
Процедура СписокЗаданийПроизводствоПередУдалением(Элемент, Отказ)
	...
	Иначе
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПроизводствоЗавершение", ЭтаФорма),
			НСтр("ru = 'Удалить регламентное задание?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
