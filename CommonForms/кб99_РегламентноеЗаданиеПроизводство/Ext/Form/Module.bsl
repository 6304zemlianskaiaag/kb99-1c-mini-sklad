...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Действие = "Добавить" Тогда
		...
		КонецЦикла;
	Иначе
		Задание = ВетИС_РегламентныеЗаданияСервер.ПолучитьРегламентноеЗадание(Параметры.Идентификатор);
		...
		Иначе
			СообщенияПользователюИОписаниеИнформацииОбОшибке =
				ВетИС_РегламентныеЗаданияСервер.СообщенияИОписанияОшибокРегламентногоЗадания(Задание);
		КонецЕсли;
		...
		Если ТипЗнч(МассивПараметров) = Тип("Массив") И МассивПараметров.Количество() > 0 Тогда
			...
			Если МассивНастроек.Свойство("ПериодЗакрытияТранзакции") Тогда
				ПериодЗакрытияТранзакции = МассивНастроек.ПериодЗакрытияТранзакции;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если Действие <> "Изменить" Тогда
		...
		Наименование = ?(
			Действие = "Добавить",
			"",
			ВетИС_РегламентныеЗаданияСервер.ПредставлениеРегламентногоЗадания(Задание));
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Действие = "Добавить" Тогда
		...
		ИмяМетаданных = "кб99_ОбработатьДокументыПроизводство";
		СинонимМетаданных = "кб99 Обработать документы ВСД2 Производство";
		ИмяМетодаМетаданных = "ВетИС_РегламентныеЗаданияСервер.кб99_ОбработатьДокументыПроизводство";
		Наименование = "кб99 Обработка ВСД2 производства";
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ВетИС_КлиентскийСлойКлиент.ПоказатьПодтверждениеЗакрытияФормы(Оповещение, Отказ, ЗавершениеРаботы);
...
Процедура ЗаписатьРегламентноеЗадание()
	...
	Оповестить("Запись_РегламентныеЗаданияОбработкаПроизводства", Результат);
...
Процедура ЗаписатьРегламентноеЗаданиеНаСервере()
	...
	СтруктураНастройки.Вставить("ПериодЗакрытияТранзакции", ПериодЗакрытияТранзакции);
	...
	Если Действие = "Изменить" Тогда
		ВетИС_РегламентныеЗаданияСервер.ИзменитьРегламентноеЗадание(Идентификатор, ПараметрыЗадания);
	Иначе
		ПараметрыЗадания.Вставить("Метаданные", Метаданные.РегламентныеЗадания[ИмяМетаданных]);
		
		Задание = ВетИС_РегламентныеЗаданияСервер.ДобавитьРегламентноеЗадание(ПараметрыЗадания);
