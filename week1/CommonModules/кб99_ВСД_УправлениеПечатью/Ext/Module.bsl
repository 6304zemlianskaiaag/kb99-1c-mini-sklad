Функция КомпонентаФормированияQRКода(Отказ)
	...
	Попытка
		...
		Иначе
			кб99_ВСД.СообщитьПользователю(ТекстОшибки, , , , Отказ);
		КонецЕсли
	Исключение
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ОписаниеОшибки());
		кб99_ВСД.СообщитьПользователю(ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки, , , , Отказ);
	КонецПопытки;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	мГенерацииШК = кб99_ВСД_Утилиты.ОбщийМодуль("ГенерацияШтрихкода");
