...
Процедура СтатусЗапросаВПроцессе(ДокСсылка)
	...
	МенеджерЗаписи.Пользователь = кб99_ВСД_Параметры.ПолучитьТекущегоПользователя();
...
Процедура Пауза(Секунд = 5) Экспорт
	...
	Попытка
		...
		Если ФоновоеЗадание = Неопределено Тогда
			...
			//	, 
			//	, 
			//	НСтр("ru = 'кб99_ВСД_Запросы.Пауза() разрешается использовать только в фоновом задании.'"));
			Параметры = Новый Массив;
			Параметры.Добавить(Секунд);
			ФоновоеЗадание = ФоновыеЗадания.Выполнить("кб99_ВСД_Запросы.Пауза", Параметры);
		КонецЕсли;
...
&НаСервере
Функция Площадка_Создать_Запрос(ПараметрыОрганизации, ВыбПлощадка) // Убраны лишние пробелы после (
...
Функция ХозСубъект_ЗагрузитьПоGUID(Знач Параметры, GUID, Ссылка_ВСД_ХозСубъект = Неопределено) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ХозСубъект_Объект.Контрагент) Тогда
			...
			Иначе // Нашли по ИНН
				Если кб99_ВСД_Утилиты.ЕстьРеквизитИлиСвойствоОбъекта(Ссылка_Контрагент, "ГоловнойКонтрагент") Тогда
...
Функция ХозСубъект_НайтиПоИНН(Контрагент) Экспорт
	...
	Попытка
		...
		ПараметрыОрганизации = кб99_ВСД.ЗагрузитьПараметры(кб99_ВСД_Параметры.ПолучитьОрганизациюПоУмолчанию());
...
Функция ПолучитьПартии2_СИзменениями_ЗапросXML(Параметры)
...
Функция Партии_Ответ_ЗагрузитьXML(Параметры, xdto)
	...
	Если ПришлоПартий > 0 Тогда // Иначе возникает ошибка если получено 0 партий
		...
		Если кб99_ВСД_Гашение.ФормироватьДвиженияПартий(Параметры.Организация) И Не Параметры.Свойство("ЭтоВходящийВСД") Тогда
			Параметры.Вставить("СписокПартий", СписокПартий);
			кб99_ВСД.ОформитьДокументПартииВСД(Параметры);
		КонецЕсли;
...
Функция УровниУпаковки_из_XML(PList, Партия = "")
	...
	Попытка
		Для каждого Узел ИЗ PListArray Цикл
			...
			Попытка 
				СтрУровня.ФормаУпаковки = НайтиФормуУпаковки(Узел.packingType.guid, Узел.packingType.name); 
			Исключение 
				КонецПопытки;
...
Функция Партия_Заполнить(Параметры, stockEntry, докСсылка = Неопределено)
	...
	Если кб99_ВСД_Утилиты.ЕстьРеквизитИлиСвойствоОбъекта(batch.origin, "country") Тогда
		СтранаПроисхожденияGuid = batch.origin.country.guid;
		СтранаПроисхожденияName = batch.origin.country.name;
		Партия.СтранаПроисхождения = НайтиСтрануПоGUID(СтранаПроисхожденияGuid, СтранаПроисхожденияName);
		...
		Если ЭтоНоваяПартия И ЗначениеЗаполнено(Партия.СтранаПроисхождения) Тогда
			...
			Если Партия.ИсключенаИзПодбора Тогда
				СообщитьИнфо(" ВСД_Партия от " + Партия.ВсдДата + " # " + Партия.НомерЗаписи + " помещена на карантин.", Партия);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьТзЛабИсследования()
...
Функция ЛабораторныеИсследованияИзXML(laboratoryResearch)
...
Процедура Фасовка_из_XML(packaging, ФасовкаНаименование = "", ФасовкаФормаУпаковки = "", ФасовкаКоличество = "", ФасовкаОбъем = "", ФасовкаЕдиницаИзм = "")
...
Функция ПолучитьСписокВСД2_ЗапросXML(ПараметрыЗапроса)
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ДанныеОДокументе) Тогда
			...
			Если ДатаПроизводства < ДатаИзготовления Тогда
				кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В ВСД [%1] дата изготовления больше даты производственной транзакции. Дата изготовления: %2, дата производственной транзакции: %3", 
			...
			ИначеЕсли ДатаПроизводства - ДатаИзготовления > МаксимальноеОтклонениеОтДатыПроизводства * 86400 Тогда
				кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В ВСД [%1] дата изготовления меньше даты производства более %2 суток. Дата изготовления: %3, дата производственной транзакции: %4", 
			...
			КонецЕсли;
		Иначе
			кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В ВСД [%1] нет данных о происхождении продукции", СтрокаТЧ.НаименованиеПродукции), СтрокаТЧ.ДокВСД);
...
Функция ПроверитьЗаполнениеДокумента(ДокСсылка, ПараметрыОрганизации)
	...
	ЗапросВетис = кб99_ВСД_Утилиты.НайтиПоследнийЗапрос(ДокСсылка);
	...
	Если ЗапросВетис.СтатусЗапросаПеречисление = Перечисления.кб99_СтатусЗапроса.IN_PROCESS ИЛИ
		...
		СообщитьИнфо("Последний запрос отправлен, но ответ не получен, статус запроса = " + ЗапросВетис.СтатусЗапроса + ". Получите ответ Ветис", ДокСсылка);
		Ответ = Ложь;
	КонецЕсли;
...
Процедура ЗафиксироватьОшибку(СообщениеОбОшибке, ДокСсылка)
	
	СообщитьИнфо(СообщениеОбОшибке, ДокСсылка);
...
Функция ПроверитьЗаполнениеДокументаТранзакция(ДокСсылка, ПараметрыОрганизации)
	...
	Если Не кб99_ВСД_Параметры.ПроверитьЗначениеПараметра(ПараметрыОрганизации, 
	...
	Для Каждого Стр Из тзПартии Цикл
		Если Стр.Количество > Стр.Партия.Количество Тогда
			Ответ = Ложь;
			ОписаниеОшибки = ОписаниеОшибки + "В строке " + Стр.НомерСтроки + " количество списания превышает остаток партии " + Символы.ПС;
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьЗаполнениеДокументаПроизводство(ДокСсылка, ПараметрыОрганизации)
	...
	Для Каждого Стр Из тзПартии Цикл
		Если Стр.Количество > Стр.Партия.Количество Тогда
			...
			кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 %2", ДокСсылка, Комментарий));
		КонецЕсли;
	КонецЦикла;
	...
	Если ПараметрыОрганизации["ПарамКонтроллироватьСрокГодностиПриОтправке"] Тогда 
		Для Каждого Стр Из тзПартии Цикл
			...
			Если ЗначениеЗаполнено(ДатаСрокГодности1) Тогда 
				Если ДатаСрокГодности1 - ПараметрыОрганизации["СрокГодностиДней"] * 86400 <= ТекущаяДата() Тогда
					кб99_ВСД.СообщитьИнфо("Выбрана просроченная партия по срокам годности: " + ДатаСрокГодности1 + " (менее " + ПараметрыОрганизации["СрокГодностиДней"] + " дней до окончания срока) в строке " + Стр.НомерСтроки + "!", ДокСсылка);
					Ответ = Ложь;
				КонецЕсли;
			КонецЕсли
		КонецЦикла;
...
Процедура СохранитьИПровестиУспешныйВСД(ПараметрыФункции)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		кб99_ВСД.кб99_СообщитьОбОшибке("Не удалось записать реквизиты запроса в " + докСсылка + " " + Символы.ПС + ОписаниеОшибки(), ДокСсылка);
	КонецПопытки;
...
Функция ЗагрузитьXML_Партии2(ПараметрыФункции, xdto)
	...
	Исключение
		СообщитьИнфо("Партии отсутствуют в запросе");
		Возврат Ложь;
	КонецПопытки;
	...
	Для Каждого stockEntry ИЗ stockEntryList Цикл
		Партия = Партия_Заполнить(Параметры, stockEntry, докСсылка);
	...
	Если кб99_ВСД_Гашение.ФормироватьДвиженияПартий(ДокСсылка.Организация) Тогда
...
Функция ЗагрузитьXML_ВСД2(ПараметрыФункции, xdto) Экспорт
	...
	Для каждого vetDocument ИЗ vetDocumentList Цикл
		...
		Попытка Фасовка_из_XML(batch.productItem.packaging, ВсдСтруктура.ФасовкаНаименование, ВсдСтруктура.ФасовкаФормаУпаковки,
		...
		Попытка
			Если Параметры.КонтролироватьНаличиеЛабИсследований Тогда
				тзЛабИсследования = ЛабораторныеИсследованияИзXML(vetDocument.authentication.laboratoryResearch);
			Иначе
				тзЛабИсследования = СоздатьТзЛабИсследования();
			КонецЕсли;
		Исключение
			тзЛабИсследования = СоздатьТзЛабИсследования();
		КонецПопытки;
		...
		Если ЗаписатьОтветXML Тогда 
			СохранитьЗапросВРегистрСведений(Параметры, xdto, ВсдСтруктура.ДокВСД, applicationId, ВсдСтруктура.Статус);
		КонецЕсли;
...
Функция ПроверитьНомерАвтомобиля(РезультатПроверки, Документ)
...
Функция ВСД2_производство_ЗапросXML(Параметры, ДокСсылка)
	...
	Для А = 0 По тз.Количество() - 1 Цикл
		...
		Попытка
			...
			КонецЕсли;
		Исключение 
			СообщитьИнфо("Обновите модуль интеграции !");
		КонецПопытки;
		...
		тзУпаковка = ДокСсылка.УровниУпаковки.Выгрузить();
		тзМаркировка = ДокСсылка.Маркировка.Выгрузить();
		Запрос = Запрос + УровниУпаковки_в_XML(СтрокаДокумента.КлючСтроки, тзУпаковка, тзМаркировка);
...
Функция ОтправитьЗапрос(ПараметрыФункции)
	...
	СтатусЗапросаВПроцессе(докСсылка);
	...
	Если ПолучитьОтветВетис Тогда 
		...
		Для А = 1 По 10 Цикл
			Если (Статус = Перечисления.кб99_СтатусЗапроса.IN_PROCESS) Тогда
				// Пауза( ПараметрыФункции.Параметры["ПаузаСек"] );
...
Процедура ПроверитьПроизводствоПередОтправкой(Результат, ПараметрыОрганизации, ДокСсылка, тзПартииСписания)
	...
	Если Не РезультатЗапроса[0].Пустой() Тогда
		...
		Иначе
			...
			МинСрокГодностиСырья = СтрокаВДату(ВыборкаПоСырью.СрокГодности);
			...
			//	кб99_ВСД.СообщитьИнфо("Изменение сроков годности в документе " + ДокСсылка);
			//	ДокОбъект = ДокСсылка.ПолучитьОбъект();
			//	Для Каждого Стр Из СтрокиКИзменениюСроков Цикл 
			//		ДокОбъект.Продукция[Стр.НомерСтроки - 1].ДатаСрокГодности1 = Стр.СрокГодности;
			//		кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В строке %1 изменен срок годности %2 -> %3",
	...
	Если ЗначениеЗаполнено(Комментарий) Тогда
		кб99_ВСД.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("В документе %1 содержаться ошибки: %2. Документ не отправлен.", ДокСсылка, СокрЛП(Комментарий)));
...
Функция ВСД2_ЛабораторныеИсследования_ОтправитьПараллельно(Знач Параметры, АдресХранилища = Неопределено) Экспорт
	...
	Для Каждого ДокСсылка Из СписокДокументовПолучитьОтвет Цикл
		appID = кб99_ВСД_Утилиты.НайтиApplicationID(ДокСсылка);
		ПараметрыФункции.Вставить("ApplicationID", appID);
...
Процедура ВСД2_транзакция_Аннулировать( Знач Параметры, ВСД2_Транзакция_Ссылка ) Экспорт
	...
	Для Каждого строкаВСД ИЗ тзВСД Цикл
		...
		Статус = ВСД2_Аннулировать( ПараметрыФункции );
	КонецЦикла;
...
Функция ЗагрузитьXML_ПеревозкаРазрешена_2( xdto, Решение="") Экспорт 
	...
	Если (ТипЗнч(seList)<>Тип("СписокXDTO"))И (ТипЗнч(seList)<>Тип("ОбъектXDTO")) Тогда
		СообщитьИнфо("Не удалось получить условия перевозки ");
		Возврат "";
	КонецЕсли;
...
Функция ЗагрузитьОтветУсловияПеревозки(seList, докСсылка) 
	
	Если (ТипЗнч(seList)<>Тип("СписокXDTO"))И (ТипЗнч(seList)<>Тип("ОбъектXDTO")) Тогда
		СообщитьИнфо("Не удалось прочитать условия перевозки ");
		Возврат "";
	КонецЕсли;
	Если ТипЗнч(seList)<>Тип("СписокXDTO") Тогда 
		RouteSectionList = Новый Массив;
...
Функция ВСД2_Входящий_ГашениеПолучитьОтвет(Параметры, appID, докСсылка, ВСД_Партия = Неопределено, СделатьПаузу = Истина ) Экспорт
	...
	Если кб99_ВСД_Гашение.ФормироватьДвиженияПартий(ДокСсылка.Организация) И ЗначениеЗаполнено(ВСД_Партия) Тогда
