...
Функция ПолучитьОрганизациюПоХС(ВыбХозСубъект) Экспорт
	...
	Возврат	кб99_ВСД_Параметры.ПолучитьОрганизациюПоУмолчанию();
	
КонецФункции

Функция НайтиХозСубъект(Контрагент) Экспорт
	
	ПереопределенныйМодуль = кб99_ВСД_Утилиты.ФункцияПереопределена("НайтиХозСубъект");
	...
	Если кб99_ВСД_Утилиты.ЕстьРеквизитИлиСвойствоОбъекта(Контрагент, "ГоловнойКонтрагент") Тогда
...
Процедура ОтправитьВСДвГИС(Знач ДокСсылка, АдресХранилища = Неопределено) Экспорт
	...
	Если кб99_ВСД_Утилиты.ЕстьРеквизитИлиСвойствоОбъекта(ДокСсылка, "Организация") Тогда 
...
Функция ЗагрузитьПараметры( _Организация ) Экспорт
	...
	ПараметрыОрганизации = кб99_ВСД_Параметры.ПараметрыСервер();
...
Функция ПодобратьПартииПоПродукцияЭлемент_( ПараметрыФункции, тзПартий, ПродукцияЭлемент, КолвоСписания, СерияНоменклатуры="", Номенклатура=Неопределено, Контрагент = Неопределено ) Экспорт
	...
	ПереопределенныйМодуль = кб99_ВСД_Утилиты.ФункцияПереопределена("ПодобратьПартииПоПродукцияЭлемент");
	Если ПереопределенныйМодуль <> Неопределено Тогда
