...
// @grace-purpose: Форма обработки новых входящих партий ВетИС — просмотр, пометка и привязка партий к документам поступления
...
Процедура ЗаписатьДокументПриходаВСправочникПартии(ВыбранныеПартии, РезультатВыбора, ЕстьОшибкаОтсутсвуетВДокументе)
	...
	НоменклатураДокумента = ВетИС_УтилитыКлиентСервер.СвернутьМассив(НоменклатураДокумента);
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Отказ Тогда
			ВетИС_ЯдроСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
