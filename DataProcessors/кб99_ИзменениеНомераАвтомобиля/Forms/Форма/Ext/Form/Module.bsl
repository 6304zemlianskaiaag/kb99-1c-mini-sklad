...
// @grace-purpose: Форма изменения номера автомобиля для подтверждённых ВСД-транзакций
...
Процедура ОтправкаДокументовВсдОбновитьТранспорт()
	...
	Для Каждого СтрокаТЗ Из тзВСД Цикл
		...
		ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(ВСД2_Транзакция.Организация);
		...
		СтрокаТЗ.СтатусЗапроса = ВетИС_APIСервер.ВСД2_ОбновитьТранспорт_Отправить(ПараметрыФункции);
...
Процедура ЗаполнитьФорму(тзТочкиМаршрута)
	...
	Если Результат.Пустой() Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("По документу %1 не оформлены ВСД", ВСД2_Транзакция));
		Возврат;
	КонецЕсли;
