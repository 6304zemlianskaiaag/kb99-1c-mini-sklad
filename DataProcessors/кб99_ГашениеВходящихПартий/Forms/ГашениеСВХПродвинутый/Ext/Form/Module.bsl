...
// @grace-purpose: Форма ГашениеСВХПродвинутый — расширенный UI гашения ВСД СВХ с поддержкой распределения продукции
...
Процедура кнГотово(Команда)
	...
	Для А = 0 По ГрузыНаСВХ.Количество() - 1 Цикл
		...
		Если РаспределеннаяПродукция.Количество() > 0 Тогда
			...
			Для Каждого элем ИЗ РаспределеннаяПродукция Цикл
				Если элем.Количество = 0 Тогда
					...
					ВетИС_ЯдроСервер.СообщитьПользователю(ТекстОшибки,, "ГрузыНаСВХ.ВСД_Партия",, Отказ);
		...
		Иначе
			...
			ВетИС_ЯдроСервер.СообщитьПользователю(ТекстОшибки,,"ГрузыНаСВХ[А].ВСД_Партия",, Отказ);
		КонецЕсли;
	КонецЦикла;
...
Функция СоздатьДокВСД(элемПродукция, ДокВСД)
	...
	Попытка
		...
		обДокВСД.Записать();
	Исключение
		ВетИС_ЯдроСервер.кб99_СообщитьОбОшибке(ОписаниеОшибки());
	КонецПопытки;
