...
// @grace-purpose: Форма гашения входящих ВСД на СВХ — загрузка входящих ВСД и отправка запросов на гашение
...
Процедура кнГотово(Команда)
	...
	Если Не ЗначениеЗаполнено(ПриходуемаяПродукция) Тогда
		ВетИС_ЯдроСервер.СообщитьПользователю("Укажите продукцию",, "ПриходуемаяПродукция", "ЭтаФорма.ПриходуемаяПродукция");
		Возврат;
	КонецЕсли;
