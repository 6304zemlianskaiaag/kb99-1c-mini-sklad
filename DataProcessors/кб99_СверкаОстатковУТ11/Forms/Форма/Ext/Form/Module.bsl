// GRACE:MODULE_START name="СверкаОстатков"
// @grace-module: СверкаОстатков
// @grace-purpose: Форма сверки остатков партий ВетИС с данными УТ11
// @grace-depends: ЯдроВСД, ИнтеграцияВетИС_API, РегистрыПартий, НСИ_Базовые
// @grace-version: 1.0
// @grace-contract-ref: docs/Contracts.xml -> module="СверкаОстатков"
...
Процедура ЗаполнитьОстаткиНаСервере(СписокНоменклатуры = Неопределено)
	...
	Для Каждого Стр Из тзПартии Цикл
		...
		Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре(Стр.Номенклатура);
		...
		тзСписок = ВетИС_ЯдроСервер.ПолучитьСписок_ВСД_Продукция_ЭлементПоНоменклатуре(Стр.Номенклатура);
	...
	Иначе
		...
		для каждого стрПартии из тзПартии цикл
			тзСписок= ВетИС_ЯдроСервер.ПолучитьСписок_ВСД_Продукция_ЭлементПоНоменклатуре(стрПартии.Номенклатура);
			для каждого стрТЗСписок из тзСписок цикл
				сп.Добавить(стрТЗСписок.ПродукцияЭлемент);
			конеццикла;
		КонецЦикла;
	...
	Для Каждого стрПартия из тзПартии Цикл
		стрПартия.ДатаСрокГодности1 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПартия.СрокГодности1);
		стрПартия.ДатаСрокГодности2 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПартия.СрокГодности2);
		стрПартия.ДатаИзготовления1 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПартия.ДатаПроизводства1);
		стрПартия.ДатаИзготовления2 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПартия.ДатаПроизводства2);
	КонецЦикла;
...
Процедура ЗаполнитьСверкуНаСервере()
	...
	Для Каждого строкаТовар из тзТовары Цикл
		...
		тзПЭ = ВетИС_ЯдроСервер.ПолучитьСписок_ВСД_Продукция_ЭлементПоНоменклатуре(Товар);
	...
	Для Каждого СтрПартии Из Партии Цикл
		...
		Если ЗначениеЗаполнено(СтрПартии.ДатаИзготовления1) Тогда
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.ЕдиницаИзмерения) Тогда
				ВетИС_ЯдроСервер.СообщитьИнфо("Не заполнена ЕдиницаИзмерения в " + НоваяСтрока.Партия);
			КонецЕсли;
...
&НаКлиенте
// @grace-contract: СоздатьИнвентаризациюДаты
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="СоздатьИнвентаризациюДаты"
...
Функция КорректировкаОстатков()
	...
	Для Каждого СтрОстатки1с Из тзОстатки1с Цикл
		Если ЗначениеЗаполнено(СтрОстатки1с.Количество) И СтрОстатки1с.Количество = СтрОстатки1с.ВНаличииОстаток Тогда
			НоваяСтрока = ДокВСД.Продукция.Добавить();
			НоваяСтрока.Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре(СтрОстатки1с.Номенклатура);
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.ЕдиницаИзмерения) Тогда
				ВетИС_ЯдроСервер.СообщитьИнфо("Не заполнена ЕдиницаИзмерения в " + НоваяСтрока.Продукция_Элемент);
			КонецЕсли;
			НоваяСтрока.НаименованиеПродукции = НоваяСтрока.Продукция_Элемент.Наименование;
	...
	Для Каждого СтрПартии Из тзОстаткиМеркурий Цикл
		Если СтрПартии.Количество = СтрПартии.КоличествоПартии Тогда
			Если СтрПартии.Количество > 0 И Не СтрПартии.Цвет = "Красный" Тогда
				...
				НоваяСтрока.ДатаИзготовления1 = ВетИС_APIСервер.СтрокаВДатаВремя(СтрДата1);
				НоваяСтрока.ДатаСрокГодности1 = СтрПартии.ДатаСрокГодности1;
				СтрДата2 = НоваяСтрока.Партия.ДатаИзготовления2;
				НоваяСтрока.ДатаИзготовления2 = ВетИС_APIСервер.СтрокаВДатаВремя(СтрДата2);
...
&НаКлиенте
// @grace-contract: СоздатьИнвентаризациюОстатки
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="СоздатьИнвентаризациюОстатки"
...
&НаКлиенте
// @grace-contract: ОткорректироватьНаСервере
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ОткорректироватьНаСервере"
...
КонецПроцедуры
// GRACE:MODULE_END name="СверкаОстатков"