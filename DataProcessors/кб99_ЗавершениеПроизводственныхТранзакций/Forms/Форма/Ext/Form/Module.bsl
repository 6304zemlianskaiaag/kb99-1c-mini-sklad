...
// @grace-purpose: Форма завершения незавершённых производственных транзакций ВетИС
...
Функция кнЗавершитьВыбранныеТранзакцииНаСервере()
	...
	Для Каждого СтрТЧ Из СписокДокументов Цикл
		Если СтрТЧ.Отметка Тогда
			...
			ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(СтрТЧ.Организация);
