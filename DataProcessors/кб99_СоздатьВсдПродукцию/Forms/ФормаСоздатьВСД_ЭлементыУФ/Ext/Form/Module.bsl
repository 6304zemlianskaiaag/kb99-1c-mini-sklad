...
// @grace-purpose: Форма создания/изменения элементов продукции ВСД (сопоставление номенклатуры с ВСД_Продукция_Элемент)
...
&НаСервере
Функция ЗаполнитьНоменклатуруВТЗ()
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрЭлемент.Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре(Выборка.Ссылка); //Проверим на наличие соответствия
...
Функция СоздатьВСДЭлементы()
	...
	Если (Не ЗначениеЗаполнено(Объект.Отправитель_Площадка)) или (Не ЗначениеЗаполнено(Объект.Организация)) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Укажите Организацию и Площадку");
		Возврат "";
	КонецЕсли;
	...
	Для Каждого СтрЭлемент Из Объект.ВСДЭлементы Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
			...
			НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
				ВетИС_ЯдроСервер.СообщитьИнфо("Обязательные реквизиты для "+СтрЭлемент.Номенклатура+" в строке "+СтрЭлемент.номерСтроки+" не заполнены !!!");
				Продолжить;
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция_Элемент) Тогда
				ВетИС_ЯдроСервер.СообщитьИнфо("Что-то пошло не так. Не создался Продукция Элемент для "+СтрЭлемент.Номенклатура);
				Продолжить;
			КонецЕсли;
			ВетИС_ЯдроСервер.Установить_Соответствие_ВСД_Продукция_Элемент(СтрЭлемент.Номенклатура,СтрЭлемент.Продукция_Элемент);
		КонецЕсли;
		
		Попытка 
			ВетИС_APIСервер.Продукция_Элемент_Изменить( ПараметрыОрганизации, СтрЭлемент.Продукция_Элемент,"CREATE");
			СтрЭлемент.GUID = СтрЭлемент.Продукция_Элемент.GUID;
		Исключение
...
&НаСервере
Функция ИзменитьВСДЭлементы(ИмяКоманды = "UPDATE")
	...
	Для Каждого СтрЭлемент Из Объект.ВСДЭлементы Цикл
		...
		Если НЕ(ЗначениеЗаполнено(СтрЭлемент.GUID)) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("В строке "+стрЭлемент.НомерСтроки+" нет элемента для изменения или удаления в Меркурий, пропускаю.");
			Продолжить;
		КонецЕсли;
		...
		Если Не(ИмяКоманды = "DELETE") Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрЭлемент.Продукция) или
				...
				НЕ ЗначениеЗаполнено(СтрЭлемент.Номенклатура) или 
				НЕ ЗначениеЗаполнено(СтрЭлемент.ЕдиницаИзмерения)	Тогда
					ВетИС_ЯдроСервер.СообщитьИнфо("Обязательные реквизиты не заполнены !!!");
					Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Попытка 
			ВетИС_APIСервер.Продукция_Элемент_Изменить( ПараметрыОрганизации, СтрЭлемент.Продукция_Элемент, ИмяКоманды );
			СтрЭлемент.GUID = СтрЭлемент.Продукция_Элемент.GUID;
		Исключение
			ВетИС_ЯдроСервер.СообщитьИнфо("Что-то пошло не так. Не удалось создать в Меркурий "+СтрЭлемент.Продукция_Элемент);
			ВетИС_ЯдроСервер.СообщитьИнфо(ОписаниеОшибки());
		КонецПопытки;
	КонецЦикла;
...
Процедура кнЗаполнить(Команда)
	...
	Если (НЕ ЗначениеЗаполнено(ВыбНоменклатура)) или
		...
		ВетИС_ЯдроСервер.СообщитьИнфо("Не все параметры для заполнения указаны");
		Возврат;
	КонецЕсли;
...
&НаКлиенте
// @grace-contract: кнСоздатьОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнСоздатьОтвет"
...
&НаКлиенте
// @grace-contract: кнИзменитьОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнИзменитьОтвет"
...
&НаКлиенте
// @grace-contract: кнУдалитьОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнУдалитьОтвет"
...
Функция ПолучитьСведенияОПродЭлементе( ВыбНоменклатура ) 	
	...
	НайденЭлемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре( ВыбНоменклатура );
