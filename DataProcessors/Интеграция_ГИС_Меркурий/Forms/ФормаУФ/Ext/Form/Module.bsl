...
// @grace-purpose: Главная рабочая форма интеграции с ГИС Меркурий — управление отгрузками, партиями, производством, лабораторными исследованиями
// @grace-depends: ЯдроВСД, ИнтеграцияВетИС_API, ДокументооборотВСД, УправлениеФормамиВСД, ДлительныеОперации
// @grace-version: 1.0
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Маршрут", Объект.Маршрут);
	...
	Элементы.Отправитель_ПлощадкаНаФорме.ТолькоПросмотр = Не ВетИС_ПараметрыСервер.ЭтоПолноправныйПользователь();
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьЗаголовокКнопкиОтмены();
...
Процедура ПредупреждениеПользователю(ТекстПредупреждения) Экспорт
	ПоказатьПредупреждение(, ТекстПредупреждения, 0, "Предупреждение");
КонецПроцедуры
...
// используется ВетИС_УтилитыКлиентСервер.СообщенияПользователю
//
...
Функция ПолучитьСообщенияНаСервере()
	...
	Исключение
		...
		Сообщения = ВетИС_УтилитыКлиентСервер.СообщенияПользователю(Истина, ИдентификаторЗадания);
	КонецПопытки;
...
Функция кнХозСубъект_ПолучитьGuidПоИНН_НаСервере(ВыбХозСубъект)
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
		                                                        "ВетИС_APIСервер.ХозСубъект_ПолучитьGuidПоИНН_ВФоне",
		                                                        ПараметрыФункции,
		                                                        НаименованиеЗадания);
		
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = ВетИС_APIСервер.ХозСубъект_ПолучитьGuidПоИНН(ПараметрыФункции, ВыбХозСубъект);
...
Функция кнЗагрузитьПлощадкиПоХозСубъектуНаСервере(ВыбХозСубъект)
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		//ПараметрыФункции = ВетИС_ПараметрыСервер.ПараметрыСервер( Объект );
		ПараметрыФункции["ВыбХозСубъект"] = ВыбХозСубъект;
		ПараметрыФункции["ЗагрузитьПлощадку"] = Истина;
		
		НаименованиеЗадания = НСтр("ru = 'Ветис запрос GUID ХозСубъекта'");
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
		                                                        "ВетИС_APIСервер.Площадка_ЗагрузитьПоХозСубъекту_ВФоне",
		                                                        ПараметрыФункции,
		                                                        НаименованиеЗадания);
		...
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = ВетИС_APIСервер.Площадка_ЗагрузитьПоХозСубъекту(ПараметрыФункции, ВыбХозСубъект, Истина);
		Результат = Новый Структура;
		Результат.Вставить("ЗаданиеВыполнено", Истина);
	КонецЕсли;
...
Функция ЗагрузитьПлощадкиСпискомПоХозСубъекту_Сервер()
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
		                                                        "ВетИС_APIСервер.Площадки_ЗагрузитьСписокПоХозСубъекту",
		                                                        ПараметрыФункции,
		                                                        НаименованиеЗадания);
		
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Ответ = ВетИС_APIСервер.Площадки_ЗагрузитьСписокПоХозСубъекту(ПараметрыФункции, "");
		Результат = Новый Структура;
		Результат.Вставить("ЗаданиеВыполнено", Истина);
	КонецЕсли;
...
Процедура ЗагрузитьПлощадкиСКонтролем2(Команда)
	...
	Если ВыбСтрока = Неопределено Или НЕ ВыбСтрока.Отметка Тогда
		Возврат;
	КонецЕсли;
	...
	Если ВыбСтрока <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыбСтрока.Хозсубъект) Тогда
			МассивХозсубъектов = ВетИС_APIСервер.ХозСубъекты_ЗагрузитьСписокПоИнн(ПараметрыОрганизации);
			...
			ИначеЕсли МассивХозсубъектов.Количество() = 1 Тогда
				...
				Иначе
					ВыбСтрока.ХозСубъект = ВетИС_ЯдроСервер.НайтиХозСубъект(ВыбСтрока.Контрагент);
					ВетИС_APIСервер.Площадка_ЗагрузитьПоХозСубъекту(ПараметрыОрганизации, ВыбСтрока.ХозСубъект, Истина);
					ОбновитьСписокРеализацийНаСервере();
				КонецЕсли;
			КонецЕсли;
		Иначе
...
Процедура ЗагрузитьВыбранныйХозсубъект(РезультатЗакрытия, ДополнительныеПараметры)
	...
	Если РезультатЗакрытия <> Неопределено И ДополнительныеПараметры <> Неопределено Тогда
		...
		ВетИС_APIСервер.ХозСубъект_ЗагрузитьПоGUID(ДополнительныеПараметры.ПараметрыОрганизации, ХозСубъектОбъект.GUID, ХозСубъектОбъект.Ссылка);
		ВетИС_APIСервер.Площадка_ЗагрузитьПоХозСубъекту(ДополнительныеПараметры.ПараметрыОрганизации, ХозсубъектОбъект.Ссылка, Истина);
		ОбновитьСписокРеализацийНаСервере();
	КонецЕсли;
...
&НаКлиенте
// @grace-contract: ПослеВыбораЗагруженногоХозСубъекта
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВыбораЗагруженногоХозСубъекта"
...
Функция ПолучитьПартии2_НаСервере( НачДата, КонДата, ТолькоАктуальныеПартии=Ложь )
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		Результат = ДлительныеОперации.ВыполнитьВФоне("ВетИС_APIСервер.Партии2_Запрос_Отправить", 
			ПараметрыФункции, 
			ПараметрыВыполнения);
			
	Иначе
		ВетИС_APIСервер.Партии2_Запрос_Отправить(ПараметрыФункции);
		Результат = Новый Структура;
...
&НаКлиенте
// @grace-contract: ЗавершитьЗагрузкуПартий
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ЗавершитьЗагрузкуПартий"
Процедура ЗавершитьЗагрузкуПартий(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ВывестиСообщенияФоновогоЗадания(Результат);
...
Функция ПолучитьПартии2_СИзменениями_НаСервере( НачДата, КонДата )
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		НаименованиеЗадания = НСтр("ru = 'Ветис загрузка партий с изменениями'");
		
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НаименованиеЗадания;
		
		Результат = ДлительныеОперации.ВыполнитьВФоне("ВетИС_APIСервер.Партии2_СИзменениями_Запрос_Отправить", 
			ПараметрыФункции, 
			ПараметрыВыполнения);
			
	Иначе
		ВетИС_APIСервер.Партии2_СИзменениями_Запрос_Отправить(ПараметрыФункции);
		Результат = Новый Структура;
		Результат.Вставить("Статус", "Выполнено");
	КонецЕсли;
...
&НаКлиенте
// @grace-contract: ПослеВводаДатыЗагрузкиПартии
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВводаДатыЗагрузкиПартии"
Процедура ПослеВводаДатыЗагрузкиПартии(ВыбДата, Параметры) Экспорт
	
	Если ЗначениеЗаполнено( ВыбДата ) Тогда
		
		РезультатЗапуска = ПолучитьПартии2_НаСервере(ВыбДата, ТекущаяДата());
		
		Если РезультатЗапуска <> Неопределено Тогда
			Если РезультатЗапуска.Свойство("ИдентификаторЗадания") Тогда
				
				ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
				ПараметрыОжидания.ТекстСообщения = "Выполняется загрузка партий из Меркурий...";
				
				ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершитьЗагрузкуПартий", ЭтаФорма);
...
Процедура ПолучитьПартииАктуальные(Команда)
	...
	РезультатЗапуска = ПолучитьПартии2_НаСервере(Дата("20000101"), ТекущаяДата(), Истина);
	
	Если РезультатЗапуска <> Неопределено Тогда
		Если РезультатЗапуска.Свойство("ИдентификаторЗадания") Тогда
			
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
			ПараметрыОжидания.ТекстСообщения = "Выполняется загрузка актуальных партий из Меркурий...";
...
&НаКлиенте
// @grace-contract: ПослеВводаДатыЗагрузкиПартииСИзменениями
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВводаДатыЗагрузкиПартииСИзменениями"
Процедура ПослеВводаДатыЗагрузкиПартииСИзменениями(ВыбДата, Параметры) Экспорт
	
	Если ЗначениеЗаполнено(ВыбДата) Тогда
		
		РезультатЗапуска = ПолучитьПартии2_СИзменениями_НаСервере(ВыбДата, ТекущаяДата());
		
		Если РезультатЗапуска <> Неопределено Тогда
			Если РезультатЗапуска.Свойство("ИдентификаторЗадания") Тогда
				
				ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
				ПараметрыОжидания.ТекстСообщения = "Выполняется загрузка измененных партий из Меркурий...";
...
&НаКлиенте
// @grace-contract: ВыборПериодаОтгрузки
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ВыборПериодаОтгрузки"
...
&НаКлиенте
// @grace-contract: ВыборПериодаПартий
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ВыборПериодаПартий"
...
Процедура кнОтметитьСОшибкойСервер()
	...
	Для Каждого стр Из Объект.Отгрузки Цикл
		...
		ПоследнийЗапрос = ВетИС_УтилитыКлиентСервер.НайтиПоследнийЗапрос(стр.ВСД);
...
Процедура УменьшитьАктуальныеПартииНаРаспределенные(тзАктуальныхПартий)
	...
	ВетИС_УправлениеФормойСервер.УменьшитьАктуальныеПартииНаРаспределенные(тзАктуальныхПартий, ТаблицаОтгрузок);
...
&НаКлиенте
// @grace-contract: ПослеВводаДатыОтложеннойОтправки
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВводаДатыОтложеннойОтправки"
...
Процедура ЗаполнитьТаблицуПартийНаСервере(Знач ЗнакСортировки = Неопределено)
	...
	ТаблицаОтгрузок = Объект.Отгрузки.Выгрузить();
	РезультатПодготовки = ВетИС_УправлениеФормойСервер.ПодготовитьТребуемыеПартии(
		ТаблицаОтгрузок, 
	...
	АктуальныеПартии = ВетИС_ЯдроСервер.ПолучитьАктуальныеПартии(
		ПараметрыОрганизации, 
	...
	// LLM_NOTE: При включённом КонтролироватьРезервыПартий этот шаг избыточен —
	//   ПолучитьАктуальныеПартии уже учла зарезервированные количества через
	//   LEFT JOIN к кб99_РезервПартийВСД. Двойное вычитание приведёт к занижению
	//   доступного остатка вдвое.
	Если НЕ ВетИС_ПараметрыСервер.ПроверитьЗначениеПараметра(ПараметрыОрганизации, "КонтролироватьРезервыПартий")
		ИЛИ НЕ ПараметрыОрганизации["КонтролироватьРезервыПартий"] Тогда
		УменьшитьАктуальныеПартииНаРаспределенные(АктуальныеПартии);
	КонецЕсли;
	
	// Распределение партий
	РаспределенныеПартии = ВетИС_УправлениеФормойСервер.РаспределитьПартииПоОстаткам(
		ТребуемыеПартии, 
		АктуальныеПартии, 
		ПараметрыОрганизации
	);
...
Процедура ПроверитьВозможностьПеремещенияВСДПоСписку()
	...
	Для Каждого строкаТЗ Из Объект.Отгрузки Цикл
		...
		КонецЕсли;		
		Попытка
			ВетИС_APIСервер.ВСД2_Транзакция_УсловияПеревозки_Отправить( ПараметрыОрганизации, ВыбВСД);
		Исключение
			ВетИС_ЯдроСервер.СообщитьИнфо(ОписаниеОшибки(), ВыбВСД);
		КонецПопытки;
	КонецЦикла;
	ВетИС_ЯдроСервер.СообщитьИнфо("-> Проверка условий перевозки Завершена ");
...
Процедура ПроверитьУсловияПеревозкиПередОтправкой(СписокДокументов, ПараметрыФункции)
	...
	Для Каждого ДокСсылка Из ДокументыДляПроверки Цикл
		ВетИС_APIСервер.ВСД2_Транзакция_УсловияПеревозки_Отправить(ПараметрыФункции, ДокСсылка, Ложь);	
	КонецЦикла;
	
	// Пауза для обработки на стороне Меркурий
	ВетИС_APIСервер.Пауза(ПараметрыФункции["ПаузаСек"]);
	
	// Получаем результаты проверки
	Для Каждого ДокСсылка Из ДокументыДляПроверки Цикл
		ИдентификаторЗаявки = ВетИС_УтилитыКлиентСервер.НайтиApplicationID(ДокСсылка);
		Если ЗначениеЗаполнено(ИдентификаторЗаявки) Тогда				
			ВетИС_APIСервер.ВСД2_Транзакция_УсловияПеревозки_ПолучитьОтвет(ПараметрыФункции, ИдентификаторЗаявки, ДокСсылка, Ложь);
		КонецЕсли;
	КонецЦикла;
...
Функция ОтправитьВСД_НаСервере()
	...
	Если Объект.ОтправлятьВФоне Тогда 
		...
		Если ПараметрыФункции.ОтправлятьДокументыПакетно Тогда 
			Результат = ДлительныеОперации.ВыполнитьВФоне("ВетИС_ЯдроСервер.ОтправитьВСД_Параллельно", 
			ПараметрыФункции,
			ПараметрыВыполнения);
		Иначе
	...
	Иначе
		...
		Если ПараметрыФункции.ОтправлятьДокументыПакетно Тогда 
			ВетИС_ЯдроСервер.ОтправитьВСД_Параллельно(ПараметрыФункции, Неопределено);
		Иначе
			...
			Для Каждого ДокСсылка Из СписокДокументовКОтправке Цикл
				ВетИС_ЯдроСервер.ОтправитьВСДвГИС(ДокСсылка, Неопределено);
			КонецЦикла;
		КонецЕсли;
...
Функция ПодготовитьСписокВСДКОтправке(ОтправлятьРанееОтправленные)
	...
	Для Каждого СтрОтгрузка Из Объект.Отгрузки Цикл
		...
		ИдентификаторЗаявки = ВетИС_УтилитыКлиентСервер.НайтиApplicationID(ВСДДокумент);
...
//&НаКлиенте
// @grace-contract: СписокОтправкаВСДОбработатьВыбор
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="СписокОтправкаВСДОбработатьВыбор"
...
//	КонецЕсли;	
//	
//	РезультатЗапуска = ОтправитьВСД_НаСервере( ВыбЭлемент );	
//	
//	Если РезультатЗапуска <> Неопределено Тогда
//		Если РезультатЗапуска.Свойство("ИдентификаторЗадания") Тогда
//			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
...
//			
//			ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершитьОтправкуВСД", ЭтаФорма);
//			ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатЗапуска, ОповещениеОЗавершении, ПараметрыОжидания);
//		Иначе
//			ЗаполнитьТабличныеЧасти();
//		КонецЕсли;
...
&НаКлиенте
Процедура ОтправитьВсд(Команда)
	
	РезультатЗапуска = ОтправитьВСД_НаСервере();	
...
&НаСервере
Функция ПолучитьСообщенияФоновогоЗадания(Знач ИдЗадания)
	
	Если Не ЗначениеЗаполнено(ИдЗадания) Тогда
		Возврат Новый Массив;
	КонецЕсли;
	
	Попытка
		// Используем стандартный метод БСП для получения сообщений
		Сообщения = ДлительныеОперации.СообщенияПользователю(Истина, ИдЗадания);
	Исключение
		// Если БСП старая или метод отсутствует, пробуем через платформенный метод
...
&НаКлиенте
Процедура ВывестиСообщенияФоновогоЗадания(Результат)
	
	Если Результат = Неопределено Или Не Результат.Свойство("ИдентификаторЗадания") Тогда
		Возврат;
	КонецЕсли;
	
	МассивСообщений = ПолучитьСообщенияФоновогоЗадания(Результат.ИдентификаторЗадания);
	
	Для Каждого Сообщение Из МассивСообщений Цикл
		// Выводим сообщение пользователю в стандартное окно сообщений
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение.Текст, , , , Сообщение.ИдентификаторНазначения);
	КонецЦикла;
...
Процедура кнПолучитьОтветВСД2НаСервере()
	...
	Для Каждого строкаОтгрузки Из Объект.Отгрузки Цикл
		Если строкаОтгрузки.Отметка И ЗначениеЗаполнено(строкаОтгрузки.ВСД) Тогда 
			...
			applicationID = ВетИС_УтилитыКлиентСервер.НайтиApplicationID( ВыбВСД );
			...
			ВетИС_APIСервер.ПолучитьРезультат_ВСД2( ПараметрыФункции );
		КонецЕсли;
	КонецЦикла;
...
Процедура кнПолучитьОтветВСД2(Команда)
	...
	ВетИС_ЯдроСервер.СообщитьИнфо("Проверка ответов завершена");
...
Процедура кнАннулироватьВыбранныеВСДНаСервере()
	...
	Для Каждого строкаОтгрузки Из Объект.Отгрузки Цикл
		...
		ВетИС_APIСервер.ВСД2_транзакция_Аннулировать( ПараметрыФункции, строкаОтгрузки.ВСД);
	КонецЦикла;
...
&НаКлиенте
// @grace-contract: кнАннулироватьВыбранныеВСДОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнАннулироватьВыбранныеВСДОтвет"
...
Процедура кнПечатьСтрОтгрузки(Команда)
	...
	Для Каждого стрДок Из Объект.Отгрузки Цикл
		Если Стрдок.Отметка Тогда
			ТабДок = ВетИС_ЯдроСервер.ПечатьВнешнейФормыВСД( СтрДок.ВСД );
...
Функция ПечатьРеестра()
	...
	Для Каждого стр Из Объект.Отгрузки Цикл				
		...
		Если Объект.РеквизитГрузополучатель = 0 Тогда     
			...
			МодульУправлениеКонтактнойИнформацией = ВетИС_УтилитыКлиентСервер.ОбщийМодуль("УправлениеКонтактнойИнформацией");
...
&НаКлиенте
// @grace-contract: ОтгрузкиВСДОчисткаОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ОтгрузкиВСДОчисткаОтвет"
...
Процедура ОтгрузкиВСДОчистка(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элемент.Родитель.ТекущиеДанные.ВСД) Тогда 
		...
		Если ЗначениеЗаполнено(ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(ДокСсылка,"СтатусВСД")) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо(""+ДокСсылка+" зарегистрирован в Меркурий - удаление невозможно");
			Возврат;
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ОтгрузкиПлощадкаПриИзмененииНаСервере( ПлощадкаСсылка, НовыйКонтрагент, ВидДокументаОтгрузка )
	
	Если НЕ (ПлощадкаСсылка = Справочники.ВСД_Площадка.ПустаяСсылка()) Тогда 
		СпрОб = ПлощадкаСсылка.ПолучитьОбъект();
		Если ВидДокументаОтгрузка = 2 Тогда
...
Процедура ОтгрузкиПлощадкаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено( ОчиститьПлощадку ) Тогда 
		ОтгрузкиПлощадкаПриИзмененииНаСервере( ОчиститьПлощадку, Неопределено, ВидДокументаОтгрузка);
	...
	ОтгрузкиПлощадкаПриИзмененииНаСервере( Элемент.Родитель.ТекущиеДанные.Площадка, НовыйГрузополучатель, ВидДокументаОтгрузка );
	...
	Для Каждого СтрОтгрузки Из Объект.Отгрузки Цикл
		...
		Если ЗначениеЗаполнено(СтрОтгрузки.ВСД) Тогда
			Если СтрОтгрузки.ВСД.Проведен Тогда
				ВетИС_ЯдроСервер.СообщитьИнфо("Для "+СтрОтгрузки.Док+" уже отправлен "+СтрОтгрузки.ВСД);
				Продолжить;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(СтрОтгрузки.ВСД.СтатусВСД) Тогда 
				ВетИС_ЯдроСервер.СообщитьИнфо("Для "+СтрОтгрузки.Док+" ВСД уже создан, но не отправлен");
				Продолжить;
			КонецЕсли;		
		КонецЕсли;		
		
		Если НЕ(ЗначениеЗаполнено(СтрОтгрузки.Площадка)) или НЕ(ЗначениеЗаполнено(СтрОтгрузки.ХозСубъект)) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("Для "+СтрОтгрузки.Док+" неизвестен ХС или Площадка получателя ");
			Продолжить;
		КонецЕсли;
		...
		ВетИС_ЯдроСервер.СообщитьИнфо("["+СтрОтгрузки.Грузополучатель+"] создан документ "+СтрОтгрузки.ВСД, СтрОтгрузки.ВСД);		
...
Процедура СоздатьВсдПоТермУсловиямПеревозки() 
	...
	Для Каждого стрТермУсловие Из тзПартииПоТермУсловиям Цикл
		тзАктуальныхПартий = Объект.Партии.Выгрузить();
		Для Каждого СтрОтгрузки Из Объект.Отгрузки Цикл
			Если НЕ(СтрОтгрузки.Отметка) ИЛИ ЗначениеЗаполнено(СтрОтгрузки.ВСД) Тогда
				Продолжить;
			КонецЕсли;
			...
			ВетИС_ЯдроСервер.СообщитьИнфо("["+СтрОтгрузки.Грузополучатель+"] создан документ "+ДокВСД+" по термическим условиям: "+стрТермУсловие.ТермическиеУсловияПеревозки, ДокВСД );		
		КонецЦикла;								
	КонецЦикла;
...
&НаКлиенте
// @grace-contract: кнСоздатьВСДОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнСоздатьВСДОтвет"
...
Процедура кнСоздатьВСД(Команда)
	...
	Если Объект.Партии.Количество() = 0 Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет Партий для создания ВСД ");
		Возврат;
	КонецЕсли;
...
Процедура ОформитьПроизводственнуюТранзакциюПострочно(ВыбДата, ПараметрыОрганизации)
	...
	Для Каждого стрПартий Из Объект.Партии Цикл
		Если НЕ(ЗначениеЗаполнено(стрПартий.Продукция_Элемент)) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("В строке партий № "+стрПартий.НомерСтроки+"  не указан ВСД_Продукция_Элемент. Пропускаем...");
			Продолжить;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(стрПартий.Партия) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("В строке партий № "+стрПартий.НомерСтроки+" указана партия. Пропускаем...");
			Продолжить;
		КонецЕсли;
		...
		Если НЕ(ЗначениеЗаполнено(стрПартий.Продукция_Элемент.ЕдиницаИзмерения)) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("В строке партий № "+стрПартий.НомерСтроки+" у "+СокрЛП(стрПартий.Продукция_Элемент.Наименование)+" не указана Единица измерения. Пропускаем...");
			Продолжить;
		КонецЕсли;
		
		Если НЕ(ЗначениеЗаполнено(стрПартий.Продукция_Элемент.СрокГодности)) Тогда
			ВетИС_ЯдроСервер.СообщитьИнфо("В строке партий № "+стрПартий.НомерСтроки+" у "+СокрЛП(стрПартий.Продукция_Элемент.Наименование)+" не указан Срок годности. Пропускаем...");
			Продолжить;
		КонецЕсли;
		...
		Если Не ДокВСД.ЗавершитьОперацию И Не ЗначениеЗаполнено(ДокВСД.operationId) Тогда
			ДокВСД.operationId = ПараметрыОрганизации.ПрефиксНомераНезавершенногоПроизводства+"-"+ВетИС_APIСервер.ДатаXML(ВыбДата);	
		КонецЕсли;
		...
		ВетИС_ЯдроСервер.СообщитьИнфо(" создан документ "+ДокВСД.Ссылка, ДокВСД.Ссылка);		
	КонецЦикла;
	РаскраситьТЧПартий();
...
Процедура ОформитьПроизводственнуюТранзакциюОбъединенно(ВыбДата, ПараметрыОрганизации)
	...
	тзПартии = Объект.Партии.Выгрузить();
	тзПартии.Свернуть("Продукция_Элемент, ВСД_Производство, Партия", "КоличествоСписания, Количество, Количество1с");
...
&НаКлиенте
// @grace-contract: ПослеВводаДатыПроизводство
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВводаДатыПроизводство"
...
Процедура ОформитьПроизводственнуюТранзакциюПоВидамПродукции(ВыбДата, ВидыПродукции)
	...
	Для Каждого ВидПродукции Из ВидыПродукции Цикл
		...
		ПартииПоВидуПродукции = тзПартии.Скопировать(Новый Структура("ВидПродукции, Партия", ВидПродукции, Справочники.ВСД_Партия.ПустаяСсылка()));
		ПартииПоВидуПродукции.Свернуть("Продукция_Элемент, ВСД_Производство, Партия, ВидПродукции","КоличествоСписания, Количество, Количество1с");
		Если Не ПартииПоВидуПродукции.Количество() Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого стрПартий Из Объект.Партии Цикл
			...
			КонецЕсли
		КонецЦикла;
		ВетИС_ЯдроСервер.СообщитьИнфо("Создан документ "+ДокВСД.Ссылка+" по виду продукции: "+ВидПродукции, ДокВСД.Ссылка);
	КонецЦикла;
	РаскраситьТЧПартий();
...
&НаКлиенте
// @grace-contract: кнПроизводствоОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнПроизводствоОтвет"
...
Процедура кнПроизводство(Команда)
	...
	Если Объект.Партии.Количество() = 0 Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет данных в таблице ");
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьВСД2_ПроизводствоНаСервере( СписокВСД )
	...
	ВетИС_ЯдроСервер.СообщитьИнфо("Начало отправки документов");
	...
	Для Каждого ДокВСД Из СписокВСД Цикл
		...
		ВетИС_ЯдроСервер.ОтправитьВСДвГИС(ВСДСсылка, Неопределено);
		
	КонецЦикла;
	ОбновитьТчПартииСписанияПослеОтправкиПроизводства(ПараметрыОрганизации);
	ВетИС_ЯдроСервер.СообщитьИнфо("Отправка документов завершена");
...
Процедура ПроизводствоОтправить(Команда)
	...
	Если СзДок.Количество() = 0 Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет производственных ВСД для отправки");
		Возврат;
	КонецЕсли;
...
Процедура кнПолучитьОтветВСДПроизводство(Команда)
	...
	Для Каждого строкаТЗ Из Объект.Партии Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыбВСД) Тогда
			Продолжить;	
		КонецЕсли;
		_applicationID = ВетИС_УтилитыКлиентСервер.НайтиApplicationID( ВыбВСД );
		Если (НЕ ЗначениеЗаполнено(_applicationID)) Тогда
			Продолжить;	
		КонецЕсли;
		сзДокументыКОтправке.Добавить(ВыбВСД);
		ВетИС_ЯдроСервер.СообщитьИнфо(""+ВыбВСД+" Добавлен в очередь на проверку ответа Меркурий");
	КонецЦикла;
	...
	Если сзДокументыКОтправке.Количество() > 0 Тогда
		...
		ПоказатьОповещениеПользователя("Выполнено");
	Иначе
		ВетИС_ЯдроСервер.СообщитьИнфо("Не выбрано документов к проверке");
	КонецЕсли;
...
&НаСервере
Процедура РаскраситьТЧОтгрузки_СтарыйВариант()
	
	Для Каждого ДанныеСтроки Из Объект.Отгрузки Цикл
		
		Если ЗначениеЗаполнено(ДанныеСтроки.ВСД) Тогда
			Запрос = ВетИС_УтилитыКлиентСервер.НайтиПоследнийЗапрос( ДанныеСтроки.ВСД );
		...
		Иначе
			СтатусВСД = "";
			СтатусЗапроса = "";
			СтатусЗапросаПеречисление = Перечисления.кб99_СтатусЗапроса.ПустаяСсылка();
		КонецЕсли;
...
Процедура РаскраситьТЧОтгрузки()
  ...
  Для Каждого ДанныеСтроки Из Объект.Отгрузки Цикл
  	...
  	// Статусы теперь берутся напрямую из строки ТЧ, без запросов в цикле!
  	СтатусЗапроса = ДанныеСтроки.СтатусЗапроса;
  	СтатусВСД = ДанныеСтроки.СтатусВСД;
  	Если ПустаяСтрока(ДанныеСтроки.ХозСубъект.GUID) Тогда
  		ДанныеСтроки.Сцвет = 1;   // красный
  	ИначеЕсли ДанныеСтроки.Площадка = Справочники.ВСД_Площадка.ПустаяСсылка() Тогда
  	...
  	ИначеЕсли СтатусВСД = Перечисления.кб99_СтатусВСД.WITHDRAWN Тогда 
  		ДанныеСтроки.Сцвет = 4;   
  	ИначеЕсли СтатусЗапроса = Перечисления.кб99_СтатусЗапроса.REJECTED Тогда
  		ДанныеСтроки.Сцвет = 4;   
  	ИначеЕсли СтатусЗапроса = Перечисления.кб99_СтатусЗапроса.IN_PROCESS Тогда
  		ДанныеСтроки.Сцвет = 5; 
  	КонецЕсли;
...
//// Автозаполнение
&НаСервере
Функция ЗаполнитьТабличныеЧасти()
	
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьТабличныеЧасти");
	Если ПереопределенныйМодуль <> Неопределено Тогда		
		ПереопределенныйМодуль.ЗаполнитьТабличныеЧасти( ВидДокументаОтгрузка );
	КонецЕсли;	
	...
	ПараметрыФункции.Вставить("СкрытьОформленные", Ложь);
	
	Если ВетИС_ПараметрыСервер.ПроверитьЗначениеПараметра(СписокКонстант, "НомерАвто") Тогда
		ПараметрыФункции.Вставить("НомерАвто", СписокКонстант.НомерАвто);    
	Иначе
		ПараметрыФункции.Вставить("НомерАвто", """""");    
	КонецЕсли;
	
	Если ВетИС_ПараметрыСервер.ПроверитьЗначениеПараметра(СписокКонстант, "СвойКонтрагент", "Свой контрагент") Тогда
	...
	Если ВидДокументаОтгрузка = 2 Тогда // Перемещения
		РезультатЗапроса = ВетИС_ДокументооборотСервер.СформироватьЗапросПеремещения( ПараметрыФункции );
	ИначеЕсли ВидДокументаОтгрузка = 1 Тогда // Заказы
		РезультатЗапроса = ВетИС_ДокументооборотСервер.СформироватьЗапросЗаказы( ПараметрыФункции );
	ИначеЕсли ВидДокументаОтгрузка = 3 Тогда // Расходные ордера
		РезультатЗапроса = ВетИС_ДокументооборотСервер.ПолучитьТзРасходныхОрдеров( ПараметрыФункции );
	Иначе // Реализации (ВидДокументаОтгрузка = 0 или по умолчанию)
		РезультатЗапроса = ВетИС_ДокументооборотСервер.ПолучитьТзРеализаций( ПараметрыФункции );
	КонецЕсли;
	...
	// Применяем фильтр по статусу ВСД через запрос
	// 1 Все
	// 2 В работе
	// 3 Завершенные
	// 4 С ошибками
	Если ФильтрСтатусВСД <> 1 Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|    Данные.Док КАК Док,
		|    Данные.ДокПредставление КАК ДокПредставление,
		|    Данные.Контрагент КАК Контрагент,
		|    Данные.Грузополучатель КАК Грузополучатель,
		|    Данные.ХозСубъект КАК ХозСубъект,
		|    Данные.Площадка КАК Площадка,
		|    Данные.ВСД КАК ВСД,
		|    Данные.ВСДПредставление КАК ВСДПредставление,
		|    Данные.СтатусВСД КАК СтатусВСД,
		|    Данные.СтатусЗапроса КАК СтатусЗапроса,
		|    Данные.Ошибка КАК Ошибка,
		|    Данные.Количество КАК Количество,
		|    Данные.НомерАвто КАК НомерАвто,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|    втДанные.Док КАК Док,
		|    втДанные.ДокПредставление КАК ДокПредставление,
		|    втДанные.Контрагент КАК Контрагент,
		|    втДанные.Грузополучатель КАК Грузополучатель,
		|    втДанные.ХозСубъект КАК ХозСубъект,
		|    втДанные.Площадка КАК Площадка,
		|    втДанные.ВСД КАК ВСД,
		|    втДанные.ВСДПредставление КАК ВСДПредставление,
		|    втДанные.СтатусВСД КАК СтатусВСД,
		|    втДанные.СтатусЗапроса КАК СтатусЗапроса,
		|    втДанные.Ошибка КАК Ошибка,
		|    втДанные.Количество КАК Количество,
		|    втДанные.НомерАвто КАК НомерАвто,
		|    втДанные.СтатусВСД КАК СтатусВСДИзДокумента,
		...
		Запрос.УстановитьПараметр("Данные", РезультатЗапроса.Выгрузить());
		
		Если ФильтрСтатусВСД = 2 Тогда
			// В работе: нет ВСД ИЛИ статус не завершенный
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеФильтра",
			"втДанные.ВСД ЕСТЬ NULL
			|    ИЛИ НЕ втДанные.СтатусВСД В (ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.CONFIRMED), 
			|                                         ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.WITHDRAWN),
			|                                         ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.UTILIZED))");
		ИначеЕсли ФильтрСтатусВСД = 3 Тогда
			// Завершенные: есть ВСД со статусом CONFIRMED, WITHDRAWN или UTILIZED
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеФильтра",
			"втДанные.СтатусВСД В (ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.CONFIRMED), 
			|                              ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.WITHDRAWN),
			|                              ЗНАЧЕНИЕ(Перечисление.кб99_СтатусВСД.UTILIZED))");
		ИначеЕсли ФильтрСтатусВСД = 4 Тогда
			// С ошибками: есть ВСД со статусом REJECTED
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеФильтра",
			"втДанные.СтатусЗапроса = ЗНАЧЕНИЕ(Перечисление.кб99_СтатусЗапроса.REJECTED)");
		КонецЕсли;
	...
	Элементы.ОтгрузкиОшибка.Видимость = ФильтрСтатусВСД = 4;
...
&НаКлиенте
Процедура ВидДокументаОтгрузкаПриИзменении(Элемент)
	
	УстановитьЗаголовокКолонкиДокумента();
	ЗаполнитьТабличныеЧасти();
...
Процедура ПолучитьАктуальнуюВерсиюИзХранилища()
	...
	ВерсияОбработки  = ВетИС_УтилитыКлиентСервер.ОписаниеВерсии();
...
&НаКлиенте
// @grace-contract: ПолучитьИнфоормациюОбОбновленииЗавершение
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПолучитьИнфоормациюОбОбновленииЗавершение"
...
&НаКлиенте
// @grace-contract: СоздатьИнвентаризацию
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="СоздатьИнвентаризацию"
...
Функция ЗаполнитьТабЧастьЭлементовДляСозданияВСД( )
	...
	РезТаб.Колонки.Добавить("Количество1с", Новый ОписаниеТипов("Число"));
	...
	Для Каждого стрПартии Из Объект.Партии Цикл
		...
		стрТаб.Количество1с = стрПартии.Количество1с;
...
Функция ВСД_ПеремещениеСРазделениемПоТермическимУсловиям()
	...
	Для Каждого стрТермУсловие Из тзПартииПоТермУсловиям Цикл
		...
		СписокКонстант = ВетИС_ЯдроСервер.ЗагрузитьПараметры( Объект.ПарамПоставщикОрганизация );
		тзАктуальныхПартий = ВетИС_ЯдроСервер.ПолучитьАктуальныеПартии(СписокКонстант, тзТовары.ВыгрузитьКолонку("Продукция_Элемент"), Объект.ПарамПоставщикПлощадка, Объект.ПарамПоставщикХозСубъект,,,тзТовары.ВыгрузитьКолонку("Номенклатура") );
		ПараметрыЗаполнения.Вставить("тзАктуальныхПартий", тзАктуальныхПартий);	
...
&НаКлиенте
// @grace-contract: ВнутреннееПеремещение
//   Вход: см. сигнатуру
//   Выход: Произвольный — результат
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ВнутреннееПеремещение"
...
&НаКлиенте
// @grace-contract: кнПереместитьОтПоставщикаОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнПереместитьОтПоставщикаОтвет"
...
&НаКлиенте
// @grace-contract: кнОформитьЛабИсследованияОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="кнОформитьЛабИсследованияОтвет"
Процедура кнОформитьЛабИсследованияОтвет(Ответ,Парам) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			ВетИС_ЯдроСервер.СообщитьИнфо("Нет производственных ВСД для создания лабораторных исследований или документ не оформлен!");
		КонецЕсли;	
	КонецЕсли;
...
Процедура кнОформитьЛабИсследования(Команда)
	...
	Если Объект.Партии.Количество() = 0 Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Нет Партий для оформления лабораторных исследований. ");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьСписокДокументов(Элемент)
	...
	Состояние("Обновление списка документов",, "Начало в "+ТекущаяДата()+" Подождите...", БиблиотекаКартинок.kb99_clock_24x24);	
	ЗаполнитьТабличныеЧасти();
...
&НаКлиенте
// @grace-contract: СтруктураОтбораПартий
//   Вход: см. сигнатуру
//   Выход: Произвольный — результат
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="СтруктураОтбораПартий"
...
Процедура ПартииПартияПриИзменении(Элемент)
	...
	Стр.ПолеСортировки = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(Стр.Партия,ПолеСортировки);
...
Процедура ОбновитьТчПартииСписанияПослеОтправкиПроизводства (ПараметрыОрганизации)
	...
	тзАктуальныхПартий = ВетИС_ЯдроСервер.ПолучитьПартииПоДокументамПроизводства(СписокДокументовПроизводство, Объект.ПарамКолонкаСортировкиПартииСписания);
	...
	Для Каждого Стр Из Объект.Партии Цикл
		Если ЗначениеЗаполнено(Стр.ВСД_Производство) Тогда
			СтрокиПартий = ВетИС_ЯдроСервер.ПодобратьПартииПоПродукцияЭлемент( ПараметрыОрганизации, тзАктуальныхПартий, Стр.Продукция_Элемент, Стр.КоличествоСписания );	
...
Процедура кнПечатьНаборныйЛист(Команда)
	...
	Для Каждого стрДок Из Объект.Отгрузки Цикл
		Если Стрдок.Отметка Тогда
			ТабДок = ВетИС_ЯдроСервер.ПечатьНаборныйЛист( СтрДок.ВСД );
...
&НаКлиенте
Процедура ПроизводствоПоВидамПродукции(Команда)
...
&НаКлиенте
// @grace-contract: ПослеВыбораВидовПродукции
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ПослеВыбораВидовПродукции"
...
Процедура ПрикрепитьВСДВоВложенияЕИС(Команда)
	...
	Для Каждого стрДок Из Объект.Отгрузки Цикл
		...
		Для Каждого Файл Из ПрисоединенныеФайлыМассив Цикл
			...
			ВетИС_УтилитыКлиентСервер.СообщенияПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Прикреплен файл %1 к документу %2", ВремФайл, стрДок.Док));
		КонецЦикла;
...
&НаКлиенте
Процедура ДатаОкончанияОтбораПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(Объект.ДатаОкончанияОтбора) Тогда
		Объект.ДатаОкончанияОтбора = КонецДня(Объект.ДатаОкончанияОтбора);
	КонецЕсли;
	
	ОбновитьСписокДокументов(Неопределено);
	
КонецПроцедуры

&НаКлиенте
Процедура ФильтрСтатусВСДПриИзменении(Элемент)
...
&НаКлиенте
Процедура УстановитьЗаголовокКнопкиОтмены()
	
	// 1 Все
	// 2 В работе
	// 3 Завершенные
	// 4 С ошибками
	Если ФильтрСтатусВСД = 2 Или ФильтрСтатусВСД = 4 Тогда
		ТекстЗаголовка = "Отменить документы";
	Иначе
		ТекстЗаголовка = "Аннулировать ВСД";
	КонецЕсли;
	
	Элементы.кнОтменитьДокументы.Заголовок = ТекстЗаголовка;
	
КонецПроцедуры

&НаСервере
Функция ОтменитьДокументыНаСервере()
	
	СписокВСД = Новый Массив;
	Для Каждого Стр Из Объект.Отгрузки Цикл
		Если Стр.Отметка И ЗначениеЗаполнено(Стр.ВСД) Тогда
			СписокВСД.Добавить(Стр.ВСД);
		КонецЕсли;
	КонецЦикла;
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = "Аннулирование ВСД (пакетно)";
		
		Возврат ДлительныеОперации.ВыполнитьВФоне("ВетИС_APIСервер.АннулироватьСписокВСД_Фон", ПараметрыПроцедуры, ПараметрыВыполнения);
		
	Иначе
		
		ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(Объект.Организация);
		ВетИС_APIСервер.АннулироватьСписокВСД(ПараметрыОрганизации, СписокВСД);
...
&НаКлиенте
// @grace-contract: ОтменитьДокументыОтвет
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ОтменитьДокументыОтвет"
Процедура ОтменитьДокументыОтвет(Ответ, Парам) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		
		РезультатЗапуска = ОтменитьДокументыНаСервере();
		
		Если РезультатЗапуска <> Неопределено Тогда
			Если РезультатЗапуска.Свойство("ИдентификаторЗадания") Тогда
				
				ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтаФорма);
				ПараметрыОжидания.ТекстСообщения = "Выполняется аннулирование ВСД...";
				
				ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершитьОтменуВСД", ЭтаФорма);
				ДлительныеОперацииКлиент.ОжидатьЗавершение(РезультатЗапуска, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаКлиенте
// @grace-contract: ЗавершитьОтменуВСД
//   Вход: см. сигнатуру
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
// GRACE:CONTRACT function="ЗавершитьОтменуВСД"
Процедура ЗавершитьОтменуВСД(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура кнОтменитьДокументы(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ОтменитьДокументыОтвет", ЭтаФорма);
    ПоказатьВопрос(Оповещение, "Отменить отмеченные документы?", РежимДиалогаВопрос.ДаНет, 0, КодВозвратаДиалога.Да, "");
...
Процедура УстановитьУсловноеОформление()
	...
	ДобавитьЭлементУсловногоОформления("Объект.Отгрузки.сЦвет", 2, Новый Цвет(255, 255, 153), "Отгрузки");
	ДобавитьЭлементУсловногоОформления("Объект.Отгрузки.сЦвет", 1, Новый Цвет(255, 204, 153), "Отгрузки");
	ДобавитьЭлементУсловногоОформления("Объект.Отгрузки.сЦвет", 3, Новый Цвет(170, 255, 160), "Отгрузки");
	ДобавитьЭлементУсловногоОформления("Объект.Отгрузки.сЦвет", 4, Новый Цвет(255, 99, 70), "Отгрузки");
	ДобавитьЭлементУсловногоОформления("Объект.Отгрузки.сЦвет", 5, Новый Цвет(255, 204, 0), "Отгрузки");
		
	ДобавитьЭлементУсловногоОформления("Объект.Партии.сЦвет", 1, Новый Цвет(250, 128, 115), "Партии");
	ДобавитьЭлементУсловногоОформления("Объект.Партии.сЦвет", 2, Новый Цвет(255, 255, 153), "Партии");
	ДобавитьЭлементУсловногоОформления("Объект.Партии.сЦвет", 3, Новый Цвет(170, 255, 160), "Партии");
...
Процедура НастроитьВидимостьЭлементовФормы()
	...
	ИспользуютсяРасходныеОрдера = 
			ВетИС_УтилитыКлиентСервер.ОпределитьТипОбъектаМетаданных("РасходныйОрдер").ТипДокумента <> Неопределено;
		
	ПереопределяемыеПоля = ВетИС_ДокументооборотСервер.ПолучитьПредставлениеПолейДокументаВЗапрос();
	
	// Заполнение списка выбора для ВидДокументаОтгрузка
	СписокВыбора = Элементы.ВидДокументаОтгрузка.СписокВыбора;
	СписокВыбора.Очистить();
	
	// Реализация всегда присутствует
	СписокВыбора.Добавить(0, "Реализации");
	
	Если Метаданные.Документы.Найти(ПереопределяемыеПоля.ТекстПоляИсточникЗаказ) <> Неопределено Тогда
		СписокВыбора.Добавить(1, "Заказы");
	КонецЕсли;
	
	Если Метаданные.Документы.Найти(ПереопределяемыеПоля.ТекстПоляИсточникПеремещение) <> Неопределено Тогда
		СписокВыбора.Добавить(2, "Перемещения");
	КонецЕсли;
	
	Если ИспользуютсяРасходныеОрдера Тогда
		СписокВыбора.Добавить(3, "Расходные ордера");
	КонецЕсли;
...
&НаСервере
Процедура УстановитьЗаголовокКолонкиДокумента()
	
	Если ВидДокументаОтгрузка = 1 Тогда
		ТекстЗаголовкаКолонки = "Заказ";
		ТекстЗаголовкаСтраницы = "Заказы";
	ИначеЕсли ВидДокументаОтгрузка = 2 Тогда
		ТекстЗаголовкаКолонки = "Перемещение";
		ТекстЗаголовкаСтраницы = "Перемещения";
	ИначеЕсли ВидДокументаОтгрузка = 3 Тогда
		ТекстЗаголовкаКолонки = "Расходный ордер";
		ТекстЗаголовкаСтраницы = "Расходные ордера";
	Иначе
		ТекстЗаголовкаКолонки = "Реализация";
		ТекстЗаголовкаСтраницы = "Реализации";
	КонецЕсли;
	
	Элементы.ОтгрузкиДокПредставление.Заголовок = ТекстЗаголовкаКолонки;
	Элементы.СтраницаОтгрузки.Заголовок = ТекстЗаголовкаСтраницы;
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьЭлементУсловногоОформления(ИмяПоля, ЗначениеДляСравнения, ЦветФона, ОформляемаяТЧ)

	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	
	Для Каждого Элемент Из Элементы[ОформляемаяТЧ].ПодчиненныеЭлементы Цикл
		ПолеОформления = ЭлементОформления.Поля.Элементы.Добавить();
		ПолеОформления.Поле = Новый ПолеКомпоновкиДанных(Элемент.Имя);
