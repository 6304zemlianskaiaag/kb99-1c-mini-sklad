...
// @grace-purpose: Форма кб99_ЛабИсследования — ввод лабораторных исследований по партиям ВетИС
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОрганизации.param_vetdoctor_login) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("У вас не достаточно прав для оформления лабораторных исследований");
		Отказ = Истина;
	ИначеЕсли ЗначениеЗаполнено(Объект.ВыбДокПроизводства) И Не ЗначениеЗаполнено(Объект.Партии) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("По документу %1 не требуется оформление лабораторных исследований."+
...
Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	...
	Исключение
		...
		ВетИС_ЯдроСервер.СообщитьИнфо("Указан неверный параметр " + Описаниеошибки());
	КонецПопытки;
...
Функция ОтправитьВСДПакетно_НаСервере()
	...
	Если Объект.ОтправлятьВФоне Тогда
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
		                                                        "ВетИС_APIСервер.ВСД2_ЛабораторныеИсследования_ОтправитьПараллельно",
		                                                        ПараметрыФункции,
		                                                        НаименованиеЗадания);
		// результат обработки
		АдресХранилища = Результат.АдресХранилища;
		...
		ИдентификаторЗадания = Результат.ИдентификаторЗадания;
	Иначе;
		Результат = ВетИС_APIСервер.ВСД2_ЛабораторныеИсследования_ОтправитьПараллельно(ПараметрыФункции);
	КонецЕсли;
...
Процедура СоздатьПоПродукцииЭлементуНаСервере()
	...
	Для Каждого стрПартия Из Объект.Партии Цикл
		...
		Если стрПартия.Пометка
			...
			Иначе
				ВетИС_ЯдроСервер.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("По продукции [%1] не указаны лабораторные исследования",
