...
// @grace-purpose: Команда открытия отчёта истории списания партии — поддерживает режимы с БСП и без
...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	// LLM_NOTE: Аналогичное ВСД_ОтчетОстаткиПоПартиям ветвление по ИспользуетсяБСП().
	// Два модуля идентичны по структуре намеренно — каждый отчёт должен быть независимым командным модулем.
	//Если Не ИспользуетсяБСП() Тогда

		ПараметрыФормы = Новый Структура("СформироватьПриОткрытии, Партия", Истина, ПараметрКоманды);
	    ОткрытьФорму(
	        "Отчет.кб99_ОтчетСписанияПартии.Форма.ФормаОтчета",
	        ПараметрыФормы,
	...
	//	ФормаОтчёта = ПолучитьФорму("Отчет.кб99_ОтчетСписанияПартии.Форма.ФормаОтчета", ПараметрыФормы, ПараметрыВыполненияКоманды.Источник, ПараметрыВыполненияКоманды.Уникальность, ПараметрыВыполненияКоманды.Окно, ПараметрыВыполненияКоманды.НавигационнаяСсылка);

	//	КомпоновщикНастроек = ФормаОтчёта.Отчет.КомпоновщикНастроек;
	//	УстановитьПараметр(КомпоновщикНастроек, "Партия", ПараметрКоманды, Истина);
...
//   Вход: Использование (Булево, необязат.) — по умолчанию Истина
...
//   Вход: ОбъектНастройки (НастройкиКомпоновкиДанных / КомпоновщикНастроек / др.)
//   Вход: Параметр (Строка / ПараметрКомпоновкиДанных)
//   Выход: ЗначениеПараметраНастроекКомпоновкиДанных или Неопределено
//   GRACE-лог: нет
