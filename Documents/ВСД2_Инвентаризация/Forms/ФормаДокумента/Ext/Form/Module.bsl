...
// GRACE:MODULE_START name="ИнвентаризацияВСД"
// @grace-module: ИнвентаризацияВСД
// @grace-purpose: Форма документа ВСД2_Инвентаризация — ввод и редактирование строк продукции, подбор партий, упаковки и маркировки
// @grace-depends: ИнвентаризацияВСД, ЯдроВСД, НСИ_ВетИС
// @grace-version: 1.0
// @grace-contract-ref: docs/Contracts.xml -> module="ИнвентаризацияВСД"
...
//   Вход: ТекстПредупреждения (Строка) — текст сообщения
//   Выход: нет (показывает немодальное предупреждение)
//   Инвариант: вызывается из внешних модулей через экспорт формы

// @grace-contract: ПослеЗакрытияПредупреждения
//   Вход: Параметры (Структура) — результат закрытия диалога
//   Выход: нет
//   Инвариант: обработчик оповещения из ПредупреждениеПользователю

// @grace-contract: СтруктураОтбораПартий
//   Вход: Выб_Продукция_Элемент (СправочникСсылка, необязательный), Выб_Получатель_Площадка, Выб_Получатель_ХозСубъект
//   Выход: Структура — параметры отбора для подбора партий
//   Инвариант: используется при открытии формы подбора партий
...
Процедура ЗаполнитьРеквизитыСтрокиНаСервере(СтрокаТЧ)
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Партия) Тогда
		...
		СтрокаТЧ.КоличествоРазница = СтрокаТЧ.Количество - СтрокаТЧ.КоличествоДоИзменения;
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.Партия = ВетИС_ЯдроСервер.ВыбратьПартию(СтрокаТабличнойЧасти.Продукция_Элемент, Объект.Владелец_Площадка, Объект.Владелец_ХозСубъект);
...
Процедура ЗаполнитьПросроченнымиТоварамиНаСервере(МинДатаСрокГодности, МаксКолвоПартийВдокументе=100)
	...
	СписокПартий = ВетИС_ЯдроСервер.СписокАктуальныхПартийПоФильтру_Запрос(, Объект.Владелец_Площадка, Объект.Владелец_ХозСубъект);
...
Процедура кнСписание(Команда)
	...
	Для Каждого стрПродукция Из Объект.Продукция Цикл
		...
		стрПродукция.КоличествоРазница = стрПродукция.Количество - стрПродукция.КоличествоДоИзменения;
	КонецЦикла
...
Процедура ПродукцияКоличествоПриИзменении(Элемент)
	...
	текСтр = Элементы.Продукция.ТекущиеДанные;
	Если текСтр <> Неопределено Тогда
		текСтр.КоличествоРазница = текСтр.Количество - текСтр.КоличествоДоИзменения;
	КонецЕсли;

КонецПроцедуры
// GRACE:MODULE_END name="ИнвентаризацияВСД"
