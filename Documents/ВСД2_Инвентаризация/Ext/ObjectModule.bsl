...
// @grace-purpose: Объектный модуль документа ВСД2_Инвентаризация — заполнение строк продукции из партий или данных расхождений
// @grace-depends: ИнвентаризацияВСД, ЯдроВСД, ИнтеграцияВетИС_API, РегистрыПартий
// @grace-version: 1.0
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если (ТипЗнч(ДанныеЗаполнения) = Тип("ТаблицаЗначений")) Тогда 
		...
		Для Каждого СтрПартии Из ДанныеЗаполнения Цикл
			...
			НоваяСтрока.КоличествоРазница	= Разница;
	...
	ИначеЕсли ( ТипЗнч(ДанныеЗаполнения) = Тип("Структура") ) Тогда 
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("РаспределеннаяПродукция") Тогда
			...
			Для Каждого стрПродукция из ДанныеЗаполнения.РаспределеннаяПродукция Цикл
				...
				Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(стрПродукция, "ДатаИзготовления") Тогда
					СтрТовары.ДатаИзготовления1 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПродукция.ДатаИзготовления);	
				КонецЕсли;
				
				Если ВетИС_УтилитыКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(стрПродукция, "ДатаСрокГодности") Тогда
					СтрТовары.ДатаСрокГодности1 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПродукция.ДатаСрокГодности);	
				КонецЕсли;
