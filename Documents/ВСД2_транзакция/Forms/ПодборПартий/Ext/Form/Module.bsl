...
// @grace-purpose: Форма подбора партий для документа ВСД2_транзакция — поиск и распределение партий по строкам
// @grace-depends: ТранзакцияВСД, ДокументооборотВСД, НСИ_ВетИС
// @grace-version: 1.0
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДокументОснование") Тогда
		...
		СпособПодбора = ВетИС_ЗаполнениеДокументовСервер.ПолучитьСпособПодбораПоУмолчанию();
		...
		ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(ДокументОснование.Организация);
		ПоказыватьПросроченныеПартии = ПараметрыОрганизации.ПоказыватьПросроченныеПартии;
...
Процедура ЗаполнитьПоДокументуОснованию()
	...
	тзКПодбору = ВетИС_ЗаполнениеДокументовСервер.ВыгрузитьТч(ДокументОснование);
	...
	тзКПодбору.Свернуть("Номенклатура, Коэффициент", "Количество, Упаковки, Количество1с");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	тчКПодбору.Номенклатура КАК Номенклатура,
	|	тчКПодбору.КлючСтроки КАК КлючСтроки,
	|	тчКПодбору.Коэффициент КАК КоэффициентУпаковок,
	|	тчКПодбору.Количество / тчКПодбору.Количество1с КАК Коэффициент1с
	...
	|	втТчКПодбору.КоэффициентУпаковок КАК Коэффициент,
	|	ВСД_Партия.Количество / втТчКПодбору.КоэффициентУпаковок КАК Упаковки,
	|	ВСД_Партия.Количество / втТчКПодбору.Коэффициент1с КАК Количество1с
	|ИЗ
	...
	|		ПО втТчКПодбору.Номенклатура = ВСД_Соответсвия.Владелец
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаИзготовления1"; 
	...
	Для Каждого Стр Из тзРезультат Цикл
		...
		Если Не ПоказыватьПросроченныеПартии Тогда
			СрокГодности = ВетИС_APIСервер.СтрокаВДату(Стр.СрокГодности1);
			Если СрокГодности < ТекущаяДатаСеанса() Тогда
				Продолжить;	
			КонецЕсли;
		КонецЕсли;
		...
		новСтрока.ДатаИзготовления1 = ?(ЗначениеЗаполнено(Стр.ДатаИзготовления1), ВетИС_APIСервер.СтрокаВДату(Стр.ДатаИзготовления1), Неопределено);
		новСтрока.ДатаИзготовления2 = ?(ЗначениеЗаполнено(Стр.ДатаИзготовления2), ВетИС_APIСервер.СтрокаВДату(Стр.ДатаИзготовления2), Неопределено);
		новСтрока.СрокГодности1 = ?(ЗначениеЗаполнено(Стр.СрокГодности1), ВетИС_APIСервер.СтрокаВДату(Стр.СрокГодности1), Неопределено);
		новСтрока.СрокГодности2 = ?(ЗначениеЗаполнено(Стр.СрокГодности2), ВетИС_APIСервер.СтрокаВДату(Стр.СрокГодности2), Неопределено);
	КонецЦикла;
...
Процедура ПриВводеЧисла(Результат, ТекущиеСтроки) Экспорт 
	...
	Если Результат <> Неопределено Тогда
		...
		Коэфициент1с = ТекущаяСтрокаТчКПодбору.Количество / ТекущаяСтрокаТчКПодбору.Количество1с;
		
		Если СпособПодбора = 1 Тогда
			...
			КоличествоКг = Результат;
			ПодобраноУпаковок = Результат / ?(ТекущаяСтрокаТчКПодбору.Коэффициент = 0, 
												1, ТекущаяСтрокаТчКПодбору.Коэффициент);
		...
		Иначе
			...
			ПодобраноУпаковок = Результат;
			
		КонецЕсли;
			
		Подобрано1с = КоличествоКг / Коэфициент1с;
		
		ТекущаяСтрокаПартииВНаличии.Количество = ТекущаяСтрокаПартииВНаличии.Количество - КоличествоКг;
		ТекущаяСтрокаПартииВНаличии.Упаковки = ТекущаяСтрокаПартииВНаличии.Упаковки - ПодобраноУпаковок;
		ТекущаяСтрокаПартииВНаличии.Количество1с = ТекущаяСтрокаПартииВНаличии.Количество1с - Подобрано1с;
		
		ТекущаяСтрокаТчКПодбору.Количество = ТекущаяСтрокаТчКПодбору.Количество - КоличествоКг;
		ТекущаяСтрокаТчКПодбору.Упаковки = ТекущаяСтрокаТчКПодбору.Упаковки - ПодобраноУпаковок;
		ТекущаяСтрокаТчКПодбору.Количество1с = ТекущаяСтрокаТчКПодбору.Количество1с - Подобрано1с;
		...
		Если стрПодобранныеПартии.Количество() > 0 Тогда
			стрПодобранныеПартии = стрПодобранныеПартии[0];
			стрПодобранныеПартии.Количество = стрПодобранныеПартии.Количество + КоличествоКг;
			стрПодобранныеПартии.Упаковки = стрПодобранныеПартии.Упаковки + ПодобраноУпаковок;
			стрПодобранныеПартии.Количество1с = стрПодобранныеПартии.Количество1с + Подобрано1с; 
		Иначе
			стрПодобранныеПартии = ПодобранныеПартии.Добавить();
			ЗаполнитьЗначенияСвойств(стрПодобранныеПартии, ТекущаяСтрокаПартииВНаличии,, "Количество, Упаковки, Количество1с");
			стрПодобранныеПартии.Количество = КоличествоКг;
			стрПодобранныеПартии.Упаковки = ПодобраноУпаковок;
			стрПодобранныеПартии.Количество1с = Подобрано1с;
		КонецЕсли;
...
Процедура ПартииВНаличииВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущаяСтрокаТчКПодбору.Количество > 0
		...
		Подсказка = "Введите количество" + ?(СпособПодбора = 1, "(кг.)", "(упаковок)");
		Если СпособПодбора = 1 Тогда
...
Процедура ЗавершитьПодбор(Команда)
	...
	Для Каждого Стр Из тчКПодбору Цикл
		Если Стр.Количество > 0 Тогда
			...
			СтрСтруктура.Вставить("Количество1с", Стр.Количество1с);
