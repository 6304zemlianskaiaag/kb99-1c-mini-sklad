...
// @grace-purpose: Форма документа ВСД2_транзакция — управление ВСД-отгрузкой, подбором партий, UI-логика
// @grace-depends: ЯдроВСД, УправлениеФормамиВСД, ДокументооборотВСД, ДлительныеОперации
// @grace-version: 1.0
...
//   Вход: Параметры — параметры оповещения
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
...
//   Вход: Выб_Продукция_Элемент (СправочникСсылка, опционально) — элемент продукции для отбора
//   Вход: Выб_Получатель_Площадка (СправочникСсылка) — площадка получателя
//   Вход: Выб_Получатель_ХозСубъект (СправочникСсылка) — хозсубъект получателя
//   Вход: ВозможнаПодменаПродукции (Булево) — признак возможности подмены
//   Выход: Структура — структура отбора для формы выбора партий
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
...
Функция СтруктураОтбораПартий( Выб_Продукция_Элемент = Неопределено, Выб_Получатель_Площадка, Выб_Получатель_ХозСубъект, ВозможнаПодменаПродукции) Экспорт
	...
	Если ЗначениеЗаполнено(Выб_Продукция_Элемент) Тогда
		...
		//	СтруктураОтбора.Вставить("ВидПродукции", ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(Выб_Продукция_Элемент, "ВидПродукции"));	
		//Иначе
...
Процедура ОбновитьДанныеКолонокТовары()
	...
	Для Каждого Стр Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(стр.Партия) Тогда
			Стр.ДатаСрокГодности = ВетИС_APIСервер.СтрокаВДатаВремя(стр.Партия.ДатаСрокГодности1);
			Стр.ДатаСрокГодности2 = ВетИС_APIСервер.СтрокаВДатаВремя(стр.Партия.ДатаСрокГодности2);
			Стр.ДатаИзготовления = ВетИС_APIСервер.СтрокаВДатаВремя(стр.Партия.ДатаИзготовления1);
			Стр.ДатаИзготовления2 = ВетИС_APIСервер.СтрокаВДатаВремя(стр.Партия.ДатаИзготовления2);
		Иначе
...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Если (Не ЗначениеЗаполнено(Объект.Организация)) и ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			...
			ДокВСД = ВетИС_ЯдроСервер.НайтиВсдТранзакцию(Объект.ДокументОснование, Объект.ЭтоПеремещениеОтПоставщика);
...
Процедура ЗаписатьСоответсвиеХозСубъекта(ДокументОснование, Получатель_ХозСубъект)
	...
	Если НЕ ЗначениеЗаполнено(ДокументОснование) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Не выбран документ основания, соответсвие не записано");
		Возврат;
	КонецЕсли;
	
	Если ВетИС_УтилитыКлиентСервер.ЭтоДокументПеремещение(ДокументОснование)
	...
	Пока Выборка.Следующий() Цикл
		...
		ВетИС_ЯдроСервер.СообщитьИнфо("Изменен хозсубъект " + Выборка.Ссылка);
	КонецЦикла;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Получатель_ХозСубъект) Тогда
		Контрагент = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(Объект.Получатель_ХозСубъект, "Контрагент");
	Иначе
		Контрагент = Неопределено;
	КонецЕсли;
	
	СтрокаТабличнойЧасти.Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре(СтрокаТабличнойЧасти.Номенклатура, Контрагент);
	СтрокаТабличнойЧасти.Партия = ВетИС_ЯдроСервер.ВыбратьПартию(СтрокаТабличнойЧасти.Продукция_Элемент, Объект.Отправитель_Площадка, Объект.Отправитель_ХозСубъект);
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Партия) Тогда
		ТоварыПартияПриИзменении(Элемент);
	Иначе
		СтрокаТабличнойЧасти.ЕдиницаИзмерения = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(СтрокаТабличнойЧасти.Продукция_Элемент, "ЕдиницаИзмерения");
	КонецЕсли;
...
Процедура Установить_Соответствие_ВСД_Продукция_Элемент(Номенклатура, Продукция_Элемент)
	ВетИС_ЯдроСервер.Установить_Соответствие_ВСД_Продукция_Элемент(Номенклатура, Продукция_Элемент);
КонецПроцедуры
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Партия) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Партия) Тогда
			...
			Если ПартияКоличество.ОстатокМеньше Тогда
				...
				ВетИС_ЯдроСервер.СообщитьПользователю(ТекстСообщения,, Поле);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыСтрокиПродукцияСервер(СтрокаТЧ)
	...
	ПереопределенныйМодуль = ВетИС_УтилитыКлиентСервер.ФункцияПереопределена("ЗаполнитьРеквизитыСтрокиПродукцияСервер");
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.Партия) Тогда
		Если ВетИС_ПараметрыСервер.ПроверитьЗначениеПараметра(ПараметрыОрганизации, "РазрешитьПодменуПродукцииВИсходящемВСД", "Разрешить подмену продукции в исходящем ВСД") Тогда
		...
		СтрокаТЧ.ДатаИзготовления = ВетИС_APIСервер.СтрокаВДатаВремя(СтрокаТЧ.Партия.ДатаИзготовления1);
		СтрокаТЧ.ДатаИзготовления2 = ВетИС_APIСервер.СтрокаВДатаВремя(СтрокаТЧ.Партия.ДатаИзготовления2);
		СтрокаТЧ.ДатаСрокГодности = ВетИС_APIСервер.СтрокаВДатаВремя(СтрокаТЧ.Партия.ДатаСрокГодности1);
		...
		СтрокаУпак.Количество = ВетИС_ЗаполнениеДокументовСервер.РассчитатьКоличествоПотребительскихУпаковок(СтрокаУпак, СтрокаТЧ);
	...
	СтрокаТЧ.Цель = СтрокаТЧ.Партия.vetDocument.Цель;
	Если Не ЗначениеЗаполнено(СтрокаТЧ.Цель) Тогда
		СтрокаТЧ.Цель = СтрокаТЧ.Продукция_Элемент.ВидПродукции.Цель;
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Цель) Тогда 
...
Функция кнОтправитьНаСервере()
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне( УникальныйИдентификатор,
		"ВетИС_ЯдроСервер.ОтправитьВСДвГИС",
	Объект.Ссылка,
	НаименованиеЗадания);
...
Процедура ЗаполнитьПросроченнымиТоварамиНаСервере(МинДатаСрокГодности, МаксКолвоПартийВдокументе = 100)
	...
	ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(Объект.Организация);
	СписокПартий = ВетИС_ЯдроСервер.СписокАктуальныхПартийПоФильтру_Запрос(, Объект.Отправитель_Площадка, Объект.Отправитель_ХозСубъект);
	...
	Для Каждого стрПартия Из СписокПартий Цикл
		...
		ДатаСрокГодности1 = ВетИС_APIСервер.СтрокаВДатаВремя(стрПартия.ДатаСрокГодности1);
...
Функция СтруктураРеквизитовСтроки()
	...
	ДанныеСтрокиТЧ.Вставить("Количество1с");
...
Процедура ПодборПоДокументуОснованию(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		Если ВетИС_УтилитыКлиентСервер.ЭтоДокументОтгрузка(Объект.ДокументОснование) Тогда
		...
		Иначе
			ВетИС_ЯдроСервер.СообщитьИнфо("Данная функция работает только при подборе по реализации товаров и услуг");
		КонецЕсли;
	КонецЕсли;
	...
	Если РезультатПодбора <> Неопределено Тогда
		...
		Если ПараметрыОрганизации["ПарамЗаполнятьТранзакциюПриОтсутствииПартий"] Тогда
			Для Каждого СтрНеподобранныеТовары Из РезультатПодбора.НеПодобранныеТовары Цикл
				...
				нСтрТовары.ЕдиницаИзмерения = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(нСтрТовары.Продукция_Элемент, "ЕдиницаИзмерения");
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекСтрока.Партия) Тогда
		...
		УпаковкиПоСтроке = Объект.УровниУпаковки.НайтиСтроки(Новый Структура("СтрокаПродукции", ТекСтрока.КлючСтроки));
		Для Каждого ТекСтрокаУпаковки Из УпаковкиПоСтроке Цикл
			...
			ТекСтрокаУпаковки.Количество = ВетИС_ЗаполнениеДокументовСервер.РассчитатьКоличествоПотребительскихУпаковок(СтрокаУпак, СтрокаТЧ);
...
Процедура ПроверитьВидыПродукции(текСтрока)
	...
	ПродукцияЭлементВидПродукции = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(текСтрока.Продукция_Элемент, "ВидПродукции");
	ПартияВидПродукции = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(текСтрока.Партия, "ВидПродукции");
...
Процедура ТоварыПродукция_ЭлементНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если МожноПодменятьПродукцию И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Партия) Тогда
		СтандартнаяОбработка = Ложь;
		СтруктураОтбора = Новый Структура("ВидПродукции", ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(СтрокаТабличнойЧасти.Партия, "ВидПродукции"));
...
Процедура ЗаписатьСоответсвиеПлощадки(ДокументОснование, Площадка)
	...
	СписокКонстант = ВетИС_ЯдроСервер.ЗагрузитьПараметры(ДокументОснование.Организация);
	...
	ИначеЕсли СписокКонстант["РеквизитГрузополучатель"] = 1 Тогда
		...
		Иначе
			ВетИС_ЯдроСервер.СообщитьИнфо("В документе основании нет реквизита [Адрес доставки]");
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли СписокКонстант["РеквизитГрузополучатель"] = 2 Тогда
		...
		Иначе
			ВетИС_ЯдроСервер.СообщитьИнфо("В документе основании нет реквизита [Партнер]");
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли СписокКонстант["РеквизитГрузополучатель"] = 3 Тогда
		...
		Иначе
			ВетИС_ЯдроСервер.СообщитьИнфо("В документе основании нет реквизита [Грузополучатель]");
			Возврат;
		КонецЕсли;
		
	Иначе	
		// Договор
		Контрагент = ДокументОснование[ПереопределяемыеПоляЗапроса.ПолеДоговор];
	...
	Пока Выборка.Следующий() Цикл
		...
		ВетИС_ЯдроСервер.СообщитьИнфо("Изменена площадка " + Выборка.Ссылка);
	КонецЦикла;
...
Процедура ТочкиМаршрутаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураСтроки = ВетИС_КлиентскийСлойКлиент.СтруктураТочкиМультимодальнойПеревозки();
