...
// @grace-purpose: Менеджер документа ВСД2_транзакция — печать ВСД и набора листов
// @grace-depends: ЯдроВСД, УправлениеФормамиВСД
// @grace-version: 1.0
// @grace-contract-ref: docs/Contracts.xml -> module="ТранзакцияВСД"
// GRACE:DEPENDENCY target="кб99_ВСД"
// GRACE:DEPENDENCY target="ВетИС_ПечатьQRКлиент"

// @grace-contract: Печать
//   Вход: ТабДокумент (ТабличныйДокумент) — документ для вывода
//   Вход: ДокСсылка (ДокументСсылка.ВСД2_транзакция) — ссылка на документ
//   Выход: нет
//   Исключение: нет
//   Инвариант: ДокСсылка должен быть заполнен
//   GRACE-лог: нет
// GRACE:CONTRACT function="Печать"

// @grace-contract: ПечатьНаборныйЛист
//   Вход: ТабДок (ТабличныйДокумент) — документ для вывода
//   Вход: Ссылка (ДокументСсылка.ВСД2_транзакция или Массив) — ссылка(и) на документ(ы)
//   Выход: нет
//   Исключение: нет
//   Инвариант: нет
//   GRACE-лог: нет
...
Процедура Печать( ТабДокумент, ДокСсылка) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДокСсылка) Тогда
		ВетИС_ЯдроСервер.СообщитьИнфо("Не выбран документ ВСД для печати");
		Возврат;
	КонецЕсли;
	
	ТекстЗапроса = ЗапросПоТранзакции();
	
	ДанныеQRКода = ВетИС_ПечатьQRКлиент.ДанныеQRКода("Проверка", 1, 190);
	...
	Пока Выборка.Следующий() цикл
		...
		ДанныеQRКода = ВетИС_ПечатьQRКлиент.ДанныеQRКода(ШтрихКод, 1, 190);
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаТовары.Номенклатура) Тогда
				ОбластьТовары.Параметры.Номенклатура = ВетИС_ЯдроСервер.ПолучитьНоменклатуруПоПродукцияЭлемент(ВыборкаТовары.Продукция_Элемент, Истина);
			КонецЕсли;
