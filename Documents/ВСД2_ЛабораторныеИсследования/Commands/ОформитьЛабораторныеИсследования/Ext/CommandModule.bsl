...
// @grace-purpose: Команда оформления лабораторных исследований — создание или открытие документов ВСД2_ЛабораторныеИсследования
// @grace-depends: ПроизводствоВСД, ЯдроВСД, НСИ_ВетИС
// @grace-version: 1.0
...
Функция ОформитьЛабИсследования(ДокОснование)
	...
	ПараметрыОрганизации = ВетИС_ЯдроСервер.ЗагрузитьПараметры(ДокОснование.Организация);
	...
	Пока ВыборкаПартии.Следующий() Цикл
		...
		Если НЕ СокрЛП(ВыборкаПартии.СтатусПартии) = "103"
			...
			ВетИС_ЯдроСервер.СообщитьИнфо(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Партия %1 была изменена. Оформление лабораторных исследований невозможно",
		...
		// LLM_NOTE: Запись нового документа лабораторных исследований без проведения.
		// Документ создаётся или обновляется на основе партии. НЕ добавлять проведение — оно не предусмотрено.
