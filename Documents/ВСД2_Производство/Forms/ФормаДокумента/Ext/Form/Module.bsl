...
// @grace-purpose: Форма документа ВСД2_Производство — управление производственными партиями, сырьём, UI-логика
// @grace-depends: ЯдроВСД, УправлениеФормамиВСД, ПроизводствоВСД, ИнтеграцияВетИС_API
// @grace-version: 1.0
...
Процедура ПриОткрытии(Отказ)
	Если Параметры.Ключ.Пустая() Тогда
		Если (Не ЗначениеЗаполнено(Объект.Организация)) и ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			...
			ДокВСД = ВетИС_ЯдроСервер.НайтиВСД_Производство(Объект.ДокументОснование);
...
Процедура Производитель_ПлощадкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// Отбор по ХС включим
	ГУИДХСдляОтбора = ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(Объект.Производитель_ХозСубъект,"GUID");
...
Функция ПолучитьРеквизитыЭлемента(ВыбЭлемент)
	...
	Рез.Вставить("Скоропортящийся",		ВыбЭлемент.Скоропортящийся);
...
Процедура ЗаполнитьРеквизитыСтрокиПродукция(СтрокаТЧ,ЧтоИзменили)
	...
	СтрокаТЧ.НомерПартии = ВетИС_ПроизводствоСервер.ПолучитьНомерПартии(Новый Структура, "");	
	...
	КонецЕсли;
	СтрокаТЧ.Скоропортящийся =  ПродЭлемент.Скоропортящийся;
	
	РеквизитыДляЗаполнения = Новый Структура("КлючСтроки, Количество, Продукция_Элемент");
	ЗаполнитьЗначенияСвойств(РеквизитыДляЗаполнения, СтрокаТЧ);
	
	ЗаполнитьУровниУпаковкиИМаркировку(РеквизитыДляЗаполнения);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьУровниУпаковкиИМаркировку(СтрокаТЧ)
	
	ВетИС_ПроизводствоСервер.ЗаполнитьУровниУпаковкиИМаркировку(СтрокаТЧ, Объект, Неопределено);	
...
Процедура ПродукцияНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре( СтрокаТабличнойЧасти.Номенклатура );
...
Процедура ПродукцияДатаИзготовления1ПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.ДатаСрокГодности1 = СтрокаТабличнойЧасти.ДатаИзготовления1+60*60*24*ВетИС_ЯдроСервер.ПолучитьЗначениеРевизитаОбъекта_НаСервере(СтрокаТабличнойЧасти.Продукция_Элемент,"СрокГодности");
КонецПроцедуры
...
Процедура ПартииСписанияНоменклатураПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.Продукция_Элемент = ВетИС_ЯдроСервер.Продукция_Элемент_ПолучитьПоНоменклатуре( СтрокаТабличнойЧасти.Номенклатура );
	СтрокаТабличнойЧасти.Партия = ВетИС_ЯдроСервер.ВыбратьПартию(СтрокаТабличнойЧасти.Продукция_Элемент, Объект.Производитель_Площадка, Объект.Производитель_ХозСубъект);
...
Процедура ПартииСписанияПартияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПодбора = Новый Структура("ЗакрыватьПриВыборе, РежимВыбора, МножественныйВыбор, Отбор, ПоказыватьПросроченныеПартии",
										Истина, Истина, Ложь, СтруктураОтбора, ПоказыватьПросроченныеПартии);	
...
Процедура ПартииСписанияПартияПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) и ЗначениеЗаполнено(СтрокаТабличнойЧасти.Продукция_Элемент) и АвтоЗаписьВСДСоответствия Тогда
		ВетИС_ЯдроСервер.Установить_Соответствие_ВСД_Продукция_Элемент(СтрокаТабличнойЧасти.Номенклатура,СтрокаТабличнойЧасти.Продукция_Элемент);	
	КонецЕсли;
...
Процедура кнЗаполнитьПоПродукцияЭлементНаСервере()
	ЭлементыСпецификации = ВетИС_ПроизводствоСервер.ЗаполнитьСырьеИзВСД_Продукция_Элемент( Объект.Продукция.Выгрузить() );
...
Процедура кнЗаполнитьПартииСписанияПоВСД_Продукция_ЭлементНаСервере()
	...
	Сырье = ВетИС_ПроизводствоСервер.ЗаполнитьСырьеИзВСД_Продукция_Элемент( Объект.Продукция.Выгрузить() );
	
	ПартииСписания = ВетИС_ЯдроСервер.тзПартииСписанияПоТзПродукция_Элемент( ПараметрыФункции, Сырье );
...
Процедура кнЗаполнитьПоСпецификацииНоменклатурыНаСервере()
	...
	ПартииСписания = ВетИС_ПроизводствоСервер.ЗаполнитьТчСписаниеИзСпецификацииНоменклатуры( ПараметрыФункции, Объект.Продукция.Выгрузить() );
